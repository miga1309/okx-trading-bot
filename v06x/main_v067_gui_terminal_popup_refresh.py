# ============================================================
# OKX Turtle Bot
# Version: v067_gui_terminal_popup_refresh
# Date: 2026-03-14
#
...
def build_analysis_export_bundle(snapshot: Optional[dict] = None, cfg: Optional["BotConfig"] = None) -> Path:
    ...
    filter_diagnostic_rows = [row for row in engine_event_rows if str(row.get("event") or "") == "filter_diagnostics"]
    filter_headers = sorted({key for row in filter_diagnostic_rows for key in row.keys()} or {"ts", "event"})
    _write_csv(export_dir / "filter_diagnostics.csv", filter_headers, filter_diagnostic_rows)
...
class BotConfig:
    ...
    trend_stop_activation_r: float = 1.20
    trend_stop_peak_atr_multiple: float = 1.60
...
class EntryContextDialog(QDialog):
    def __init__(self, payload: dict, context_file: str, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLabel { color: #e6ecff; }
            QFrame { background: #08111f; border: 1px solid #1b3b63; border-radius: 16px; }
            QTextEdit {
                background: #08111f;
                color: #d8f6ff;
                border: 1px solid #1b3b63;
        ...
        side_color = '#00FFA8' if side_value == 'long' else ('#FF3A5E' if side_value == 'short' else '#475569')
        ...
        title.setStyleSheet("font-size: 16px; font-weight: 800; color: #edf6ff;")
        ...
        side_badge.setStyleSheet(
            f"font-size: 15px; font-weight: 900; color: #06111f; background: {side_color}; border: 1px solid #c8fff0; border-radius: 14px; padding: 7px 14px;"
        )
...
class TurtleEngine(QObject):
    ...
    def _collect_trade_ready_metrics(self) -> dict:
        ...
        blacklist = set(getattr(self.cfg, "blacklist", []) or [])
        blocked = set(getattr(self, "blocked_instruments", {}).keys())
        temp_blocked = set()
        now_ts = time.time()
        for inst_id, until_ts in list(getattr(self, "temp_blocked_until", {}).items()):
            try:
                ...
                continue
        illiquid = set(getattr(self, "illiquid_instruments", {}).keys())
        stopout = set()
        for inst_id, data in list(getattr(self, "recent_stopouts", {}).items()):
            try:
                if float((data or {}).get("until", 0.0) or 0.0) > now_ts:
                    stopout.add(inst_id)
            except Exception:
                continue
        execution_watch = set(getattr(self, "execution_risk_events", {}).keys())
        open_positions = set(getattr(self, "position_state", {}).keys())
        hard_blocked = blacklist | blocked | temp_blocked | illiquid | stopout
        eligible = [inst_id for inst_id in universe if inst_id not in hard_blocked]
        ...
        metrics = {
            ...
            "blacklist_count": len(blacklist & set(universe)),
            "blocked_count": len(blocked & set(universe)),
            "temp_blocked_count": len(temp_blocked & set(universe)),
            "illiquid_count": len(illiquid & set(universe)),
            "stopout_cooldown_count": len(stopout & set(universe)),
            "execution_watchlist_count": len(execution_watch & set(universe)),
            "open_positions_count": len(open_positions & set(universe)),
    ...
    def evaluate_entry(self, inst_id: str, cycle_id: Optional[int] = None) -> list[dict]:
        ...
        for signal in signals:
            ...
            if ok:
                ...
                quality_score = 0.0
                preferred_min = float(getattr(self.cfg, "preferred_breakout_distance_atr_min", 0.05) or 0.05)
                preferred_max = float(getattr(self.cfg, "preferred_breakout_distance_atr_max", 0.30) or 0.30)
                ...
                if 0.18 <= atr_pct <= 0.45:
                    quality_score += 2.0
                elif 0.45 < atr_pct <= 0.80:
                    quality_score += 1.0
                channel_ratio = float(vol_metrics.get("channel_atr_ratio", 0.0))
                if 1.6 <= channel_ratio <= 4.5:
                    quality_score += 2.0
                elif 4.5 < channel_ratio <= 7.5:
                    quality_score += 0.8
    ...
    def is_flat_market(self, candles: List[List[float]], price: float, atr: float) -> tuple[bool, str]:
        ...
        if channel_range_pct < self.cfg.min_channel_range_pct * 0.60 * strict_min:
            return True, f"крайне узкий диапазон {channel_range_pct:.3f}%"
        if atr_pct < self.cfg.min_atr_pct * 0.68 * strict_min:
            return True, f"крайне низкий ATR {atr_pct:.3f}%"
        if channel_atr_ratio < self.cfg.flat_min_channel_atr_ratio * 0.76 * strict_min:
        ...
        if channel_atr_ratio < self.cfg.flat_min_channel_atr_ratio * 0.88 * strict_min and efficiency_ratio < self.cfg.min_efficiency_ratio * 0.85 * strict_min:
    ...
    def _confirm_breakout(self, candles: list, atr: float, side: str, level: float) -> tuple[bool, str]:
        ...
        min_body_ratio = float(getattr(self.cfg, "min_body_to_range_ratio", 0.0) or 0.0)
        ...
        max_breakout_dist = float(getattr(self.cfg, "breakout_max_distance_atr", 999.0) or 999.0)
    ...
    def emit_snapshot(self) -> None:
        ...
        payload = {
            ...
            "analytics": {
                ...
                "blocked_total": int((trade_ready_metrics.get("blacklist_count", 0) or 0) + (trade_ready_metrics.get("blocked_count", 0) or 0) + (trade_ready_metrics.get("temp_blocked_count", 0) or 0) + (trade_ready_metrics.get("illiquid_count", 0) or 0) + (trade_ready_metrics.get("stopout_cooldown_count", 0) or 0)),
                "open_pnl": open_pnl,
...
class MainWindow(QMainWindow):
    ...
    def reload_ui(self) -> None:
        ...
        try:
            module_name = getattr(self, "_ui_layout_module_name", "ui_layout_v062_5_singleline_riskbar")
    ...
    def _build_ui(self) -> None:
        self._ui_layout_module_name = "ui_layout_v062_5_singleline_riskbar"
    ...
    def refresh_blocked_instruments_view(self) -> None:
        ...
        if hasattr(self, "lbl_ready_count") and self.engine is not None:
            try:
                metrics = self.engine._collect_trade_ready_metrics()
                self.lbl_ready_count.setText(f"READY: {int(metrics.get('trade_ready_count', 0))}/{int(metrics.get('scan_universe_total', 0))}")
            except Exception:
                pass
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        if hasattr(self, 'lbl_ready_count') and self.lbl_ready_count is not None:
            self.lbl_ready_count.setText(f"READY: {ready_count}/{scan_total}")
