...
def build_main_window_ui(window, deps):
    ...
    chip_row.addWidget(window.btn_reload_ui)
    ...
    balance_layout.addWidget(window.lbl_balance_equity)
    ...
    window.risk_radar_visual.setMaximumHeight(150)
    ...
    # Equity pulse taller and cleaner
    ...
    window.balance_chart.setMinimumHeight(250)
    chart_layout.addWidget(window.balance_chart, 1)
    center_col.addWidget(chart_box, stretch=4)

    # Wider Turtle signal center in main working area
    ...
    turtle_head.setSpacing(8)
    window.turtle_glyph = NeonGlyphWidget()
    window.turtle_glyph.setMinimumSize(82, 82)
    window.turtle_glyph.setMaximumSize(92, 92)
    turtle_head.addWidget(window.turtle_glyph, alignment=Qt.AlignmentFlag.AlignTop)

    turtle_diag = QVBoxLayout()
    turtle_diag.setSpacing(6)
    ...
    turtle_diag.addWidget(window.lbl_turtle_regime)
    ...
    window.lbl_turtle_next_candidate = QLabel("NEXT CANDIDATE: —")
    window.lbl_turtle_next_candidate.setProperty("card", "true")
    window.lbl_turtle_filter_stack = QLabel("FILTER STACK: flat → structure → liquidity → breakout")
    window.lbl_turtle_filter_stack.setProperty("card", "true")
    diag_grid.addWidget(window.lbl_turtle_state_a, 0, 0)
    diag_grid.addWidget(window.lbl_turtle_state_b, 0, 1)
    diag_grid.addWidget(window.lbl_turtle_state_c, 1, 0)
    diag_grid.addWidget(window.lbl_turtle_score, 1, 1)
    diag_grid.addWidget(window.lbl_turtle_next_candidate, 2, 0)
    diag_grid.addWidget(window.lbl_turtle_filter_stack, 2, 1)
    turtle_diag.addLayout(diag_grid)
    turtle_head.addLayout(turtle_diag, 1)
    turtle_layout.addLayout(turtle_head)
    center_col.addWidget(turtle_box, stretch=2)

    # Market radar reduced to one compact row
    ...
    # Activity feed smaller diagnostic block
    ...
    top_row.addLayout(center_col, 6)
    top_row.addLayout(right_col, 4)
    layout.addLayout(top_row, stretch=5)

    # Bottom tabs bigger
    window.tabs = QTabWidget()

    window.table = QTableView()
    window.table.setMinimumHeight(380)
    ...
    window.tabs.setMinimumHeight(380)
    layout.addWidget(window.tabs, stretch=6)
