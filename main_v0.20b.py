    ...
    if is_dark:
        return """
            ...
            QTableView {
                ...
                alternate-background-color: #16202f;
                background: #0f172a;
                color: #e5e7eb;
            }
            QTableView::item {
...
class TurtleEngine(QObject):
    ...
    def evaluate_entry(self, inst_id: str) -> None:
        lookback = max(self.cfg.long_entry_period, self.cfg.short_entry_period, self.cfg.atr_period, self.cfg.long_exit_period, self.cfg.short_exit_period) + 5
        ...
        if atr <= 0:
            return
        is_flat, flat_reason = self.is_flat_market(candles, last[4], atr)
        if is_flat:
            logging.info("%s: пропуск входа, ровный рынок (%s)", inst_id, flat_reason)
            return

        long_window = candles[-self.cfg.long_entry_period :]
        short_window = candles[-self.cfg.short_entry_period :]
        long_breakout = last_high >= max(c[2] for c in long_window)
        short_breakout = last_low <= min(c[3] for c in short_window)

        if long_breakout:
            self.stats_logger.log("entry_signal", inst_id=inst_id, side="long", price=last[4], atr=atr, system_name="Turtle 55", timeframe=self.cfg.timeframe)
            self.enter_position(inst_id, "long", last[4], atr, "Turtle 55")
        elif short_breakout:
            self.stats_logger.log("entry_signal", inst_id=inst_id, side="short", price=last[4], atr=atr, system_name="Turtle 20", timeframe=self.cfg.timeframe)
            self.enter_position(inst_id, "short", last[4], atr, "Turtle 20")
    ...
    def is_flat_market(self, candles: List[List[float]], price: float, atr: float) -> tuple[bool, str]:
        ...
        lookback = min(len(candles), max(5, self.cfg.flat_lookback_candles))
        window = candles[-lookback:]
        highs = [c[2] for c in window]
        lows = [c[3] for c in window]
        opens = [c[1] for c in window]
        closes = [c[4] for c in window]
        channel_range_pct = ((max(highs) - min(lows)) / price) * 100.0 if highs and lows else 0.0
        atr_pct = (atr / price) * 100.0 if price > 0 else 0.0
        repeated_close_ratio = 0.0
        if len(closes) > 1:
            unchanged = sum(1 for i in range(1, len(closes)) if abs(closes[i] - closes[i - 1]) <= max(price * 0.00005, 1e-12))
            repeated_close_ratio = unchanged / (len(closes) - 1)

        candle_ranges = [max(c[2] - c[3], 1e-12) for c in window]
        body_ratios = [abs(c[4] - c[1]) / rng for c, rng in zip(window, candle_ranges)]
        ...
        for opn, cls in zip(opens, closes):
            delta = cls - opn
            if abs(delta) <= max(price * 0.00003, 1e-12):
                directions.append(0)
            else:
        ...
        if repeated_close_ratio >= 0.55:
            return True, f"слишком много одинаковых закрытий {repeated_close_ratio:.0%}"
        if avg_body_ratio < self.cfg.min_body_to_range_ratio:
            return True, f"слишком маленькие тела свечей {avg_body_ratio:.2f}"
        if efficiency_ratio < self.cfg.min_efficiency_ratio and flip_ratio >= self.cfg.max_direction_flip_ratio:
            return True, f"пила без направления: ER {efficiency_ratio:.2f}, flips {flip_ratio:.0%}"
        return False, ""
