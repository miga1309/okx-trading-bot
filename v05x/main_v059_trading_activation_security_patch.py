# ============================================================
# OKX Turtle Bot
# Version: v059_trading_activation_security_patch
# Date: 2026-03-13
#
# Changelog v059:
# - Softened 5m flat/structure entry filters to let more valid Turtle breakouts pass diagnostic testing
# - Added ATR warmup guards and cleaner rejection reasons for markets without enough stable history
# - Added signal funnel export, near-pass diagnostics and richer cycle statistics for post-test analysis
# - Sanitized analysis export so OKX/Telegram secrets are redacted and only safe strategy config is exported
#
# Previous changelog:
# Changelog v058:
...
def build_analysis_export_bundle(snapshot: Optional[dict] = None, cfg: Optional["BotConfig"] = None) -> Path:
    ...
    # Config snapshot
    config_snapshot = _sanitize_for_export(asdict(cfg) if cfg is not None else {})
    ...
    (export_dir / "strategy_config.json").write_text(json.dumps({
        "exported_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "version": APP_VERSION,
    ...
    (export_dir / "signal_funnel.json").write_text(json.dumps(signal_funnel, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    if secret_hits:
        (export_dir / "secret_scan_alert.json").write_text(json.dumps({
            "exported_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "hits": secret_hits,
        }, ensure_ascii=False, indent=2), encoding="utf-8")
...
class BotConfig:
    ...
    breakout_buffer_atr: float = 0.16
    breakout_min_body_atr: float = 0.14
    breakout_close_near_extreme_ratio: float = 0.30
    breakout_min_range_expansion: float = 0.00
    breakout_max_prebreak_distance_atr: float = 0.60
    breakout_retest_invalid_ratio: float = 1.00
    breakout_volume_factor: float = 0.00
    flat_max_repeated_close_ratio: float = 0.72
    flat_max_inside_ratio: float = 0.78
    flat_max_wick_to_range_ratio: float = 0.78
    flat_min_channel_atr_ratio: float = 1.80
    flat_max_micro_pullback_ratio: float = 0.90
...
class TurtleEngine(QObject):
    ...
    def is_flat_market(self, candles: List[List[float]], price: float, atr: float) -> tuple[bool, str]:
        ...
        if self._timeframe_seconds() <= 300:
            strict_min *= 0.88
        ...
        if len(hard_flags) >= 2 and len(soft_flags) >= 2:
            return True, "; ".join((hard_flags + soft_flags)[:3])

        if channel_atr_ratio < self.cfg.flat_min_channel_atr_ratio * 0.94 * strict_min and efficiency_ratio < self.cfg.min_efficiency_ratio * 0.96 * strict_min:
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.btn_start_bot.setMinimumHeight(40)
        ...
        self.btn_export_analysis.setMinimumHeight(32)
        ...
        self.btn_reset_test.setMinimumHeight(32)
        ...
        self.btn_clear_bans.setMinimumHeight(32)
        ...
        metric_grid.addWidget(self.lbl_balance_equity, 1, 0, 1, 3)
        ...
        cards=[self.lbl_open_pnl,self.lbl_realized,self.lbl_winrate,self.lbl_closed_stats,self.lbl_avg_open,self.lbl_best,self.lbl_worst,self.lbl_long_short,self.lbl_trade_speed]
        ...
        self.glow_band.setFixedHeight(24)
