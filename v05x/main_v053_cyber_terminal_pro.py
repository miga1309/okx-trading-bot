# ============================================================
# OKX Turtle Bot
# Version: v053_cyber_terminal_refit
# Date: 2026-03-13
#
# Changelog v053_cyber_terminal_refit:
# - Rebuilt the FullHD GUI layout to reduce panel overlap and improve spacing stability
# - Clarified control semantics with labeled start/stop, mode, reset and export buttons
# - Reworked Market Radar into an informative scanner with status, reason and quieter fallback animation
# - Tightened right-column composition and converted Turtle Signal Center into a stable neon panel
# - Slimmed the lower neon flow band and improved header telemetry wording for a cleaner terminal UX
#
# Previous changelog:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"OKX Turtle Bot {APP_VERSION} — Cyber Terminal Pro FullHD")
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_terminal_title = QLabel("OKX TURTLE BOT v052  —  CYBER TERMINAL PRO")
        ...
        self.btn_start_bot.clicked.connect(self.start_engine_from_controls)
        button_grid.addWidget(self.btn_start_bot, 0, 0)

        self.btn_stop_bot = QPushButton("STOP BOT")
        self.btn_stop_bot.clicked.connect(self.stop_engine)
        button_grid.addWidget(self.btn_stop_bot, 0, 1)
        ...
        self.lbl_balance_equity = QLabel("EQUITY\n0 USDT")
        self.lbl_balance_equity.setProperty("card", "true")
        self.lbl_balance_formula = QLabel("BALANCE - USED = AVAILABLE\n0 - 0 = 0")
        self.lbl_balance_formula.setProperty("card", "true")
        metric_grid.addWidget(self.lbl_balance_used, 0, 0)
        metric_grid.addWidget(self.lbl_balance_available, 0, 1)
        metric_grid.addWidget(self.lbl_balance_equity, 1, 0)
        metric_grid.addWidget(self.lbl_balance_formula, 1, 1)
        ...
        self.turtle_glyph.setMinimumSize(108, 108)
        self.turtle_glyph.setMaximumHeight(120)
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        if hasattr(self, 'lbl_balance_equity'): self.lbl_balance_equity.setText('EQUITY\n0 USDT')
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        self.lbl_balance_equity.setText(f"EQUITY\n{equity_total:,.2f} USDT")
        ...
        liquidity_pass = "PASS" if analytics.get('liquidity_pass_rate', 0.0) >= 50 else "WAIT"
        self.lbl_turtle_regime.setText(f"Кандидат: {regime_inst} | Режим: {regime_label} | Entry allowed: {entry_allowed}")
        self.lbl_turtle_state_a.setText(f"DONCHIAN / REGIME: {regime_label} | Score {regime_score}/4")
        self.lbl_turtle_state_b.setText(f"TREND / ATR: ch/ATR {regime_channel:.2f} | eff {regime_eff:.2f}")
        self.lbl_turtle_state_c.setText(f"LIQUIDITY: {liquidity_pass} | ATR {regime_atr_pct:.2f}% | MODE {mode.upper()}")
        if regime_label == "Трендовый":
            ...
            turtle_style = ""
        self.lbl_turtle_score.setText(f"ENTRY STATUS: {entry_allowed} | ENGINE SCORE {regime_score}/4")
        ...
        preferred_watch = []
        if self.engine is not None and getattr(self.engine, 'last_scan_candidates', None):
            preferred_watch = list(self.engine.last_scan_candidates)[:6]
        if not preferred_watch:
            preferred_watch = list((getattr(self.current_cfg, 'execution_risk_watchlist', []) if self.current_cfg else []) or [])
        for inst in preferred_watch:
            if len(radar_rows) >= 6:
                break
            if any(r[0] == inst for r in radar_rows):
                continue
            if inst in blocked:
                radar_rows.append((inst, 0.0, 0.0, 'FILTERED', 41.0, 'Low liquidity / risky exit'))
            else:
                radar_rows.append((inst, 0.0, 0.0, 'WATCH', 18.0, 'Ожидание валидного сигнала'))
        while len(radar_rows) < 6:
            radar_rows.append((f'SCAN-{len(radar_rows)+1}', 0.0, 0.0, 'WATCH', 0.0, 'Ожидание валидного сигнала'))
