# ============================================================
# OKX Turtle Bot
# Version: v052_cyber_terminal_refit
# Date: 2026-03-13
#
# Changelog v052_cyber_terminal_refit:
...
class LaunchConfigWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        launch_layout.setContentsMargins(10, 16, 10, 10)
        ...
        button_grid.addWidget(self.btn_export_analysis, 2, 0, 1, 2)

        self.btn_reset_test = QPushButton("RESET ТЕСТА")
        self.btn_reset_test.clicked.connect(self.reset_test_run)
        button_grid.addWidget(self.btn_reset_test, 3, 0)

        self.btn_clear_bans = QPushButton("CLEAR WATCHLIST / BANS")
        self.btn_clear_bans.clicked.connect(self.clear_ban_lists)
        button_grid.addWidget(self.btn_clear_bans, 3, 1)
        ...
        self.lbl_balance_summary = QLabel("Balance: 0  |  Used: 0  |  Available: 0")
        ...
        self.lbl_risk_panel = QLabel("Equity: 0.00 | Использовано риска: 0.00% / 0.00%")
        ...
        risk_layout.setContentsMargins(10, 16, 10, 10)
        ...
        chart_layout = QVBoxLayout(chart_box)
        chart_layout.setContentsMargins(10, 16, 10, 10)
        ...
        radar_layout.setContentsMargins(10, 16, 10, 10)
        ...
        analytics_layout.setContentsMargins(10, 16, 10, 10)
        ...
        turtle_layout.setContentsMargins(10, 16, 10, 10)
        turtle_layout.setSpacing(8)
        self.turtle_glyph = NeonGlyphWidget()
        self.turtle_glyph.setMaximumSize(128, 128)
        ...
        self.lbl_turtle_regime.setMinimumHeight(48)
        self.lbl_turtle_regime.setWordWrap(True)
        self.lbl_turtle_state_a = QLabel("DONCHIAN: —")
        self.lbl_turtle_state_a.setProperty("card", "true")
        self.lbl_turtle_state_b = QLabel("TREND / ATR: —")
        self.lbl_turtle_state_b.setProperty("card", "true")
        self.lbl_turtle_state_c = QLabel("ENTRY MODE: —")
        self.lbl_turtle_state_c.setProperty("card", "true")
        self.lbl_turtle_score = QLabel("SCORE: — / 4")
        ...
        activity_layout.setContentsMargins(10, 16, 10, 10)
        ...
        right_col.addWidget(activity_box, stretch=4)
    ...
    def _apply_trade_mode_to_controls(self, mode: str) -> None:
        ...
        if hasattr(self, "mode_switch_toggle"):
            ...
            self.mode_switch_toggle.setText("MODE: MANUAL CONFIRMATION" if normalized == "manual" else "MODE: AUTO EXECUTION")
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        self.lbl_balance_summary.setText(f"Balance: {bal_total:,.2f}  |  Used: {bal_used:,.2f}  |  Available: {bal_avail:,.2f}")
        ...
        self.lbl_risk_panel.setText(f"Equity: {bal_total + analytics.get('open_pnl', 0.0):,.2f} | Использовано риска: {analytics.get('used_risk_pct', 0.0):.2f}% / {analytics.get('max_risk_budget_pct', 0.0):.2f}%")
        ...
        self.lbl_turtle_score.setText(f"SCORE: {regime_score}/4 | {regime_inst}")
        self.lbl_turtle_state_a.setText(f"DONCHIAN: {regime_label}")
        self.lbl_turtle_state_b.setText(f"TREND / ATR: ch/ATR {regime_channel:.2f} | eff {regime_eff:.2f}")
        self.lbl_turtle_state_c.setText(f"ENTRY MODE: ATR {regime_atr_pct:.2f}% | {mode.upper()}")
        ...
        # market radar
        radar_rows = []
        for row in sorted(open_positions, key=lambda x: abs(float(x.get("pnl_pct", 0.0))), reverse=True)[:4]:
            ...
            score = min(99.0, abs(pnl_pct) * 8.0 + max(0.0, trend) * 15.0 + 20.0)
            reason = f"Open position | units {int(row.get('units', 1) or 1)} | trend {trend:.2f} ATR"
            values = [pnl_pct * 0.18 + trend * 0.05 + (j - 8) * 0.012 for j in range(18)]
            radar_rows.append((row.get("inst_id", "—"), pnl_pct, values, status, score, reason))

        blocked_rows = self._collect_blocked_rows()[:2]
        for inst_id, block_type, reason, _ttl in blocked_rows:
            if len(radar_rows) >= 6:
                break
            radar_rows.append((inst_id, 0.0, [0.0] * 18, "FILTERED", 38.0, f"{block_type}: {reason}"))

        while len(radar_rows) < 6:
            radar_rows.append((f"SCAN-{len(radar_rows)+1}", 0.0, [0.0] * 18, "WATCH", 0.0, "Ожидание валидного сигнала"))

        for tile, (inst, pnl_pct, values, status, score, reason) in zip(self.market_tiles, radar_rows):
