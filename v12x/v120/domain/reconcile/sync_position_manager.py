...
def restore_sync_position_state(state, cfg, inst_id: str, pos_side: str, avg_px: float, last_px: float, qty: float, atr: float, journal_file: str | Path | None = None) -> list[str]:
    ...
    try:
        stop_qty = abs(float(getattr(state, "exchange_stop_qty", 0.0) or 0.0))
        pos_qty = abs(float(getattr(state, "qty", 0.0) or 0.0))
        if stop_qty > 0.0 and pos_qty > 0.0 and abs(stop_qty - pos_qty) > max(1e-9, pos_qty * 0.01):
            state.exchange_stop_full_position = False
            state.exchange_stop_status = str(getattr(state, "exchange_stop_status", "") or "partial_coverage_fault")
            restored.append("exchange_stop_full_position")
    except Exception:
        pass
    return restored
