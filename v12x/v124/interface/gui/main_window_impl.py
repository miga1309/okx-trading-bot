...
def _append_watchdog_event(event: str, **payload) -> None:
    try:
        path = LOG_DIR / "watchdog_events.jsonl"
        row = {"time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "event": str(event or "")}
        row.update(payload)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._snapshot_refresh_interval_sec = 4
        ...
        self._telegram_worker_check_timer = None
        ...
        self._error_indicator_timer.start(3000)
        ...
        self._runtime_transition_profile = {"gui_ms": 2300, "panel_ms": 0, "grid_ms": 0, "spark_ms": 0, "band_ms": 0, "radar_ms": 0, "glyph_ms": 0}
        self._runtime_trading_profiles = {"balanced": {"gui_ms": 1800, "panel_ms": 0, "grid_ms": 0, "spark_ms": 0, "band_ms": 0, "radar_ms": 0, "glyph_ms": 0}}
    ...
    def _restore_ui_state(self, state: dict | None) -> None:
        ...
        try:
            ...
            pass
        try:
            self._apply_ui_mode_to_controls("balanced")
    ...
    def _apply_runtime_ui_profile(self) -> None:
        ...
        if mode == 'runtime_transition':
            ...
            return
        profile = self._runtime_trading_profiles.get('balanced', next(iter(self._runtime_trading_profiles.values())))
    ...
    def _get_live_position_state(self, inst_id: str, trade_id: str = "", row: dict | None = None) -> dict:
        inst_id = str(inst_id or "").strip()
        trade_id = str(trade_id or "").strip()
        live_state = dict(row or {})
        latest_snapshot = dict(getattr(self, "latest_snapshot", {}) or {})
        for open_row in list(latest_snapshot.get("open_positions") or []):
            if str(open_row.get("inst_id") or "").strip() != inst_id:
                continue
            if trade_id and str(open_row.get("trade_id") or "").strip() not in {"", trade_id}:
                continue
            merged = dict(open_row or {})
        ...
        registry_state = None
        try:
            if engine is not None and hasattr(engine, "position_registry"):
                registry_state = engine.position_registry.get(inst_id)
            elif engine is not None and hasattr(engine, "position_state"):
                registry_state = engine.position_state.get(inst_id)
        except Exception:
            registry_state = None
        if registry_state is not None:
            for key in (
                "inst_id", "side", "qty", "avg_px", "last_px", "unrealized_pnl", "margin", "atr", "stop_price",
                "next_pyramid_price", "entry_time", "base_unit_qty", "units", "system_name", "entry_period", "exit_period",
                "initial_stop_price", "trade_id", "risk_amount_usdt", "position_notional_usdt", "active_stop_mode",
                "state_tag", "position_health_state", "stop_state",
            ):
                try:
                    value = getattr(registry_state, key)
                except Exception:
                    continue
    ...
    def _build_position_markers(self, inst_id: str, trade_id: str, live_payload: dict, live_state: dict, payload: dict) -> list:
        markers = []
        journal_rows = _read_jsonl_rows(POSITION_JOURNAL_FILE) if POSITION_JOURNAL_FILE.exists() else []
        matched = [r for r in journal_rows if str(r.get("inst_id") or "").strip() == inst_id and (not trade_id or str(r.get("trade_id") or "").strip() == trade_id)]
        candle_times = [int(c[0]) for c in list(live_payload.get("candles") or []) if c]

        def marker_index_from_ts(event_ts: str) -> int:
            marker_index = max(0, len(candle_times) - 1)
            ts_match = None
            if event_ts:
            ...
            if ts_match is not None and candle_times:
                marker_index = min(range(len(candle_times)), key=lambda i: abs(candle_times[i] - ts_match))
            return int(marker_index)

        entry_time = str(live_state.get("entry_time") or payload.get("saved_at") or "")
        entry_price = float(live_payload.get("entry_price") or 0.0)
        if entry_price > 0:
            markers.append({"kind": "entry", "label": "Entry", "index": marker_index_from_ts(entry_time), "price": entry_price, "time": entry_time})
        stop_price = float(live_payload.get("stop_price") or 0.0)
        if stop_price > 0 and candle_times:
            markers.append({"kind": "stop", "label": "Stop", "index": max(0, len(candle_times) - 1), "price": stop_price, "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
        add_rows = [r for r in matched if str(r.get("event") or "").upper() in {"ADD_UNIT", "PYRAMID_ADD", "UNIT_ADD", "MANUAL_ADD_UNIT"}]
        for idx_add, add_row in enumerate(add_rows, start=2):
            px = float(add_row.get("price") or add_row.get("avg_px") or add_row.get("last_price") or 0.0)
            if px <= 0:
                continue
            event_ts = str(add_row.get("ts") or "").strip()
            markers.append({"kind": f"add{idx_add}", "label": f"U{idx_add}", "index": marker_index_from_ts(event_ts), "price": px, "time": event_ts})
        last_px = float(live_state.get("last_px") or 0.0)
        if last_px > 0 and candle_times:
            markers.append({"kind": "now", "label": "Now", "index": max(0, len(candle_times) - 1), "price": last_px, "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
        return markers
    ...
    def show_open_position_context(self, index) -> None:
        try:
            ...
            if row_idx < 0 or row_idx >= len(self.table_model.rows):
                return
            row = dict(self.table_model.rows[row_idx] or {})
            engine = getattr(self, "engine", None)
            ...
            if context_file and Path(context_file).exists():
                try:
                    ...
                    payload = {}
            if not payload:
                payload = build_sync_popup_payload(engine, row, POSITION_JOURNAL_FILE)

            live_payload = dict(payload)
            timeframe = str(row.get("timeframe") or payload.get("timeframe") or (getattr(getattr(engine, "cfg", None), "timeframe", "") if engine else "") or "5m").strip()
            entry_period = int(row.get("entry_period") or payload.get("entry_period") or 20)
            desired_limit = 80 if entry_period >= 55 else 60

            live_state = self._get_live_position_state(inst_id, trade_id, row)

            current_candles = list(payload.get("candles") or [])
            live_payload.update({
                "inst_id": inst_id or payload.get("inst_id"),
                "side": str(live_state.get("side") or payload.get("side") or ""),
                "timeframe": timeframe,
                "entry_period": int(live_state.get("entry_period") or payload.get("entry_period") or 20),
                "exit_period": int(live_state.get("exit_period") or payload.get("exit_period") or 10),
                "system_name": str(live_state.get("system_name") or payload.get("system_name") or ""),
                "entry_price": float(live_state.get("avg_px") or payload.get("entry_price") or 0.0),
                "current_price": float(live_state.get("last_px") or payload.get("current_price") or 0.0),
                "stop_price": float(live_state.get("stop_price") or payload.get("stop_price") or 0.0),
                "initial_stop_price": float(live_state.get("initial_stop_price") or payload.get("initial_stop_price") or payload.get("stop_price") or 0.0),
                "next_pyramid_price": float(live_state.get("next_pyramid_price") or payload.get("next_pyramid_price") or 0.0),
                "atr": float(live_state.get("atr") or payload.get("atr") or payload.get("entry_atr") or 0.0),
                "entry_atr": float(live_state.get("atr") or payload.get("entry_atr") or payload.get("atr") or 0.0),
                "qty": float(live_state.get("qty") or payload.get("qty") or 0.0),
                "units": int(live_state.get("units") or payload.get("units") or 1),
                "trade_id": trade_id or payload.get("trade_id"),
                "risk_amount_usdt": float(live_state.get("risk_amount_usdt") or payload.get("risk_amount_usdt") or 0.0),
                "position_notional_usdt": float(live_state.get("position_notional_usdt") or payload.get("position_notional_usdt") or 0.0),
                "saved_at": str(live_state.get("entry_time") or payload.get("saved_at") or ""),
                "live_chart": True,
                "chart_mode": "live",
            })
            ...
            try:
                if engine is not None and inst_id:
                    ...
                    live_payload["candles"] = list(fetched or current_candles)
                else:
            ...
            try:
                live_payload["markers"] = self._build_position_markers(inst_id, trade_id, live_payload, live_state, payload)
            except Exception:
                pass

            dlg = EntryContextDialog(live_payload, context_file, self, live_state=live_state)
            dlg.exec()
        except Exception as exc:
    ...
    def _manual_add_preview_from_snapshot(self, inst_id: str, extra_units: int = 1) -> dict:
        ...
        if engine is not None:
            manual_actions = getattr(engine, "manual_actions", None)
            if manual_actions is not None and hasattr(manual_actions, "preview_manual_add_units"):
                preview = dict(manual_actions.preview_manual_add_units(inst_id, extra_units=extra_units) or {})
                if preview:
                    return preview
            state = getattr(engine, "position_state", {}).get(inst_id) if hasattr(engine, "position_state") else None
            if state is not None:
                ...
                projected_avg_px = ((avg_px * current_qty) + (last_px * add_qty)) / projected_qty if projected_qty > 0 and last_px > 0 and current_qty > 0 else (last_px or avg_px)
                upl = float(getattr(state, "unrealized_pnl", 0.0) or 0.0)
                margin = float(getattr(state, "margin", 0.0) or 0.0)
                pnl_pct = (upl / margin * 100.0) if margin > 0 else float(getattr(state, "pnl_pct", 0.0) or 0.0)
                return {
                    "inst_id": inst_id,
                    ...
                    "current_notional_usdt": float(getattr(state, "position_notional_usdt", current_qty * last_px) or 0.0),
                    "projected_notional_usdt": float(getattr(state, "position_notional_usdt", current_qty * last_px) or 0.0) + (add_qty * last_px),
                    "active_stop_mode": str(getattr(state, "active_stop_mode", "") or getattr(state, "desired_stop_mode", "") or ""),
                }
        ...
        if not state:
            for row in list(snapshot.get("open_positions") or []):
                if str((row or {}).get("inst_id") or "").strip() == inst_id:
                    state = dict(row or {})
                    break
        if not state:
            selected_row = self._selected_open_position_row() or {}
            if str(selected_row.get("inst_id") or "").strip() == inst_id:
                state = dict(selected_row)
        if not state:
            raise KeyError(f"Живая позиция {inst_id} не найдена")
        extra_units = max(1, int(extra_units or 1))
    ...
    def _setup_scanner_review_tables(self) -> None:
        mapping = {
            ...
            "SPIKE_DEAD": getattr(self, "scanner_ripping_table", None),
            "PASS": getattr(self, "scanner_good_table", None),
        }
    ...
    def _scanner_filter_label(self, filter_type: str) -> str:
        return {"DEAD": "Мёртвый рынок", "SAW": "Пила", "SPIKE_DEAD": "SPIKE_DEAD", "PASS": "PASS"}.get(str(filter_type or '').upper(), str(filter_type or ''))
    ...
    def _build_good_trade_review_rows(self) -> List[dict]:
        ...
        if scanner is not None:
            try:
                persisted_rows = list(scanner.review_rows("PASS"))
            except Exception:
                persisted_rows = []
    ...
    def _update_scanner_review_views(self) -> None:
        ...
        if scanner is None:
            return
        for filter_type in ("DEAD", "SAW", "SPIKE_DEAD", "PASS"):
            table = self._scanner_table_for_filter(filter_type)
            if table is None:
                continue
            rows = list(scanner.review_rows(filter_type))
            ...
            for row_idx, row in enumerate(rows):
                ...
                for col_idx, value in enumerate(values):
                    item = QTableWidgetItem(str(value))
                    if filter_type == "PASS" or col_idx != 7:
    ...
    def _write_telegram_status_file(self) -> None:
        try:
            ...
            scanner_payload = {
                ...
                "spike_dead": int(analytics.get("scanner_spike_dead", 0) or 0),
    ...
    def _apply_post_stop_idle_cleanup(self) -> None:
        self.latest_snapshot = None
        self._snapshot_countdown_sec = max(5, int(getattr(self, "_snapshot_refresh_interval_sec", 2) or 2))
        self._last_heavy_snapshot_at = 0.0
        for name in ("scanner_dead_table", "scanner_saw_table", "scanner_spike_table", "scanner_good_table"):
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        self._safe_set_label_text("lbl_scanner_allowed", 'PASS: 0')
        ...
        self._safe_set_label_text("lbl_scanner_ripping", 'SPIKE_DEAD: 0')
    ...
    def clear_ban_lists(self) -> None:
        self.append_log("Clear ban-list удалён из v117: функция больше не используется")
    ...
    def _set_animation_timer_interval(self, widget, interval_ms: int) -> None:
        ...
        if isinstance(timer, QTimer):
            ...
            if interval <= 0:
                ...
                timer.setInterval(max(20, interval))
    ...
    def _on_worker_finished(self) -> None:
        unexpected_worker_exit = bool(self._bot_running) and not bool(self._engine_transitioning and self._engine_transition_action == "stopping") and not bool(self._close_after_stop_requested)
        if unexpected_worker_exit:
            _append_watchdog_event("engine_worker_finished_unexpected", app_version=APP_VERSION, bot_running=True)
            log_heartbeat("gui", "engine_worker_finished_unexpected", version=APP_VERSION)
            try:
                self.append_log("ВНИМАНИЕ: worker завершился неожиданно")
            except Exception:
                pass
        self.engine = None
        self.worker = None
        ...
        self._stop_telegram_screenshot_timer()
        self._apply_runtime_ui_profile()
        self._apply_post_stop_idle_cleanup()
        self._refresh_error_indicator()
        self.refresh_blocked_instruments_view()
        self._safe_set_runtime_label("Время работы: —")
    ...
    def _on_worker_engine_ready(self, engine: TurtleEngine) -> None:
        try:
            ...
            self._snapshot_refresh_interval_sec = max(2, int(getattr(cfg, "snapshot_interval_sec", 4) or 4))
            self._apply_trade_mode_to_controls(getattr(cfg, "trade_mode", "auto"))
            self._snapshot_countdown_sec = 0
    ...
    def _update_runtime_label(self) -> None:
        running = bool(self._bot_running) or bool(self.engine and getattr(self.engine, "running", False)) or bool(self.worker and self.worker.isRunning())
        if self._bot_started_ts and running:
            elapsed = max(0, int(time.time() - self._bot_started_ts))
            hours, rem = divmod(elapsed, 3600)
            minutes, seconds = divmod(rem, 60)
            runtime_text = f"Время работы: {hours:02d}:{minutes:02d}:{seconds:02d}"
            chip_text = f"UPTIME: {hours:02d}:{minutes:02d}:{seconds:02d}"
        else:
            runtime_text = "Время работы: —"
            chip_text = "UPTIME: —"
        self._safe_set_runtime_label(runtime_text)
        self._safe_set_label_text("lbl_header_uptime_chip", chip_text)
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        if scanner is not None:
            try:
                ...
                logging.exception("Failed to update PASS review rows")
        ...
        self._safe_set_label_text("lbl_scanner_allowed", f"PASS: {int(analytics.get('scanner_allowed', analytics.get('scanner_admitted', 0)) or 0)}")
        ...
        self._safe_set_label_text("lbl_scanner_ripping", f"SPIKE_DEAD: {int(analytics.get('scanner_spike_dead', 0) or 0)}")
