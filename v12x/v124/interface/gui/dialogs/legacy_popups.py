...
class ScannerReviewDialog(QDialog):
    REVIEW_TAGS = ["хороший", "пила", "мертвый", "всплеск", "плохой"]
    ...
    def __init__(self, payload: dict, parent=None):
        ...
        filter_label = {'DEAD': 'Мёртвый рынок', 'SAW': 'Пила', 'SPIKE_DEAD': 'SPIKE_DEAD', 'PASS': 'PASS'}.get(str(self.payload.get('scanner_filter_type') or '').upper(), str(self.payload.get('scanner_filter_type', '—')))
    ...
    def _normalize_tags(self, value) -> list[str]:
        if isinstance(value, str):
            ...
            parts = []
        legacy_map = {'подтверждено': 'хороший', 'не подтверждено': 'плохой', 'рваный': 'всплеск'}
        result = []
        for part in parts:
