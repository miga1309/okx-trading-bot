...
class StopEngine:
    ...
    def _sync_absent_position_once(self, state: PositionState, current_price: float = 0.0, reason: str = "position_absent_on_exchange") -> bool:
        ...
        if self._stop_management_disabled(state):
            return False
        entry_recovery_until = float(getattr(state, "entry_recovery_until_ts", 0.0) or 0.0)
        if entry_recovery_until > 0.0 and time.time() < entry_recovery_until:
            self._log_stop_engine("STOP_POSITION_ABSENCE_DELAYED", state, reason=reason, current_price=current_price)
            return False
        attempts = max(4, int(getattr(self.cfg, "position_absence_confirm_attempts", 4) or 4) + 1)
        delay_sec = max(0.30, float(getattr(self.cfg, "position_absence_confirm_delay_sec", 0.45) or 0.45))
        observed_absent = 0
    ...
    def _stop_retry_backoff_sec(self, state: PositionState, *, failures: int = 0) -> float:
        failure_count = max(int(failures or 0), int(getattr(state, "stop_recovery_failures", 0) or 0), int(getattr(state, "stop_health_missing_count", 0) or 0))
        if failure_count <= 0:
            return 0.0
        base = max(5.0, float(getattr(self.cfg, "exchange_stop_action_cooldown_sec", 2.5) or 2.5) * 3.0)
        return min(300.0, base * max(1.0, float(failure_count)))

    def _stop_retry_backoff_active(self, state: PositionState, attr_name: str, *, failures: int = 0) -> bool:
        last_ts = float(getattr(state, attr_name, 0.0) or 0.0)
        if last_ts <= 0.0:
            return False
        return (time.time() - last_ts) < self._stop_retry_backoff_sec(state, failures=failures)
    ...
    def _run_stop_health_check(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        if float(getattr(state, "stop_last_action_ts", 0.0) or 0.0) and now_ts - float(getattr(state, "stop_last_action_ts", 0.0) or 0.0) < max(4.0, float(getattr(self.cfg, "exchange_stop_action_cooldown_sec", 2.5) or 2.5) + 1.5):
            state.stop_state = "PENDING_RECHECK"
            self._mark_position_health(state, "STOP_UNVERIFIED", "exchange_stop_health_cooldown")
            return False
        if self._stop_retry_backoff_active(state, "stop_health_restore_last_failure_ts", failures=missing_count):
            state.stop_state = "PENDING_RECHECK"
            self._mark_position_health(state, "STOP_UNVERIFIED", "exchange_stop_health_backoff")
            return False
        ...
        if restored and self._confirm_exchange_stop_present(state, target_stop, attempts=6, delay_sec=0.25):
            state.stop_health_restore_last_failure_ts = 0.0
            state.stop_recovery_failures = 0
            state.stop_health_missing_count = 0
            ...
            self._log_stop_engine("STOP_HEALTH_RESTORED", state, current_price=current_price)
            return True
        state.stop_health_restore_last_failure_ts = time.time()
    ...
    def _stop_reconcile_required(self, state: PositionState, current_price: float = 0.0, candles: Optional[List[List[float]]] = None) -> bool:
        if self._stop_retry_backoff_active(state, "stop_recovery_last_failure_ts", failures=int(getattr(state, "stop_recovery_failures", 0) or 0)):
            return False
    ...
    def _run_stop_recovery(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "runtime_reconcile") -> bool:
        ...
        if self._exchange_stop_row_is_valid(state, existing_row, target_stop=target_stop, require_full_cover=True):
            ...
            if ok and self._confirm_exchange_stop_present(state, target_stop, attempts=6, delay_sec=0.25):
                ...
                state.active_stop_mode = target_mode
        if ok:
            state.stop_recovery_last_failure_ts = 0.0
        ...
        state.stop_recovery_last_failure_ts = time.time()
