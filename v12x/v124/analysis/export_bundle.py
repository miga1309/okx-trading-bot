...
from domain.reconcile.position_reconciler import get_dynamic_sync_interval as reconciler_dynamic_sync_interval, derive_sync_status
...
from domain.health.exchange_health import summarize_connectivity_rows
...
def _build_market_quality_rows(filter_rows: List[dict]) -> List[dict]:
    ...
    for row in filter_rows:
        ...
        scanner_filter_type = str(row.get('scanner_filter_type') or '').upper()
        looks_like_market_quality = (
            'sparse candles' in reason
            or 'liquidity' in reason
            or 'спред' in reason
            or 'стакан' in reason
            or 'мираж ликвидности' in reason
            or scanner_filter_type in {'DEAD', 'SPIKE_DEAD', 'SAW'}
            or 'scanner_dead' in reason.lower()
            or 'scanner_spike_dead' in reason.lower() or 'scanner_saw' in reason.lower()
        )
        if not looks_like_market_quality:
            continue
        rows.append({
            'timestamp': row.get('ts', ''),
            'inst_id': row.get('inst_id', ''),
            'reason': reason,
            'stage': row.get('stage', ''),
            'scanner_filter_type': row.get('scanner_filter_type', ''),
            'scanner_primary_reason': row.get('scanner_primary_reason', ''),
            'scanner_status_age_sec': _safe_float(row.get('scanner_status_age_sec', 0.0)),
            'scanner_forced_refresh': bool(row.get('scanner_forced_refresh', False)),
            'scanner_block_remaining_sec': _safe_float(row.get('scanner_block_remaining_sec', 0.0)),
...
def _build_blocked_entries_rows(entry_candidate_rows: List[dict]) -> List[dict]:
    ...
    for row in entry_candidate_rows:
        ...
        rows.append({
            ...
            'scanner_forced_refresh': bool(row.get('scanner_forced_refresh', False)),
            'scanner_block_remaining_sec': _safe_float(row.get('scanner_block_remaining_sec', 0.0)),
            'market_data_connectivity_state': row.get('market_data_connectivity_state', ''),
            'price': _safe_float(row.get('price', 0.0)),
            'atr': _safe_float(row.get('atr', 0.0)),
        })
    return rows
