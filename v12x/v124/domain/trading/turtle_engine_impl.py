...
from domain.runtime.position_orchestrator import PositionOrchestrator
from domain.runtime.runtime_scheduler import RuntimeScheduler
...
class TurtleEngine(QObject):
    ...
    def __init__(self, cfg: BotConfig):
        ...
        try:
            self.ws_manager.register_event_callback(self.position_orchestrator.enqueue_ws_event)
        except Exception:
            logging.exception('WS event callback registration failed')
    ...
    def _ws_first_private_ready(self) -> bool:
        ws_manager = getattr(self, "ws_manager", None)
        try:
            return bool(ws_manager is not None and ws_manager.private_ready())
        except Exception:
            return False

    def _consume_ws_runtime_events(self) -> int:
        orchestrator = getattr(self, "position_orchestrator", None)
        if orchestrator is None:
            return 0
        try:
            consumed = int(orchestrator.consume_pending_events())
        except Exception:
            logging.exception("WS runtime event consume failed")
            return 0
        if consumed:
            self.runtime_scheduler.refresh()
        return consumed
    ...
    def _manage_single_position(self, state: PositionState) -> None:
        ...
        try:
            setattr(state, "_cycle_candles", list(candles or []))
            ...
            try:
                setattr(state, "_cycle_candles", None)
            except Exception:
                pass
    ...
    def run_loop(self) -> None:
        ...
        while self.running and not self._stop_requested:
            cycle_started_at = time.time()
            try:
                self._consume_ws_runtime_events()
                self.runtime_scheduler.refresh()
                ...
                now_ts = time.time()
                scanner_pause = bool(getattr(self, 'position_cycle_active', False) or getattr(self, 'position_cycle_scanner_paused', False) or self.runtime_scheduler.scanner_pause_required())
                if not scanner_pause and (now_ts - float(getattr(self, 'last_entry_scan_at', 0.0) or 0.0)) >= max(1.0, float(getattr(self.cfg, 'scan_interval_sec', 15) or 15)):
                    self.scan_markets()
    ...
    def sync_positions_from_exchange(self) -> None:
        ...
        exchange_positions = []
        ws_positions = None
        if self._ws_first_private_ready():
            try:
                ws_positions = self.ws_manager.get_positions_snapshot(max_age_sec=max(8.0, float(getattr(self.cfg, 'positions_snapshot_ttl_sec', 4.0) or 4.0) * 2.0))
            except Exception:
                ws_positions = None
        if ws_positions:
            exchange_positions = list(ws_positions or [])
            try:
                self.stats_logger.log("positions_sync_source", source="ws", count=len(exchange_positions))
            except Exception:
                pass
        else:
            exchange_positions = self.gateway.get_positions(force=not self._ws_first_private_ready())
            try:
                self.stats_logger.log("positions_sync_source", source="rest", count=len(exchange_positions))
            except Exception:
                pass
        seen = set()
        changed = False
        for pos in exchange_positions:
            ...
            if current:
                ...
                self._start_entry_recovery_window(self.position_state[inst_id])
        ...
        for inst_id in list(self.position_state):
            if inst_id not in seen:
                ...
                if prev_state is None:
                    continue
                recent_ws_position = self.position_orchestrator.recent_position_event(prev_state.inst_id, max_age_sec=max(10.0, float(getattr(self.cfg, 'positions_snapshot_ttl_sec', 4.0) or 4.0) * 3.0))
                if self._ws_first_private_ready() and recent_ws_position:
                    changed = True
                    continue
                ...
                base_finalize_threshold = max(8, int(getattr(self.cfg, "missing_exchange_finalize_count", 5) or 5) + 2)
                if self._ws_first_private_ready():
                    base_finalize_threshold = max(base_finalize_threshold, int(getattr(self.cfg, 'missing_exchange_finalize_count', 5) or 5) + 5)
                in_entry_recovery = self._entry_recovery_active(prev_state)
                stop_fault = stop_state in {"ERROR", "MISSING", "PENDING_RECHECK"} or health_state in {"STOP_UNVERIFIED", "ERROR"}
                recovery_age = max(0.0, time.time() - float(getattr(prev_state, "entry_recovery_started_ts", 0.0) or 0.0))
                stop_fault_threshold = base_finalize_threshold + 2
                should_finalize = absence_synced or close_pending or protective_pending or missing_count >= base_finalize_threshold
                if stop_fault and not in_entry_recovery and recovery_age >= max(12.0, float(getattr(self.cfg, "post_entry_recovery_sec", 20.0) or 20.0) * 0.6):
                    should_finalize = should_finalize or missing_count >= stop_fault_threshold
                if in_entry_recovery and not (absence_synced or close_pending or protective_pending):
    ...
    def scan_markets(self) -> None:
        ...
        if getattr(self.cfg, "scanner_enabled", True):
            ...
            processed = self.market_scanner.run_chunk(getattr(self.cfg, "scanner_chunk_size", 3))
            if processed > 0:
                self.last_scanner_refresh_ts = now_ts
                pending = int(getattr(self.market_scanner, "queue_size", lambda: 0)() or 0)
                self.log_line.emit(f"[SCANNER] refreshed {processed} instrument(s), pending={pending}")
        ...
        for inst_id in self.gateway.swap_ids:
            ...
            if getattr(self.cfg, "scanner_enabled", True):
                ...
                scanner_queue_pending = bool(getattr(self.market_scanner, "has_pending_queue", lambda: False)())
                if bool(getattr(self.cfg, "scanner_entry_force_refresh", True)) and (not scanner_queue_pending) and (scanner_status_age_sec > float(getattr(self.cfg, "scanner_status_ttl_sec", 3900) or 3900) or not self.market_scanner.is_ready(inst_id)):
    ...
    def enter_position(self, inst_id: str, side: str, price: float, atr: float, system_name: str, candidate: Optional[dict] = None) -> bool:
        ...
        self.position_orchestrator.mark_position_critical('pyramid_submitted', duration_sec=8.0)
        if resp.get("code") != "0":
        ...
        state = PositionState(
            ...
            state_tag="ENTRY_SUBMITTED",
        )
        self.position_state[inst_id] = state
        self.position_orchestrator.mark_position_critical('entry_submitted', duration_sec=max(6.0, float(getattr(self.cfg, 'initial_stop_verify_delay_sec', 8.0) or 8.0)))
        self._start_entry_recovery_window(state)
        state.popup_snapshot_dirty = True
    ...
    def try_pyramid(self, state: PositionState, last_close: float, candles: List[List[float]], candle_ts: int = 0) -> None:
        ...
        state.state_tag = 'PYRAMID_PENDING'
    ...
    def _entry_recovery_active(self, state: Optional[PositionState]) -> bool:
        if state is None:
            return False
        until_ts = float(getattr(state, "entry_recovery_until_ts", 0.0) or 0.0)
        return until_ts > 0.0 and time.time() < until_ts

    def _start_entry_recovery_window(self, state: Optional[PositionState]) -> None:
        if state is None:
            return
        grace_sec = max(5.0, float(getattr(self.cfg, "post_entry_recovery_sec", 20.0) or 20.0))
        now_ts = time.time()
        state.entry_recovery_started_ts = now_ts
        state.entry_recovery_until_ts = now_ts + grace_sec
        if not str(getattr(state, "state_tag", "") or "").strip() or str(getattr(state, "state_tag", "") or "").upper() == "ACTIVE":
            state.state_tag = "ENTRY_RECOVERING"

    def _finish_entry_recovery_window(self, state: Optional[PositionState]) -> None:
        if state is None:
            return
        if self._entry_recovery_active(state):
            return
        if float(getattr(state, "entry_recovery_until_ts", 0.0) or 0.0) > 0.0:
            state.entry_recovery_until_ts = 0.0
            if str(getattr(state, "state_tag", "") or "").upper() == "ENTRY_RECOVERING":
                state.state_tag = "ACTIVE"
    ...
    def _mark_position_missing_on_exchange(self, state: PositionState) -> tuple[int, str, float]:
        ...
        elif (stop_state in {"ERROR", "MISSING"} or health_state in {"STOP_UNVERIFIED", "ERROR"}) and not self._entry_recovery_active(state):
            reason = f"SYNC confirmed close after stop fault: {stop_state or health_state or 'unknown'}"
        elif self._entry_recovery_active(state):
            reason = f"SYNC recheck during entry recovery: {stop_state or health_state or 'awaiting_confirmation'}"
        self.position_journal_logger.log(
    ...
    def _finalize_closed_trade(self, state: PositionState, price: float, reason: str, candles: Optional[List[List[float]]] = None) -> None:
        ...
        self._finish_entry_recovery_window(state)
        info = self.gateway.instrument_info(state.inst_id)
        ct_val = float(info.get("ctVal") or 1.0)
    ...
    def close_position(self, state: PositionState, price: float, reason: str, candles: Optional[List[List[float]]] = None) -> None:
        ...
        state.last_close_error = ""
        self.position_orchestrator.mark_position_critical('close_requested', duration_sec=10.0)
        state.state_tag = 'CLOSING'
    ...
    def emit_snapshot(self) -> None:
        ...
        payload = {
            ...
            "analytics": {
                ...
                "scanner_spike_dead": int(trade_ready_metrics.get("scanner_spike_dead", 0) or 0),
