...
class RemoteControlController:
    ALLOWED_COMMANDS = {"status", "start_bot", "stop_bot", "reset_test", "analysis", "chart", "update_component", "update_full"}
    ...
    def _stop_bot(self) -> dict[str, Any]:
        ...
        if not stopped:
            ...
            try:
                ...
                worker_running = False
            if not worker_running and not bool(getattr(self.window, "_bot_running", False)):
                try:
                    setattr(self.window, "_engine_transitioning", False)
                    setattr(self.window, "_engine_transition_action", "")
                except Exception:
                    pass
                stopped = self._wait_until(lambda: (not self._bot_running()) and (not bool(getattr(self.window, "_engine_transitioning", False))), timeout_sec=2.0, step_sec=0.1)
    ...
    def _capture_chart(self) -> dict[str, Any]:
        refresh_fn = getattr(self.window, "_refresh_telegram_chart_cache", None)
        if not callable(refresh_fn):
            raise RuntimeError("chart handler not available")
        chart_path = Path(str(refresh_fn(force=True) or "")).expanduser()
        if chart_path.exists() and chart_path.is_file():
            return {"status": "ok", "message": f"chart ready: {chart_path.name}", "file_path": str(chart_path)}
        raise RuntimeError("chart not ready; окно должно быть открыто и не свернуто")
    ...
    def _apply_update(self, loaded, *, restart_app: bool) -> dict[str, Any]:
        ...
        cleaned_update_artifacts: list[str] = []
        restarted_engine = False
        restart_script = None
        if restart_app:
            cleaned_update_artifacts = self.updater.cleanup_loaded_artifacts(loaded)
        ...
        if cleaned_update_artifacts:
            message += f" | update archives cleaned={len(cleaned_update_artifacts)}"
        ...
        return {
            ...
            "cleaned_update_artifacts": cleaned_update_artifacts,
