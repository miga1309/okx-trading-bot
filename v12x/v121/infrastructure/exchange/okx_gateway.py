...
class OkxGateway:
    ...
    def __init__(self, cfg: BotConfig, hidden_checker: Optional[Callable[[object], bool]] = None):
        ...
        self._leverage_applied: Dict[tuple[str, str], float] = {}
    ...
    def ensure_instrument_leverage(self, inst_id: str) -> bool:
        inst = str(inst_id or "").strip()
        if not inst:
            return False
        try:
            target = float(self.cfg.leverage or 1)
        except Exception:
            target = 1.0
        target = max(1.0, target)
        cache_key = (inst, str(getattr(self.cfg, "td_mode", "isolated") or "isolated"))
        current = self._leverage_applied.get(cache_key)
        if current is not None and abs(float(current) - target) < 1e-9:
            return True
        setter = getattr(self.account_api, "set_leverage", None)
        if setter is None:
            logging.warning("OKX SDK does not expose set_leverage; runtime leverage cache only for %s", inst)
            self._leverage_applied[cache_key] = target
            return False
        payload = {
            "lever": str(int(target) if float(target).is_integer() else target),
            "mgnMode": str(getattr(self.cfg, "td_mode", "isolated") or "isolated"),
            "instId": inst,
        }
        try:
            resp = setter(**payload)
            ...
            logging.warning("Failed to set leverage for %s: %s", inst, exc)
            return False
        if str((resp or {}).get("code")) == "0":
            self._leverage_applied[cache_key] = target
            logging.info("Applied leverage %s to %s (%s)", payload["lever"], inst, payload["mgnMode"])
            return True
        logging.warning("Exchange rejected leverage setup for %s: %s", inst, resp)
        return False
    ...
    def place_market_order(self, inst_id: str, side: str, qty: float, reduce_only: bool = False, attached_stop: dict | None = None) -> dict:
        if not reduce_only:
            try:
                self.ensure_instrument_leverage(inst_id)
            except Exception as exc:
                logging.warning("Pre-order leverage ensure failed for %s: %s", inst_id, exc)
        info = self.instrument_info(inst_id)
        params = dict(
            instId=inst_id,
            tdMode=self.cfg.td_mode,
