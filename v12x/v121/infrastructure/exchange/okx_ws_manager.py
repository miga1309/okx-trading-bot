...
class OkxWsManager:
    def __init__(self, cfg: Any, log_callback: Optional[Callable[[str], None]] = None):
        ...
        self._event_callbacks: List[Callable[[str, List[dict], float], None]] = []
    ...
    def register_event_callback(self, callback: Callable[[str, List[dict], float], None]) -> None:
        if callback is None:
            return
        with self._lock:
    ...
    def _emit_event_callbacks(self, channel: str, rows: List[dict], recv_ts: float) -> None:
        callbacks = []
        with self._lock:
            callbacks = list(self._event_callbacks)
        for callback in callbacks:
            try:
                callback(str(channel or '').lower(), [dict(r or {}) for r in list(rows or [])], float(recv_ts or time.time()))
            except Exception:
                logging.exception('WS event callback failed: %s', channel)

    def get_orders_snapshot(self, inst_id: str = '', max_age_sec: float = 10.0) -> List[dict]:
        if not self.private_ready() or (time.time() - float(self._private_last_msg_ts or 0.0)) > max_age_sec:
            return []
        inst = str(inst_id or '').upper().strip()
        with self._lock:
            rows = [dict(v) for v in (self._orders or {}).values()]
        if inst:
            rows = [r for r in rows if str(r.get('instId') or '').upper() == inst]
        return rows
    ...
    def _handle_payload(self, payload: dict, private: bool) -> None:
        ...
        with self._lock:
            if channel == 'tickers':
                ...
                self._emit_event_callbacks('positions', self._positions, recv_ts)
                ...
                self._emit_event_callbacks('orders', data, recv_ts)
