    ...
    def sync_positions_from_exchange(self) -> None:
        ...
        for inst_id in list(self.position_state):
            if inst_id not in seen:
                ...
                base_finalize_threshold = max(6, int(getattr(self.cfg, "missing_exchange_finalize_count", 5) or 5) + 1)
                if self._ws_first_private_ready():
                    base_finalize_threshold = max(base_finalize_threshold, int(getattr(self.cfg, 'missing_exchange_finalize_count', 5) or 5) + 3)
