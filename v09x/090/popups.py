...
class ScannerReviewDialog(QDialog):
    ...
    def __init__(self, payload: dict, parent=None):
        ...
        filter_label = {'DEAD': 'Мёртвый рынок', 'SAW': 'Пила', 'RIPPING': 'Рваный рынок'}.get(str(self.payload.get('scanner_filter_type') or '').upper(), str(self.payload.get('scanner_filter_type', '—')))
        ...
        self.comment_edit.setPlaceholderText("Короткий комментарий по фильтру")
    ...
    def _apply_verdict_style(self) -> None:
        ...
        if verdict == 'Подтверждено':
            style = 'QComboBox { background:#1d7f4e; color:#f8fbff; border:1px solid #2be28c; border-radius:8px; padding:4px 8px; }'
        elif verdict == 'Не подтверждено':
            style = 'QComboBox { background:#a52a36; color:#f8fbff; border:1px solid #ff7080; border-radius:8px; padding:4px 8px; }'
        else:
            style = 'QComboBox { background:#0b1525; color:#e6ecff; border:1px solid #2b4b66; border-radius:8px; padding:4px 8px; }'
