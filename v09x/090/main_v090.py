...
class MainWindow(QMainWindow):
    ...
    def _scanner_filter_label(self, filter_type: str) -> str:
        return {"DEAD": "Мёртвый рынок", "SAW": "Пила", "RIPPING": "Рваный рынок"}.get(str(filter_type or '').upper(), str(filter_type or ''))
    ...
    def _apply_scanner_verdict_item_style(self, item: Optional[QTableWidgetItem], verdict: str) -> None:
        ...
        if verdict == 'Подтверждено':
            item.setBackground(QBrush(QColor('#1d7f4e')))
        elif verdict == 'Не подтверждено':
            item.setBackground(QBrush(QColor('#a52a36')))
