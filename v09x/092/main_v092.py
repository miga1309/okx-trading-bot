...
class TurtleEngine(QObject):
    ...
    def enter_position(self, inst_id: str, side: str, price: float, atr: float, system_name: str, candidate: Optional[dict] = None) -> bool:
        ...
        state = PositionState(
            ...
            initial_stop_verify_due_ts=time.time() + 5.0,
        ...
        if stop_ok:
            ...
            state.initial_stop_verify_due_ts = state.initial_stop_set_ts + 5.0
        ...
        for attempt in range(1, attempts + 1):
            ...
            if str(resp.get("code", "")) == "0":
                ...
                if float(getattr(state, "initial_stop_verify_due_ts", 0.0) or 0.0) <= 0.0:
                    state.initial_stop_verify_due_ts = float(getattr(state, "initial_stop_set_ts", time.time()) or time.time()) + 5.0
    ...
    def _verify_initial_stop_if_needed(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        confirmed = self._confirm_exchange_stop_present(state, target_stop, attempts=3, delay_sec=0.2)
        if confirmed:
    ...
    def _run_stop_health_check(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        restored = self._place_exchange_stop(state, float(getattr(state, "stop_price", 0.0) or 0.0), reason="health_restore")
        if restored:
    ...
    def _apply_stop_policy(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "manage") -> None:
        ...
        if abs(target_stop - prev_stop) >= threshold:
            ...
            if moved and desired_mode == "ATR" and not getattr(state, "trailing_activated_at", ""):
    ...
    def update_and_maybe_exit_or_pyramid(self, state: PositionState) -> None:
        ...
        strategy_candle_due = self._is_stop_strategy_candle_due(state, candles)
        if strategy_candle_due:
