...
class ScannerReviewDialog(QDialog):
    VERDICTS = ["Не проверено", "Подтверждено", "Не подтверждено"]
    ...
    def __init__(self, payload: dict, parent=None):
        ...
        verdict_lbl.setStyleSheet("font-weight:600;")
        verdict_row.addWidget(verdict_lbl)
        self.verdict_combo = QComboBox(self)
        self.verdict_combo.addItems(self.VERDICTS)
        current_verdict = str(self.payload.get('scanner_user_verdict') or 'Не проверено').strip() or 'Не проверено'
        idx = self.verdict_combo.findText(current_verdict)
        self.verdict_combo.setCurrentIndex(idx if idx >= 0 else 0)
        self.verdict_combo.setMinimumWidth(220)
        self.verdict_combo.currentTextChanged.connect(self._apply_verdict_style)
        verdict_row.addWidget(self.verdict_combo)

        comment_lbl = QLabel("Комментарий:")
        comment_lbl.setStyleSheet("font-weight:600;")
        verdict_row.addWidget(comment_lbl)
        ...
        verdict_row.addWidget(self.comment_edit, 1)
        ...
        self._apply_combo_item_colors()
        self._apply_verdict_style()


    def _apply_combo_item_colors(self) -> None:
        colors = {
            "Не проверено": (QColor("#64748b"), QColor("#e6ecff")),
            "Подтверждено": (QColor("#1d7f4e"), QColor("#f8fbff")),
            "Не подтверждено": (QColor("#a52a36"), QColor("#f8fbff")),
        }
        for idx, text in enumerate(self.VERDICTS):
            bg, fg = colors.get(text, (QColor("#0b1525"), QColor("#e6ecff")))
            self.verdict_combo.setItemData(idx, bg, Qt.ItemDataRole.BackgroundRole)
            self.verdict_combo.setItemData(idx, fg, Qt.ItemDataRole.ForegroundRole)

    def _combo_view_style(self) -> str:
        return (
            'QComboBox QAbstractItemView { background:#0b1525; color:#e6ecff; selection-background-color:#16365d; '
            'selection-color:#f8fbff; border:1px solid #2b4b66; padding:4px; }'
        )

    def _apply_verdict_style(self) -> None:
        verdict = str(self.verdict_combo.currentText() or 'Не проверено').strip() or 'Не проверено'
        if verdict == 'Подтверждено':
            style = 'QComboBox { background:#1d7f4e; color:#f8fbff; border:1px solid #2be28c; border-radius:8px; padding:4px 8px; } ' + self._combo_view_style()
        elif verdict == 'Не подтверждено':
            style = 'QComboBox { background:#a52a36; color:#f8fbff; border:1px solid #ff7080; border-radius:8px; padding:4px 8px; } ' + self._combo_view_style()
        else:
            style = 'QComboBox { background:#64748b; color:#f8fbff; border:1px solid #94a3b8; border-radius:8px; padding:4px 8px; } ' + self._combo_view_style()
        self.verdict_combo.setStyleSheet(style)
    ...
    def selected_verdict(self) -> str:
        verdict = str(self.verdict_combo.currentText() or 'Не проверено').strip() or 'Не проверено'
        return verdict if verdict in self.VERDICTS else 'Не проверено'
