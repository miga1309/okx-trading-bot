...
# Version: v093_2
# Date: 2026-03-20
# Changes: hard-blocked stablecoin-base swap instruments; fixed exchange-stop synchronization and desync tracking; strengthened initial stop verification and stop replace fallback; replaced scanner popup verdict with multi-tag review buttons for manual labeling.
...
class TurtleEngine(QObject):
    ...
    def _apply_stop_policy(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "manage") -> None:
        ...
        if active_mode != desired_mode:
            ...
            if moved or (prev_exchange_stop <= 0.0 and self._confirm_exchange_stop_present(state, target_stop, attempts=4, delay_sec=0.25)):
...
class MainWindow(QMainWindow):
    ...
    def _apply_scanner_verdict_item_style(self, item: Optional[QTableWidgetItem], verdict: str) -> None:
        ...
        elif any(tag in verdict for tag in ('плохой', 'мертвый', 'рваный', 'пила')):
            item.setBackground(QBrush(QColor('#7c2d12' if 'пила' in verdict else '#a52a36' if 'плохой' in verdict else '#394867' if 'мертвый' in verdict else '#7b3f00')))
