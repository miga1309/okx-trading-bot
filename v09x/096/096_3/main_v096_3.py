...
# Version: v096_3
# Date: 2026-03-22
# Changes: added top-screen error counter sourced from traceback events; tracebacks are saved to a dedicated file and included in deep analysis exports; reset now clears the runtime error counter and file.
...
class RuntimeErrorTracker:
    ...
    def record(self, *, source: str, title: str, message: str = "", traceback_text: str = "", extra: Optional[dict] = None) -> int:
        payload = {
...
def record_runtime_error(source: str, title: str, *, exc_info=None, message: str = "", traceback_text: str = "", extra: Optional[dict] = None) -> int:
    ...
    if not msg and exc_info and len(exc_info) >= 2 and exc_info[1] is not None:
        msg = str(exc_info[1])
    return RUNTIME_ERROR_TRACKER.record(source=source, title=title, message=msg, traceback_text=tb_text, extra=extra)
    ...
    def emit(self, record):
        try:
            ...
            if record.exc_info:
                ...
                traceback_text = record.getMessage()
