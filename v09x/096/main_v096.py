...
# Version: v096
# Date: 2026-03-22
# Changes: reworked stop architecture for sequential candle-close stop cycles with inter-request delays; initial stop is attached on entry with fallback placement; reduced aggressive intra-candle stop reconcile/health churn and added stop-cycle telemetry.
...
class TurtleEngine(QObject):
    ...
    def __init__(self, cfg: BotConfig):
        ...
        self.exchange_health_check_interval_sec: float = 5.0
    ...
    def enter_position(self, inst_id: str, side: str, price: float, atr: float, system_name: str, candidate: Optional[dict] = None) -> bool:
        ...
        resp = self.gateway.place_market_order(inst_id, order_side, qty, attach_stop_trigger_px=stop_price, attach_stop_trigger_type="mark")
        ...
        attach_attempts = max(2, min(4, int(getattr(self.cfg, "initial_stop_verify_attempts", 10) or 10)))
        attach_delay = max(0.2, min(0.5, float(getattr(self.cfg, "initial_stop_verify_delay_between_attempts_sec", 0.4) or 0.4)))
        stop_ok = self._confirm_exchange_stop_present(state, stop_price, attempts=attach_attempts, delay_sec=attach_delay)
        if stop_ok:
            state.exchange_stop_last_update_method = "attach"
            ...
            self._log_stop_engine("STOP_INIT_ATTACHED", state, requested_stop=stop_price, reason="initial_turtle_stop_attached")
        else:
            stop_ok = self._place_exchange_stop(state, stop_price, reason="initial_turtle_stop_fallback")
            if stop_ok:
                state.initial_stop_set_ts = time.time()
                state.initial_stop_verify_due_ts = state.initial_stop_set_ts + float(getattr(self.cfg, "initial_stop_verify_delay_sec", 12.0) or 12.0)
                state.initial_stop_verified = False
                self._log_stop_engine("STOP_INIT_PLACED", state, requested_stop=stop_price, reason="initial_turtle_stop_fallback")
    ...
    def manage_open_positions(self) -> None:
        ...
        for inst_id, state in list(self.position_state.items()):
            ...
            if retry_after and retry_after > time.time():
                continue
            cycle_before = float(getattr(state, "stop_cycle_last_run_at", 0.0) or 0.0)
            try:
            ...
            cycle_after = float(getattr(state, "stop_cycle_last_run_at", 0.0) or 0.0)
            if cycle_after > cycle_before:
                self._stop_cycle_pause(after_position=True)
    ...
    def _stop_cycle_request_delay(self) -> float:
        return max(0.0, float(getattr(self.cfg, "stop_cycle_request_delay_sec", 0.35) or 0.35))

    def _stop_cycle_position_delay(self) -> float:
        return max(0.0, float(getattr(self.cfg, "stop_cycle_position_delay_sec", 0.60) or 0.60))

    def _stop_cycle_pause(self, after_position: bool = False) -> None:
        delay = self._stop_cycle_position_delay() if after_position else self._stop_cycle_request_delay()
        if delay > 0:
            time.sleep(delay)

    def _run_stop_cycle_for_position(self, state: PositionState, current_price: float, strategy_price: float, candles: List[List[float]], latest_closed_ts: int, reason: str = "closed_candle") -> bool:
        if latest_closed_ts <= 0:
            return False
        if int(getattr(state, "stop_cycle_last_candle_ts", 0) or 0) >= latest_closed_ts:
            return False
        state.stop_cycle_last_candle_ts = latest_closed_ts
        state.stop_cycle_last_run_at = time.time()
        state.stop_cycle_last_action = "started"
        self._log_stop_engine("STOP_CYCLE_START", state, candle_ts=latest_closed_ts, current_price=current_price, strategy_price=strategy_price, reason=reason)
        target_stop = float(getattr(state, "stop_price", 0.0) or 0.0)
        existing_row = self._find_existing_exchange_stop_row(state, target_stop=target_stop)
        if existing_row is not None:
            status = "active" if self._exchange_stop_row_is_valid(state, existing_row, target_stop=target_stop, require_full_cover=True) else "cycle_snapshot_invalid"
            self._attach_existing_exchange_stop(state, existing_row, status=status)
        elif float(getattr(state, "qty", 0.0) or 0.0) > 0.0:
            self._mark_exchange_stop_desync(state, "stop_cycle_missing_snapshot")
            self._log_stop_engine("STOP_CYCLE_MISSING", state, candle_ts=latest_closed_ts, current_price=current_price, requested_stop=target_stop, reason=reason)
        self._stop_cycle_pause()
        prev_exchange_stop = float(getattr(state, "exchange_stop_price", 0.0) or 0.0)
        prev_mode = str(getattr(state, "active_stop_mode", "ATR") or "ATR")
        self._apply_stop_policy(state, strategy_price, candles, reason=f"stop_cycle_{reason}")
        state.stop_strategy_last_candle_ts = latest_closed_ts
        state.stop_cycle_last_run_at = time.time()
        new_exchange_stop = float(getattr(state, "exchange_stop_price", 0.0) or 0.0)
        new_mode = str(getattr(state, "active_stop_mode", prev_mode) or prev_mode)
        if abs(new_exchange_stop - prev_exchange_stop) > 1e-12 or new_mode != prev_mode:
            state.stop_cycle_last_action = "updated"
            self._log_stop_engine("STOP_CYCLE_UPDATED", state, candle_ts=latest_closed_ts, previous_exchange_stop=prev_exchange_stop, new_exchange_stop=new_exchange_stop, previous_mode=prev_mode, new_mode=new_mode, reason=reason)
        else:
            state.stop_cycle_last_action = "checked_no_change"
            self._log_stop_engine("STOP_CYCLE_NO_CHANGE", state, candle_ts=latest_closed_ts, exchange_stop_price=new_exchange_stop, mode=new_mode, reason=reason)
        return True
    ...
    def _verify_initial_stop_if_needed(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        if due_ts <= 0.0 or time.time() < due_ts:
            return False
        target_stop = float(getattr(state, "initial_stop_price", 0.0) or getattr(state, "stop_price", 0.0) or 0.0)
        ...
        if self._force_replace_exchange_stop(state, target_stop, current_price=current_price, reason="initial_stop_verify_recover"):
        ...
        if qty <= 0.0:
            return False
        self.position_journal_logger.log("STOP_INIT_FORCE_CLOSE", trade_id=getattr(state, "trade_id", ""), inst_id=state.inst_id, side=state.side, price=current_price, stop_price=target_stop, qty=state.qty, units=state.units, note="initial_stop_not_found")
        self.close_position(state, current_price or float(getattr(state, "last_px", 0.0) or getattr(state, "avg_px", 0.0) or 0.0), "Ошибка: первичный стоп не подтверждён на бирже")
        return True
    ...
    def update_and_maybe_exit_or_pyramid(self, state: PositionState) -> None:
        ...
        if stop_strategy_candle_due:
            ...
            self._run_stop_cycle_for_position(state, current_price=current_price, strategy_price=strategy_price, candles=candles, latest_closed_ts=latest_closed_ts, reason="closed_candle")
    ...
    def _build_good_trade_review_rows(self) -> List[dict]:
        ...
        def _push(item: dict, *, active: bool) -> None:
            ...
            rows.append({
                ...
                "hit_count": 1,
                "reason": note,
                "user_verdict": trade_id or status,
