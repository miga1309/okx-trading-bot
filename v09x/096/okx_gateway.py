...
class OkxGateway:
    ...
    def place_market_order(self, inst_id: str, side: str, qty: float, reduce_only: bool = False, attach_stop_trigger_px: float = 0.0, attach_stop_trigger_type: str = "mark") -> dict:
        info = self.instrument_info(inst_id)
        params = dict(
            instId=inst_id,
            tdMode=self.cfg.td_mode,
        ...
        elif float(attach_stop_trigger_px or 0.0) > 0.0:
            trigger_text = (f"{float(attach_stop_trigger_px):.12f}").rstrip("0").rstrip(".")
            params["attachAlgoOrds"] = [{
                "slTriggerPx": trigger_text,
                "slOrdPx": "-1",
                "slTriggerPxType": str(attach_stop_trigger_type or "mark"),
            }]
        logging.info("Placing order %s %s qty=%s attach_stop=%s", inst_id, side, qty, attach_stop_trigger_px if not reduce_only else 0.0)
