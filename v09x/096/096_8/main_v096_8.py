...
# Version: v096_8
# Date: 2026-03-22
# Changes: hardened Telegram worker runtime paths and GUI launch flow, moved queue/log storage into runtime folder, preserved prior export fixes.
...
class TurtleEngine(QObject):
    ...
    def __init__(self, cfg: BotConfig):
        ...
        self.telegram_last_status_ts: float = 0.0
        self.telegram_last_risk_alert_ts: float = 0.0
        self.telegram_risk_alert_cooldown_sec: float = 30.0 * 60.0
        self.telegram_status_interval_sec: float = 60.0 * 60.0
        self.telegram_big_move_alerts: set[str] = set()
        self.telegram_stop_alert_ts: Dict[str, float] = {}
    ...
    def _update_exchange_connectivity_state(self, detected_state: str, components: dict, detail: dict) -> None:
        ...
        if target_state in {"DEGRADED", "DISCONNECTED"}:
            failed = ", ".join(down) if down else "unknown"
            self._notify(
                f"⚠️ CONNECTION LOST\n\n"
                f"State: {target_state}\n"
                f"Components down: {failed}\n"
                f"Error: {self.exchange_connectivity_last_error or '-'}"
            )
        elif prev_state in {"DEGRADED", "DISCONNECTED", "RECOVERING"} and target_state in {"RECOVERING", "CONNECTED"}:
            self._notify(
                f"✅ CONNECTION RESTORED\n\n"
                f"From: {prev_state}\n"
                f"To: {target_state}\n"
                f"Open positions: {len(self.position_state)}"
            )
    ...
    def _telegram_status_text(self) -> str:
        open_positions = len(self.position_state)
        total_eq = float(getattr(self, "current_balance_total", 0.0) or 0.0)
        avail_eq = float(getattr(self, "current_balance_available", 0.0) or 0.0)
        used_eq = max(total_eq - avail_eq, 0.0)
        scanner_total = int(getattr(self.market_scanner, "last_total_count", 0) or 0)
        scanner_allowed = int(getattr(self.market_scanner, "last_allowed_count", 0) or 0)
        scanner_blocked = int(getattr(self.market_scanner, "last_blocked_count", 0) or 0)
        return (
            f"📊 BOT STATUS\n\n"
            f"Version: {APP_VERSION}\n"
            f"State: {getattr(self, 'exchange_connectivity_state', 'IDLE')}\n"
            f"Open positions: {open_positions}\n"
            f"Equity: {total_eq:.2f} USDT\n"
            f"Available: {avail_eq:.2f} USDT\n"
            f"Used: {used_eq:.2f} USDT\n\n"
            f"Scanner total: {scanner_total}\n"
            f"Allowed: {scanner_allowed}\n"
            f"Blocked: {scanner_blocked}\n"
            f"Errors: {int(getattr(self, 'error_count', 0) or 0)}"
        )

    def _maybe_emit_telegram_status_and_alerts(self) -> None:
        if TELEGRAM_HARD_DISABLED or not getattr(self.telegram, "enabled", False):
            return
        now_ts = time.time()
        if (now_ts - float(getattr(self, "telegram_last_status_ts", 0.0) or 0.0)) >= float(getattr(self, "telegram_status_interval_sec", 3600.0) or 3600.0):
            self.telegram_last_status_ts = now_ts
            self._notify(self._telegram_status_text())
        total_eq = float(getattr(self, "current_balance_total", 0.0) or 0.0)
        avail_eq = float(getattr(self, "current_balance_available", 0.0) or 0.0)
        used_eq = max(total_eq - avail_eq, 0.0)
        used_ratio = (used_eq / total_eq) if total_eq > 0 else 0.0
        if used_ratio >= 0.70 and (now_ts - float(getattr(self, "telegram_last_risk_alert_ts", 0.0) or 0.0)) >= float(getattr(self, "telegram_risk_alert_cooldown_sec", 1800.0) or 1800.0):
            self.telegram_last_risk_alert_ts = now_ts
            self._notify(
                f"⚠️ HIGH RISK ALERT\n\n"
                f"Used margin ratio: {used_ratio * 100.0:.1f}%\n"
                f"Open positions: {len(self.position_state)}\n"
                f"Used: {used_eq:.2f} USDT\n"
                f"Equity: {total_eq:.2f} USDT"
            )
        for state in list(self.position_state.values()):
            try:
                pnl_pct = float(getattr(state, "unrealized_pnl_pct", 0.0) or 0.0)
            except Exception:
                pnl_pct = 0.0
            trade_id = str(getattr(state, "trade_id", "") or f"{state.inst_id}:{state.side}")
            if pnl_pct >= 1.0 and trade_id not in self.telegram_big_move_alerts:
                self.telegram_big_move_alerts.add(trade_id)
                self._notify(
                    f"🚀 BIG MOVE\n\n"
                    f"Instrument: {state.inst_id}\n"
                    f"Side: {state.side.upper()}\n"
                    f"PnL %: {pnl_pct:.2f}%\n"
                    f"Units: {int(getattr(state, 'units', 0) or 0)}"
                )
    ...
    def stop(self) -> None:
        self.request_stop()
        self.finalize_stop()
        self._notify(f"🛑 BOT STOPPED\n\nVersion: {APP_VERSION}\nOpen positions: {len(self.position_state)}\nClosed trades: {len(self.closed_trades)}")
    ...
    def run_loop(self) -> None:
        while self.running and not self._stop_requested:
            ...
            try:
                ...
                self.emit_snapshot()
                self._maybe_emit_telegram_status_and_alerts()
                ...
                self._notify(f"⚠️ BOT ERROR\n\nModule: strategy_cycle\nError: {msg}")
    ...
    def _log_stop_engine(self, event: str, state: Optional[PositionState] = None, **payload) -> None:
        ...
        critical_events = {"STOP_INIT_FAILED", "STOP_RECOVERY_FAILED", "STOP_RECOVERY_CRITICAL", "STOP_DESYNC", "STOP_ERROR"}
        if event in critical_events and state is not None:
            cooldown_key = f"{getattr(state, 'inst_id', '')}:{event}"
            now_ts = time.time()
            last_ts = float(self.telegram_stop_alert_ts.get(cooldown_key, 0.0) or 0.0)
            if (now_ts - last_ts) >= 300.0:
                self.telegram_stop_alert_ts[cooldown_key] = now_ts
                self._notify(
                    f"⚠️ STOP ENGINE ALERT\n\n"
                    f"Event: {event}\n"
                    f"Instrument: {getattr(state, 'inst_id', '')}\n"
                    f"Side: {str(getattr(state, 'side', '')).upper()}\n"
                    f"Stop mode: {getattr(state, 'stop_mode', '')}\n"
                    f"Reason: {payload.get('reason', '') or payload.get('error', '') or '-'}"
                )
    ...
    def try_pyramid(self, state: PositionState, last_close: float, candles: List[List[float]], candle_ts: int = 0) -> None:
        ...
        self._notify(
            ...
            f"Instrument: {state.inst_id}\n"
            f"Side: {state.side.upper()}\n"
            f"Unit: {state.units}/4\n"
            f"Add qty: {add_qty}\n"
            f"Price: {self._fmt_price(fill_price)}\n"
            f"Next add: {self._fmt_price(state.next_pyramid_price)}"
        )
        self._save_state()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._telegram_runtime_enabled = False
        self._telegram_last_cfg_enabled = False
    ...
    def _telegram_credentials_from_env(self) -> tuple[bool, str, str]:
        load_dotenv()
        enabled_raw = str(os.getenv("TELEGRAM_ENABLED", "0") or "0").strip().lower()
        enabled_flag = (enabled_raw in {"1", "true", "yes", "on"}) and not TELEGRAM_HARD_DISABLED
        token = "" if TELEGRAM_HARD_DISABLED else str(os.getenv("TELEGRAM_BOT_TOKEN", "") or "").strip()
        chat_id = "" if TELEGRAM_HARD_DISABLED else str(os.getenv("TELEGRAM_CHAT_ID", "") or "").strip()
        return enabled_flag, token, chat_id

    def _telegram_can_run_from_env(self) -> bool:
        enabled_flag, token, chat_id = self._telegram_credentials_from_env()
        return bool(enabled_flag and token and chat_id)

    def _telegram_worker_script_path(self) -> Path:
        return APP_DIR / "run_telegram_worker.py"

    def _telegram_runtime_root(self) -> Path:
        runtime_dir = APP_DIR / "runtime"
        runtime_dir.mkdir(parents=True, exist_ok=True)
        return runtime_dir

    def _telegram_queue_dir(self) -> Path:
        queue_dir = self._telegram_runtime_root() / "telegram_queue"
        queue_dir.mkdir(parents=True, exist_ok=True)
        return queue_dir

    def _telegram_worker_log_path(self) -> Path:
        log_path = self._telegram_runtime_root() / "telegram_worker.log"
    ...
    def _update_telegram_header_controls(self) -> None:
        label = getattr(self, "lbl_header_telegram_chip", None)
        btn_start = getattr(self, "btn_header_tg_start", None)
        btn_stop = getattr(self, "btn_header_tg_stop", None)
        can_run = self._telegram_can_run_from_env()
        is_running = self._is_telegram_worker_running()
        runtime_enabled = bool(getattr(self, "_telegram_runtime_enabled", False))
        if label is not None:
            if TELEGRAM_HARD_DISABLED:
                label.setText("TG: BLOCKED")
                label.setStyleSheet("background:#3f1d1d;color:#fecaca;border:1px solid #ef4444;border-radius:10px;padding:4px 8px;font-weight:800;")
            elif not can_run:
                label.setText("TG: ENV OFF")
                label.setStyleSheet("background:#3b2f0f;color:#fde68a;border:1px solid #f59e0b;border-radius:10px;padding:4px 8px;font-weight:800;")
            elif runtime_enabled and is_running:
                label.setText("TG: ON")
                label.setStyleSheet("background:#052e22;color:#bbf7d0;border:1px solid #10b981;border-radius:10px;padding:4px 8px;font-weight:800;")
            elif runtime_enabled:
                label.setText("TG: STARTING")
                label.setStyleSheet("background:#082f49;color:#bae6fd;border:1px solid #38bdf8;border-radius:10px;padding:4px 8px;font-weight:800;")
            else:
                label.setText("TG: OFF")
                label.setStyleSheet("")
        if btn_start is not None:
            btn_start.setEnabled((not TELEGRAM_HARD_DISABLED) and can_run and not is_running)
        if btn_stop is not None:
            btn_stop.setEnabled(is_running or runtime_enabled)

    def _set_engine_telegram_runtime_enabled(self, enabled: bool) -> None:
        cfg = getattr(self, "current_cfg", None)
        if cfg is not None:
            setattr(cfg, "telegram_enabled", bool(enabled))
        engine = getattr(self, "engine", None)
        if engine is not None:
            try:
                if hasattr(engine, "cfg") and engine.cfg is not None:
                    setattr(engine.cfg, "telegram_enabled", bool(enabled))
            except Exception:
                pass
            try:
                notifier = getattr(engine, "telegram", None)
                if notifier is not None:
                    notifier.enabled = bool(enabled and getattr(notifier, "bot_token", "") and getattr(notifier, "chat_id", ""))
            except Exception:
                pass

    def _start_telegram_worker_process(self) -> bool:
        if TELEGRAM_HARD_DISABLED:
            return False
        if self._is_telegram_worker_running():
            self._update_telegram_header_controls()
            return True
        if not self._telegram_can_run_from_env():
            self.append_log("Telegram: запуск невозможен — проверь TELEGRAM_ENABLED / TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID в .env")
            self._update_telegram_header_controls()
            return False
        worker_script = self._telegram_worker_script_path()
        if not worker_script.exists():
            self.append_log(f"Telegram: не найден worker: {worker_script}")
            self._update_telegram_header_controls()
            return False
        try:
            env = os.environ.copy()
            queue_dir = self._telegram_queue_dir()
            log_path = self._telegram_worker_log_path()
            env["TELEGRAM_QUEUE_DIR"] = str(queue_dir)
            env["TELEGRAM_WORKER_LOG"] = str(log_path)
            creationflags = 0
            if os.name == "nt":
                creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0)
            self._telegram_worker_process = subprocess.Popen(
                [sys.executable, str(worker_script), "--queue-dir", str(queue_dir), "--log-file", str(log_path)],
                cwd=str(APP_DIR),
                env=env,
                creationflags=creationflags,
            )
            self.append_log(f"Telegram: worker started | queue={queue_dir} | log={log_path}")
            return True
        except Exception as exc:
            self._telegram_worker_process = None
            self.append_log(f"Telegram: не удалось запустить worker: {exc}")
            self._update_telegram_header_controls()
            return False

    def _stop_telegram_worker_process(self) -> None:
        proc = getattr(self, "_telegram_worker_process", None)
        if proc is None:
            self._telegram_worker_process = None
            return
    ...
    def start_telegram_notifications(self) -> None:
        if TELEGRAM_HARD_DISABLED:
            self.append_log("Telegram: полностью отключён в сборке")
            self._update_telegram_header_controls()
            return
        if not self._telegram_can_run_from_env():
            self.append_log("Telegram: запуск невозможен — проверь .env")
            self._update_telegram_header_controls()
            return
        started = self._start_telegram_worker_process()
        if not started:
            return
        self._telegram_runtime_enabled = True
        self._set_engine_telegram_runtime_enabled(True)
        cfg = getattr(self, "current_cfg", None)
        if cfg is None:
            enabled_flag, token, chat_id = self._telegram_credentials_from_env()
            if enabled_flag and token and chat_id:
                cfg = BotConfig(
                    api_key="",
                    secret_key="",
                    passphrase="",
        ...
        self._update_telegram_header_controls()
        self.append_log("Telegram: уведомления включены")

    def stop_telegram_notifications(self) -> None:
        self._telegram_runtime_enabled = False
        self._set_engine_telegram_runtime_enabled(False)
        self._stop_telegram_screenshot_timer()
        self.telegram_ui_notifier = None
        self._stop_telegram_worker_process()
        self._update_telegram_header_controls()
        self.append_log("Telegram: уведомления отключены")
    ...
    def _configure_telegram_screenshots(self, cfg: Optional[BotConfig]) -> None:
        ...
        if TELEGRAM_HARD_DISABLED:
            self._update_telegram_header_controls()
            return
        if not bool(getattr(self, "_telegram_runtime_enabled", False)):
            self._update_telegram_header_controls()
            return
        if cfg is None:
    ...
    def export_analysis_bundle(self) -> None:
        try:
            ...
            self._append_scanner_exports_to_target(archive_path)
    ...
    def _append_scanner_exports_to_target(self, export_target: Path) -> None:
        try:
            scanner = getattr(self.engine, "market_scanner", None) if self.engine is not None else getattr(self, "market_scanner", None)
            rows = scanner.export_validation_rows() if scanner is not None else []
            csv_payload = None
            json_payload = None
            if rows:
            ...
            for path_obj, arc_name in ((SCANNER_VALIDATION_LOG_FILE, 'scanner_validation_log.csv'), (SCANNER_VALIDATION_STATE_FILE, 'scanner_validation_state.json')):
                if path_obj.exists():
                    extra_files.append((path_obj, arc_name))

            if export_target.is_dir():
                if csv_payload is not None:
                    (export_target / 'scanner_validation_summary.csv').write_text(csv_payload, encoding='utf-8')
                if json_payload is not None:
                    (export_target / 'scanner_validation_summary.json').write_text(json_payload, encoding='utf-8')
                for path_obj, arc_name in extra_files:
                    shutil.copy2(path_obj, export_target / arc_name)
                return

            with zipfile.ZipFile(export_target, 'a', compression=zipfile.ZIP_DEFLATED) as zf:
                if csv_payload is not None:
                    zf.writestr('scanner_validation_summary.csv', csv_payload)
                if json_payload is not None:
                    zf.writestr('scanner_validation_summary.json', json_payload)
                for path_obj, arc_name in extra_files:
    ...
    def _on_worker_engine_ready(self, engine: TurtleEngine) -> None:
        try:
            ...
            if cfg is None:
                return
            self._telegram_last_cfg_enabled = bool(getattr(cfg, "telegram_enabled", False))
            self._telegram_runtime_enabled = bool(getattr(cfg, "telegram_enabled", False))
            if self._telegram_runtime_enabled:
                self._start_telegram_worker_process()
            self._configure_telegram_screenshots(cfg)
