...
# Version: v097
# Date: 2026-03-22
# Changes: based on stable v096_5, added isolated Telegram queue worker with GUI start/stop controls, switched Telegram queue to runtime/telegram_queue with absolute paths, restored standalone .env loading for worker, and fixed analysis export dynamic reconciler import.
...
class MainWindow(QMainWindow):
    ...
    def _refresh_telegram_controls(self) -> None:
        ...
        status = "TG: ON" if running else ("TG: OFF" if env_ready else "TG: ENV OFF")
        ...
        if btn_start is not None:
            btn_start.setEnabled(env_ready and not running)
    ...
    def start_telegram_worker(self) -> None:
        ...
        try:
            ...
            cmd = [sys.executable, str(APP_DIR / "run_telegram_worker.py"), "--queue-dir", str(queue_dir), "--log-file", str(log_file), "--pid-file", str(pid_file)]
            creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0) if os.name == "nt" else 0
            proc = subprocess.Popen(cmd, cwd=str(APP_DIR), creationflags=creationflags)
            self._telegram_worker_process = proc
            self.append_log(f"Telegram: worker запущен | queue={queue_dir}")
        except Exception as exc:
            self.append_log(f"Telegram: ошибка запуска worker: {exc}")
        self._refresh_telegram_controls()
