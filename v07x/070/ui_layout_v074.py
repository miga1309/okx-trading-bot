...
def build_main_window_ui(window, deps):
    ...
    center_col.addWidget(chart_box, stretch=9)

    window.market_radar_box = NeonPanel("Market Radar")
    radar_outer = QVBoxLayout(window.market_radar_box)
    radar_outer.setContentsMargins(8, 4, 8, 8)
    radar_outer.setSpacing(4)

    radar_header = QHBoxLayout()
    radar_header.setContentsMargins(0, 0, 0, 0)
    radar_header.setSpacing(6)
    window.btn_market_radar_toggle = QPushButton("MARKET RADAR ▼")
    window.btn_market_radar_toggle.setCheckable(True)
    window.btn_market_radar_toggle.setChecked(False)
    window.btn_market_radar_toggle.setMinimumHeight(28)
    window.btn_market_radar_toggle.clicked.connect(window.toggle_market_radar)
    radar_header.addWidget(window.btn_market_radar_toggle, 0)
    radar_header.addStretch(1)
    radar_outer.addLayout(radar_header)

    window.market_radar_content = QWidget()
    radar_layout = QGridLayout(window.market_radar_content)
    ...
    radar_outer.addWidget(window.market_radar_content)
    window.market_radar_content.setVisible(False)
    center_col.addWidget(window.market_radar_box, stretch=0)
