...
# ============================================================
# OKX Turtle Bot
# Version: v079_export_reentry_sync
# Date: 2026-03-15
#
# Changelog v079_export_reentry_sync:
# - Fixed analysis export crash by loading pyramid/re-entry/breakout diagnostic rows before bundle assembly
# - Hardened close-position sync: exchange code 51023 (position missing) is now treated as an already-closed position and local state is synchronized
# - Tightened minimal Turtle breakout confirmation buffer from config so weak close-beyond-channel entries are filtered more aggressively
#
...
class TurtleEngine(QObject):
    ...
    def _confirm_breakout(self, candles: list, atr: float, side: str, level: float) -> tuple[bool, str]:
        ...
        if self._minimal_turtle_entry_mode():
            ...
            close_buffer = float(getattr(self.cfg, "breakout_buffer_atr", 0.15) or 0.15) * atr
...
class LaunchConfigWidget(QWidget):
    ...
    def build_config(self) -> BotConfig:
        ...
        telegram_enabled = telegram_enabled_raw in {"1", "true", "yes", "on"}
    ...
    def launch_engine(self, cfg: BotConfig) -> None:
        ...
        if self.telegram_ui_notifier is not None and getattr(self.telegram_ui_notifier, "enabled", False):
            try:
                ...
                self.telegram_ui_notifier.send(
                    f"OKX Turtle Bot {APP_VERSION} запущен | account={account_label} | tf={cfg.timeframe} | mode={getattr(cfg, 'trade_mode', 'auto')}"
                )
                self.append_log("Telegram: отправлено тестовое сообщение при запуске")
                QTimer.singleShot(2500, self.send_main_window_screenshot_to_telegram)
            except Exception as exc:
                self.append_log(f"Telegram: не удалось отправить стартовое сообщение: {exc}")
