...
# ============================================================
# OKX Turtle Bot
# Version: v076_minimal_breakout_filters
# Date: 2026-03-14
#
...
def reset_local_runtime_files() -> dict:
    ...
    for file_path in (ENGINE_STATS_FILE, SIGNAL_AUDIT_FILE, POSITION_JOURNAL_FILE, POSITION_SNAPSHOTS_FILE, SYSTEM_HEALTH_FILE, APP_LOG):
        try:
...
def _write_export_mode(export_dir: Path, dataset: dict, mode: str) -> None:
    ...
    if mode == 'quick':
        ...
        return
    _write_csv(export_dir / 'balance_timeline.csv', ['time', 'balance_total', 'balance_available', 'balance_used'], dataset['balance_timeline'])
    for name in ['closed_trades', 'open_positions_endstate', 'positions_lifecycle', 'entry_candidates', 'decision_trace', 'execution_log', 'risk_events', 'market_context_on_entry', 'system_health']:
        rows = dataset[name]
...
def build_analysis_export_bundle(mode: str = 'full', snapshot: Optional[dict] = None, cfg: Optional["BotConfig"] = None) -> Path:
    ...
    if mode == 'hourly':
        ...
        all_dts = []
        for key in ['positions_lifecycle', 'entry_candidates', 'decision_trace', 'execution_log', 'risk_events', 'system_health']:
    ...
    def _save_state(self) -> None:
        ...
        if current_minute != self.last_system_health_minute:
            self.system_health_logger.log(
                ...
                cache_errors=_safe_int((self.market_data_cache.snapshot_stats() or {}).get('error_count', 0)),
                cache_fetch_count=_safe_int((self.market_data_cache.snapshot_stats() or {}).get('fetch_count', 0)),
...
class MainWindow(QMainWindow):
    ...
    def reload_ui(self) -> None:
        ...
        try:
            module_name = getattr(self, "_ui_layout_module_name", "ui_layout_v077")
    ...
    def _build_ui(self) -> None:
        self._ui_layout_module_name = "ui_layout_v077"
