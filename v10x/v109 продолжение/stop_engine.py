...
class StopEngine:
    ...
    def _ensure_exchange_stop(self, state: PositionState, target_stop: float, current_price: float = 0.0, reason: str = "") -> bool:
        if self._stop_management_disabled(state):
            return False
        target_stop = self._normalize_stop_price(state, self._safe_short_stop_target(state, float(target_stop or 0.0), current_price=current_price))
        if target_stop <= 0.0:
            return False
        ...
        if self._adopt_existing_exchange_stop(state, target_stop=target_stop, status="active"):
            ...
            threshold = self._exchange_stop_move_threshold(state, target_stop)
            full_cover = bool(getattr(state, "exchange_stop_full_position", False))
            if full_cover and current_exchange_stop > 0.0 and abs(current_exchange_stop - target_stop) <= max(threshold, 1e-12):
    ...
    def _sync_absent_position_once(self, state: PositionState, current_price: float = 0.0, reason: str = "position_absent_on_exchange") -> bool:
        ...
        self._log_stop_engine("STOP_POSITION_ABSENT_SYNC", state, reason=reason, current_price=current_price)
    ...
    def _place_exchange_stop(self, state: PositionState, target_stop: float, reason: str = "") -> bool:
        if not bool(getattr(self.cfg, "exchange_protective_stop_enabled", True)):
            return False
        target_stop = self._normalize_stop_price(state, self._safe_short_stop_target(state, float(target_stop or 0.0), current_price=float(getattr(state, "last_px", 0.0) or getattr(state, "avg_px", 0.0) or 0.0)))
