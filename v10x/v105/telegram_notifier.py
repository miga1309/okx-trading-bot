...
class TelegramQueueWorker:
    def __init__(self, bot_token: str, chat_id: str, queue_dir: Optional[str] = None, max_attempts: int = 10, status_file: Optional[str] = None, offset_file: Optional[str] = None):
        self.bot_token = str(bot_token or "").strip()
        self.chat_id = str(chat_id or "").strip()
        self.queue_dir = ensure_queue_layout(queue_dir)
    ...
    def _handle_command(self, text: str) -> None:
        command = str(text or "").strip().split()[0].split("@", 1)[0].lower()
        if command == "/ping":
            ...
            payload = self._read_status_payload()
            chart_path = Path(str(payload.get("chart_image_path") or "")).expanduser()
            if chart_path.exists() and chart_path.is_file():
                self._send_photo({"photo_path": str(chart_path), "caption": f"GUI snapshot {payload.get('updated_at', '')}"})
            else:
                self._send_message({"text": "Chart not ready yet. Окно должно быть открыто и не свернуто."})
        elif command == "/help":
...
def run_worker_forever(queue_dir: Optional[str] = None, poll_interval: float = 2.0, max_attempts: int = 10, log_file: Optional[str] = None, pid_file: Optional[str] = None, status_file: Optional[str] = None, offset_file: Optional[str] = None) -> None:
    load_dotenv()
    ...
    worker = TelegramQueueWorker(bot_token=bot_token, chat_id=chat_id, queue_dir=str(queue_path), max_attempts=max_attempts, status_file=status_file, offset_file=offset_file)
...
def main() -> None:
    ...
    run_worker_forever(queue_dir=args.queue_dir, poll_interval=args.poll_interval, max_attempts=args.max_attempts, log_file=args.log_file, pid_file=args.pid_file, status_file=args.status_file, offset_file=args.offset_file)
