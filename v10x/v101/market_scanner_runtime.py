...
class MarketScannerRuntime:
    ...
    def allows_entry(self, inst_id: str) -> tuple[bool, str, ScannerStatus]:
        ...
        if bool(raw.get('book_stale')):
            return False, 'scanner_book_stale', status
        if bool(raw.get('candle_stale')):
    ...
    def refresh_symbol(self, inst_id: str) -> ScannerStatus:
        ...
        try:
            ...
            analysis['book_stale'] = bool(getattr(self.cfg, 'ws_books_enabled', True)) and not any(str(r.get('source') or '') == 'ws' for r in book_rows)
    ...
    def _analyze_symbol(self, inst_id: str, long_candles: List[List[float]], fast_candles: List[List[float]], trades: List[dict], book_rows: List[dict]) -> Dict[str, Any]:
        ...
        if dead_flag:
            ...
            final_score = 0.0
        book_metrics = {
            'spread_bps': statistics.fmean([float(r.get('spread_bps') or 0.0) for r in book_rows if r.get('spread_bps') is not None]) if book_rows else 0.0,
            'depth_sum': statistics.fmean([float(r.get('depth_sum') or 0.0) for r in book_rows]) if book_rows else 0.0,
            'liquidity_hole_ratio': min([float(r.get('liquidity_hole_ratio') or 1.0) for r in book_rows] or [1.0]),
        }
        thin_book = book_metrics['depth_sum'] <= float(getattr(self.cfg, 'ws_book_min_side_notional_usdt', 2500.0) or 2500.0) * 2.0
        liquidity_hole = book_metrics['liquidity_hole_ratio'] <= float(getattr(self.cfg, 'ws_book_liquidity_hole_ratio', 0.22) or 0.22)
