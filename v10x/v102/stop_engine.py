...
if TYPE_CHECKING:
    from trade_models import PositionState
...
class StopEngine:
    ...
    def _place_exchange_stop(self, state: PositionState, target_stop: float, reason: str = "") -> bool:
        ...
        for attempt in range(1, attempts + 1):
            ...
            if fail_code == "51280" or "trigger price must be less than the last price" in fail_msg.lower() or "trigger price must be greater than the last price" in fail_msg.lower():
                ...
                return False
            if fail_code == "51023" or "position doesn't exist" in fail_msg.lower():
                ...
                self._mark_position_health(state, "STOP_UNVERIFIED", reason="position_absent_on_exchange")
                self.log_line.emit(f"[STOP] {state.inst_id}: позиция отсутствует на бирже, stop retry остановлен и запущена синхронизация")
                self._update_stop_tracking(state, False, reason="position_absent_on_exchange", hard=True)
                return False
    ...
    def _verify_initial_stop_if_needed(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        attempts = max(2, int(getattr(self.cfg, "initial_stop_verify_attempts", 6) or 6))
        ...
        placed = self._place_exchange_stop(state, target_stop, reason="initial_stop_verify_recover")
    ...
    def _run_stop_health_check(self, state: PositionState, current_price: float = 0.0) -> bool:
        ...
        if existing_row is None:
            ...
            self._log_stop_engine("STOP_HEALTH_INVALID", state, current_price=current_price, row_state=self._algo_row_state(existing_row))
            self._clear_exchange_stop_state(state, status="health_invalid")
            restored = self._place_exchange_stop(state, target_stop, reason="health_refresh")
        if restored and self._confirm_exchange_stop_present(state, target_stop, attempts=4, delay_sec=0.25):
            state.stop_recovery_failures = 0
    ...
    def _amend_exchange_stop(self, state: PositionState, target_stop: float, current_price: float = 0.0, reason: str = "") -> bool:
        ...
        replaced = self._place_exchange_stop(state, target_stop, reason=f"amend_fallback:{reason}")
        if replaced:
    ...
    def _stop_reconcile_required(self, state: PositionState, current_price: float = 0.0, candles: Optional[List[List[float]]] = None) -> bool:
        ...
        if bool(getattr(state, "exchange_stop_desync", False)):
            return True
        if desired_mode == "TURTLE" and active_mode == "ATR" and candles:
    ...
    def _run_stop_recovery(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "runtime_reconcile") -> bool:
        ...
        turtle_stop = self._turtle_stop_target(state, candles)
        if desired_mode == "TURTLE" and active_mode != "TURTLE" and not self._can_switch_to_turtle(state, turtle_stop):
            target_mode = "ATR"
        ...
        if self._exchange_stop_row_is_valid(state, existing_row, target_stop=target_stop, require_full_cover=True):
            ...
            ok = self._place_exchange_stop(state, target_stop, reason=reason or "runtime_reconcile")
    ...
    def _apply_stop_policy(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "manage") -> None:
        ...
        if active_mode != target_mode:
            ...
            if existing_valid and not needs_cover_replace:
                ...
                moved = self._force_replace_exchange_stop(state, target_stop, current_price=current_price, reason=f"switch_to_{target_mode.lower()}_full_cover")
                ...
                if abs(target_stop - prev_exchange_stop) >= threshold:
                    ...
                    if not moved:
                        moved = self._force_replace_exchange_stop(state, target_stop, current_price=current_price, reason=f"switch_to_{target_mode.lower()}_refresh")
            else:
        ...
        if needs_cover_replace:
            moved = self._force_replace_exchange_stop(state, target_stop, current_price=current_price, reason=f"{target_mode.lower()}_full_cover")
            if moved:
                state.stop_price = target_stop
                self._clear_exchange_stop_desync(state)
                return
