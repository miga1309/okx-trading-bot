...
# Version: v106
# Date: 2026-03-24
# Changes: rebuilt stop lifecycle around a single stop_engine path, taught the engine to adopt/amend existing exchange TP/SL orders instead of duplicating them, improved absent-position synchronization, and cleaned the initial ATR stop flow.
...
from telegram_notifier import TelegramNotifier, ensure_queue_layout, resolve_log_file, resolve_pid_file, resolve_status_file, resolve_offset_file
...
from okx_ws_manager import OkxWsManager
...
class TurtleEngine(QObject):
    ...
    def enter_position(self, inst_id: str, side: str, price: float, atr: float, system_name: str, candidate: Optional[dict] = None) -> bool:
        ...
        self._log_execution_event("order_submit", inst_id=inst_id, side=side, system_name=system_name, qty=qty, price=price, atr=atr, stop_price=stop_price, pending_entry_id=pending.pending_entry_id)
        try:
        ...
        stop_ok = self._place_exchange_stop(state, stop_price, reason="initial_atr_stop")
        if stop_ok:
            ...
            self._log_stop_engine("STOP_INIT_PLACED", state, requested_stop=stop_price, reason="initial_atr_stop")
        self._update_stop_tracking(state, stop_ok, reason="initial_exchange_stop", verified=False if stop_ok else True)
        self.pending_entries.pop(pending.pending_entry_id, None)
    ...
    def start_telegram_worker(self) -> None:
        ...
        try:
            ...
            if self._telegram_worker_exe_exists():
                cmd = [str(TELEGRAM_WORKER_EXE_FILE), "--queue-dir", str(queue_dir), "--log-file", str(log_file), "--pid-file", str(pid_file), "--status-file", str(status_file), "--offset-file", str(offset_file)]
                launch_desc = f"exe={TELEGRAM_WORKER_EXE_FILE}"
            else:
                py_exec = sys.executable or "python"
                cmd = [str(py_exec), str(TELEGRAM_WORKER_SCRIPT_FILE), "--queue-dir", str(queue_dir), "--log-file", str(log_file), "--pid-file", str(pid_file), "--status-file", str(status_file), "--offset-file", str(offset_file)]
                launch_desc = f"python={py_exec} script={TELEGRAM_WORKER_SCRIPT_FILE}"
