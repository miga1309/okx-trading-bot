...
def _write_export_mode(export_dir: Path, dataset: dict, mode: str) -> None:
    ...
    if mode == 'quick':
        ...
        return
    _write_csv(export_dir / 'balance_timeline.csv', ['time', 'balance_total', 'balance_available', 'balance_used'], dataset['balance_timeline'])
    for name in ['closed_trades', 'open_positions_endstate', 'positions_lifecycle', 'entry_candidates', 'decision_trace', 'execution_log', 'risk_events', 'market_context_on_entry', 'trade_diagnostics', 'trade_analysis', 'market_analysis', 'system_health', 'pyramid_diagnostics', 'reentry_diagnostics', 'breakout_quality', 'stop_engine']:
        rows = dataset[name]
