...
# Version: v089
# Date: 2026-03-19
# Changes: replaced old market scanner buckets with binary DEAD/SAW/RIPPING filters; added Market Scanner review tabs with manual validation; allowed pairs are admitted progressively during scan; scanner validation logs are exported with analysis bundles.
...
class TurtleEngine(QObject):
    ...
    def sync_positions_from_exchange(self) -> None:
        ...
        for pos in exchange_positions:
            ...
            if current:
                ...
                restored_fields = restore_sync_position_state(current, self.cfg, inst_id, pos_side, avg_px, last_px, qty, atr)
                if restored_fields:
                    changed = True
                ...
                restored_fields = restore_sync_position_state(self.position_state[inst_id], self.cfg, inst_id, pos_side, avg_px, last_px, qty, atr)
    ...
    def scan_markets(self) -> None:
        ...
        if getattr(self.cfg, "scanner_enabled", True):
            ...
            processed = self.market_scanner.run_chunk(getattr(self.cfg, "scanner_chunk_size", 4))
            if processed > 0:
    ...
    def _confirm_exchange_stop_present(self, state: PositionState, target_stop: float, attempts: int = 6, delay_sec: float = 0.25) -> bool:
        for _ in range(max(1, attempts)):
            if self._sync_stop_from_exchange_snapshot(state, target_stop=target_stop, status="active"):
                ...
                if current > 0.0 and abs(current - float(target_stop or 0.0)) <= max(self._exchange_stop_move_threshold(state, target_stop), 1e-12):
                    return True
            if delay_sec > 0:
                time.sleep(delay_sec)
        return False
    ...
    def _apply_stop_policy(self, state: PositionState, current_price: float, candles: List[List[float]], reason: str = "manage") -> None:
        ...
        if active_mode != desired_mode:
            ...
            old_stop = float(getattr(state, "exchange_stop_price", 0.0) or prev_stop or 0.0)
            cancelled = self._cancel_exchange_stop(state, reason=f"switch_to_{desired_mode.lower()}")
            if not cancelled:
                self._log_stop_engine("STOP_SWITCH_FAILED", state, old_mode=old_mode, new_mode=desired_mode, old_stop=old_stop, requested_stop=target_stop, reason="cancel_failed")
                return
            if not self._confirm_exchange_stop_absent(state):
                self._log_stop_engine("STOP_SWITCH_FAILED", state, old_mode=old_mode, new_mode=desired_mode, old_stop=old_stop, requested_stop=target_stop, reason="cancel_not_confirmed")
                return
            placed = self._place_exchange_stop(state, target_stop, reason=f"switch_to_{desired_mode.lower()}")
            if not placed:
                self._log_stop_engine("STOP_SWITCH_FAILED", state, old_mode=old_mode, new_mode=desired_mode, old_stop=old_stop, requested_stop=target_stop, reason="place_failed")
                return
            if not self._confirm_exchange_stop_present(state, target_stop):
                self._log_stop_engine("STOP_SWITCH_FAILED", state, old_mode=old_mode, new_mode=desired_mode, old_stop=old_stop, requested_stop=target_stop, reason="place_not_confirmed")
                return
            state.active_stop_mode = desired_mode
            state.stop_price = target_stop
            self._log_stop_engine("STOP_MODE_SWITCHED", state, old_mode=old_mode, new_mode=desired_mode, old_stop=old_stop, new_stop=target_stop, reason=reason)
        ...
        state.stop_price = target_stop
        if abs(target_stop - prev_stop) >= threshold:
            self._replace_exchange_stop_if_needed(state, prev_stop, target_stop, current_price=current_price, reason=f"{desired_mode.lower()}_update")
    ...
    def update_and_maybe_exit_or_pyramid(self, state: PositionState) -> None:
        ...
        self._apply_stop_policy(state, current_price, candles, reason="manage_open_positions")
...
class MainWindow(QMainWindow):
    ...
    def _build_scanner_verdict_combo(self, filter_type: str, pair: str, verdict: str) -> QComboBox:
        combo = QComboBox()
        combo.addItems(["Не проверено", "Подтверждено", "Не подтверждено"])
        combo.setProperty("scanner_filter_type", str(filter_type or "").upper())
        combo.setProperty("scanner_pair", str(pair or "").strip())
        idx = combo.findText(verdict)
        combo.setCurrentIndex(idx if idx >= 0 else 0)
        combo.currentTextChanged.connect(self._on_scanner_verdict_changed)
        return combo
    ...
    def _update_scanner_review_views(self) -> None:
        ...
        if scanner is None:
            return
        for filter_type in ("DEAD", "SAW", "RIPPING"):
    ...
    def _on_scanner_table_cell_changed(self, filter_type: str, row: int, col: int) -> None:
        ...
        table = self._scanner_table_for_filter(filter_type)
        scanner = getattr(self.engine, "market_scanner", None) if self.engine is not None else getattr(self, "market_scanner", None)
        if table is None or scanner is None:
            return
        pair_item = table.item(row, 0)
        comment_item = table.item(row, 7)
        verdict_widget = table.cellWidget(row, 5)
        ...
        if isinstance(verdict_widget, QComboBox):
            verdict = str(verdict_widget.currentText() or "Не проверено").strip() or "Не проверено"
        if verdict not in {"Не проверено", "Подтверждено", "Не подтверждено"}:
            verdict = "Не проверено"
    ...
    def _on_scanner_verdict_changed(self, verdict: str) -> None:
        combo = self.sender()
        scanner = getattr(self.engine, "market_scanner", None) if self.engine is not None else getattr(self, "market_scanner", None)
        if not isinstance(combo, QComboBox) or scanner is None:
            return
        filter_type = str(combo.property("scanner_filter_type") or "").upper()
        pair = str(combo.property("scanner_pair") or "").strip()
        if not pair or pair == "—":
            return
        table = self._scanner_table_for_filter(filter_type)
        comment = ""
        if table is not None:
            for row in range(table.rowCount()):
                pair_item = table.item(row, 0)
                verdict_widget = table.cellWidget(row, 5)
                if pair_item is None or verdict_widget is not combo:
                    continue
                comment_item = table.item(row, 7)
                comment = str(comment_item.text() or "").strip() if comment_item is not None else ""
                break
        verdict = str(verdict or "Не проверено").strip() or "Не проверено"
        if verdict not in {"Не проверено", "Подтверждено", "Не подтверждено"}:
            verdict = "Не проверено"
        scanner.update_validation(pair, filter_type, verdict, comment)
        QTimer.singleShot(0, self._update_scanner_review_views)
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        if hasattr(self, 'lbl_scanner_total'): self.lbl_scanner_total.setText('TOTAL: 0')
        if hasattr(self, 'lbl_scanner_scanned'): self.lbl_scanner_scanned.setText('SCANNED: 0')
        if hasattr(self, 'lbl_scanner_allowed'): self.lbl_scanner_allowed.setText('ALLOWED: 0')
        if hasattr(self, 'lbl_scanner_blocked'): self.lbl_scanner_blocked.setText('BLOCKED: 0')
        if hasattr(self, 'lbl_scanner_dead'): self.lbl_scanner_dead.setText('DEAD: 0')
        if hasattr(self, 'lbl_scanner_saw'): self.lbl_scanner_saw.setText('SAW: 0')
        if hasattr(self, 'lbl_scanner_ripping'): self.lbl_scanner_ripping.setText('RIPPING: 0')
        if hasattr(self, 'lbl_scanner_pending'): self.lbl_scanner_pending.setText('PENDING: 0')
        if hasattr(self, 'lbl_scanner_status'): self.lbl_scanner_status.setText('STATUS: IDLE')
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        if hasattr(self, 'lbl_scanner_total'): self.lbl_scanner_total.setText(f"TOTAL: {int(analytics.get('scanner_total', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_scanned'): self.lbl_scanner_scanned.setText(f"SCANNED: {int(analytics.get('scanner_scanned', analytics.get('scanner_ready', 0)) or 0)}")
        if hasattr(self, 'lbl_scanner_allowed'): self.lbl_scanner_allowed.setText(f"ALLOWED: {int(analytics.get('scanner_allowed', analytics.get('scanner_admitted', 0)) or 0)}")
        if hasattr(self, 'lbl_scanner_blocked'): self.lbl_scanner_blocked.setText(f"BLOCKED: {int(analytics.get('scanner_blocked', analytics.get('scanner_risky', 0)) or 0)}")
        if hasattr(self, 'lbl_scanner_dead'): self.lbl_scanner_dead.setText(f"DEAD: {int(analytics.get('scanner_dead', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_saw'): self.lbl_scanner_saw.setText(f"SAW: {int(analytics.get('scanner_saw', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_ripping'): self.lbl_scanner_ripping.setText(f"RIPPING: {int(analytics.get('scanner_ripping', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_pending'): self.lbl_scanner_pending.setText(f"PENDING: {int(analytics.get('scanner_pending', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_status'): self.lbl_scanner_status.setText(f"STATUS: {str(analytics.get('scanner_status_text', 'IDLE') or 'IDLE')}")
