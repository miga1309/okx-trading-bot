...
VERDICT_REJECTED = "Не подтверждено"
...
class MarketScannerRuntime:
    ...
    def refresh_symbol(self, inst_id: str) -> ScannerStatus:
        ...
        try:
            ...
            trades = self._collect_trades(inst_id)
            book_rows = self._collect_liquidity(inst_id)
    ...
    def review_rows(self, filter_type: str) -> List[dict]:
        ...
        def _sort_key(row: dict):
            verdict = str(row.get("user_verdict") or VERDICT_UNCHECKED)
            pr = 0 if verdict == VERDICT_UNCHECKED else 1 if row.get("recheck_needed") else 2 if verdict == VERDICT_REJECTED else 3
            return (pr, -int(row.get("hit_count") or 0), -(float(row.get("last_seen_ts") or 0.0)))
        rows.sort(key=_sort_key)
        return rows

    def update_validation(self, inst_id: str, filter_type: str, verdict: str, comment: str = "") -> None:
        ...
        if not record:
            return
        record["user_verdict"] = verdict or VERDICT_UNCHECKED
    ...
    def build_popup_payload(self, inst_id: str, filter_type: str) -> dict:
        ...
        review = self._review_records.get(self._key(inst_id, filter_type), {})
        return {
            "inst_id": inst_id,
            "timeframe": tf_popup,
            "side": "long",
    ...
    def _compute_dead_metrics(self, long_candles, fast_candles, trades, book_rows):
        ...
        candle_dead = atr_ratio < float(getattr(self.cfg, 'scanner_dead_atr_ratio_max', 0.004) or 0.004) and active_ratio < float(getattr(self.cfg, 'scanner_dead_active_ratio_max', 0.35) or 0.35) and spike_ratio > float(getattr(self.cfg, 'scanner_dead_spike_ratio_min', 4.0) or 4.0) and follow_through < float(getattr(self.cfg, 'scanner_dead_follow_through_max', 0.5) or 0.5)
        trade_dead = trade_count_per_min < float(getattr(self.cfg, 'scanner_dead_trade_count_per_min_max', 6.0) or 6.0) or zero_trade_ratio > float(getattr(self.cfg, 'scanner_dead_zero_trade_ratio_min', 0.30) or 0.30) or intertrade_gap_p95 > float(getattr(self.cfg, 'scanner_dead_gap_p95_min_sec', 45.0) or 45.0)
        book_dead = spread_bps > float(getattr(self.cfg, 'scanner_dead_spread_bps_min', 8.0) or 8.0) or top_depth < float(getattr(self.cfg, 'scanner_dead_depth_usdt_max', 2500.0) or 2500.0)
        summary = f"dead_votes candle={int(candle_dead)} trade={int(trade_dead)} book={int(book_dead)}"
        return {
    ...
    def _compute_ripping_metrics(self, fast_candles, trades, book_rows):
        ...
        candle_ripping = atr_ratio > float(getattr(self.cfg, 'scanner_ripping_atr_ratio_min', 0.006) or 0.006) and range_cv > float(getattr(self.cfg, 'scanner_ripping_range_cv_min', 1.0) or 1.0) and jump_ratio > float(getattr(self.cfg, 'scanner_ripping_jump_ratio_min', 3.0) or 3.0)
        trade_gaps = self._trade_gaps_seconds(trades)
        intertrade_gap_p95 = self._percentile(trade_gaps, 95)
        ...
        flow_ripping = burst_trade_ratio > float(getattr(self.cfg, 'scanner_ripping_burst_trade_ratio_min', 0.5) or 0.5) or trade_size_cv > float(getattr(self.cfg, 'scanner_ripping_trade_size_cv_min', 1.25) or 1.25) or intertrade_gap_p95 > float(getattr(self.cfg, 'scanner_ripping_gap_p95_min_sec', 25.0) or 25.0)
        spreads = [float(r.get('spread_bps') or 0.0) for r in book_rows if r.get('spread_bps') is not None]
        depths = [float(r.get('depth_sum') or 0.0) for r in book_rows if r.get('depth_sum') is not None]
        spread_bps = statistics.fmean(spreads) if spreads else 0.0
        spread_cv = (statistics.pstdev(spreads) / spread_bps) if len(spreads) > 1 and spread_bps > 0 else 0.0
        depth_cv = (statistics.pstdev(depths) / statistics.fmean(depths)) if len(depths) > 1 and statistics.fmean(depths) > 0 else 0.0
        top_depth = statistics.fmean(depths) if depths else 0.0
        book_ripping = spread_cv > float(getattr(self.cfg, 'scanner_ripping_spread_cv_min', 0.5) or 0.5) or depth_cv > float(getattr(self.cfg, 'scanner_ripping_depth_cv_min', 0.8) or 0.8) or top_depth < float(getattr(self.cfg, 'scanner_ripping_depth_usdt_max', 2500.0) or 2500.0)
    ...
    def _append_validation_log(self, record: dict) -> None:
        ...
        with self.validation_log_path.open('a', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['timestamp','pair','filter_type','user_verdict','comment','hit_count','reason','metrics_json'])
            if write_header:
                writer.writeheader()
            writer.writerow({
