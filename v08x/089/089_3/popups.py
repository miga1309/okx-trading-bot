...
class ScannerReviewDialog(QDialog):
    ...
    def __init__(self, payload: dict, parent=None):
        ...
        self.payload = payload or {}
        self._saved = False
        self.setWindowTitle(f"Scanner Review — {self.payload.get('inst_id', '—')}")
        self.resize(1040, 760)
        root = QVBoxLayout(self)
        root.setContentsMargins(12, 12, 12, 12)
        root.setSpacing(8)

        title = QLabel(f"{self.payload.get('inst_id', '—')} · {self.payload.get('scanner_filter_type', '—')} · TF {self.payload.get('timeframe', '—')}")
        ...
        notes = QLabel(str(self.payload.get('scanner_notes') or ''))
        notes.setWordWrap(True)
        notes.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        notes.setStyleSheet("padding:8px; border:1px solid #2b4b66; border-radius:8px; background:#0a1422;")
