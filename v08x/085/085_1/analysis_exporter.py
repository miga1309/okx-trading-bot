...
def _apply_context(ctx: ExportBundleContext) -> None:
    ...
    global POSITION_SNAPSHOTS_FILE, SYSTEM_HEALTH_FILE, PYRAMID_DIAGNOSTICS_FILE
    global REENTRY_DIAGNOSTICS_FILE, BREAKOUT_QUALITY_FILE, STOP_ENGINE_FILE, ANALYSIS_EXPORT_DIR
...
def _build_position_runtime_regime_rows(open_positions: List[dict]) -> List[dict]:
    ...
    for row in open_positions or []:
        rows.append({
            ...
            'side': row.get('side', ''),
            'system_name': row.get('system_name', ''),
            'turtle_exit_period': _safe_int(row.get('turtle_exit_period', 0), 0),
            'hybrid_stop_mode': row.get('hybrid_stop_mode', ''),
            'trend_hold_state': row.get('trend_hold_state', ''),
            'sync_status': row.get('sync_status', ''),
            'reconciler_interval_sec': _safe_int(row.get('reconciler_interval_sec', 0), 0),
            'qty': _safe_float(row.get('qty', 0.0)),
            'avg_px': _safe_float(row.get('avg_px', 0.0)),
            'last_px': _safe_float(row.get('last_px', 0.0)),
            'pnl_pct': _safe_float(row.get('pnl_pct', 0.0)),
...
def _write_export_mode(export_dir: Path, dataset: dict, mode: str) -> None:
    ...
    if mode == 'quick':
        ...
        return
    _write_csv(export_dir / 'balance_timeline.csv', ['time', 'balance_total', 'balance_available', 'balance_used'], dataset['balance_timeline'])
    for name in ['closed_trades', 'open_positions_endstate', 'positions_lifecycle', 'entry_candidates', 'decision_trace', 'execution_log', 'risk_events', 'market_context_on_entry', 'trade_diagnostics', 'trade_analysis', 'market_analysis', 'position_reconcile_history', 'position_sync_status', 'position_stop_regime', 'position_runtime_regime', 'trend_capture_analysis', 'trend_hold_analysis', 'sync_drift_events', 'system_health', 'pyramid_diagnostics', 'reentry_diagnostics', 'breakout_quality', 'stop_engine']:
        rows = dataset[name]
...
def build_analysis_export_bundle(ctx: ExportBundleContext, mode: str = 'full', snapshot: Optional[dict] = None, cfg: Optional["BotConfig"] = None) -> Path:
    ...
    if mode == 'hourly':
        ...
        all_dts = []
        for key in ['positions_lifecycle', 'entry_candidates', 'decision_trace', 'execution_log', 'risk_events', 'system_health', 'pyramid_diagnostics', 'reentry_diagnostics', 'breakout_quality', 'stop_engine']:
