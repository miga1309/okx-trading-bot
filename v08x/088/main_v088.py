...
# Version: v088
# Date: 2026-03-18
# Changes: full integration of scanner v021_3 as progressive entry admission gate; manual hard block cleaned to BREV only; Market Scanner counters added to GUI; hourly scanner rescan and scanner-based trade-ready metrics added.
...
class TurtleEngine(QObject):
    ...
    def __init__(self, cfg: BotConfig):
        ...
        self.market_scanner = MarketScannerRuntime(self.gateway, cfg, log_callback=self.log_line.emit, hard_blocked=set(getattr(cfg, "blacklist", []) or []))
...
class MainWindow(QMainWindow):
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        if hasattr(self, 'lbl_scanner_ready'): self.lbl_scanner_ready.setText('READY: 0')
        if hasattr(self, 'lbl_scanner_pending'): self.lbl_scanner_pending.setText('PENDING: 0')
        if hasattr(self, 'lbl_scanner_failed'): self.lbl_scanner_failed.setText('FAILED: 0')
        if hasattr(self, 'lbl_scanner_safe'): self.lbl_scanner_safe.setText('SAFE: 0')
        if hasattr(self, 'lbl_scanner_caution'): self.lbl_scanner_caution.setText('CAUTION: 0')
        if hasattr(self, 'lbl_scanner_risky'): self.lbl_scanner_risky.setText('RISKY: 0')
        if hasattr(self, 'lbl_scanner_blacklist'): self.lbl_scanner_blacklist.setText('BLACKLIST: 0')
        if hasattr(self, 'lbl_scanner_exec_watch'): self.lbl_scanner_exec_watch.setText('EXEC WATCH: 0')
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        if hasattr(self, 'lbl_scanner_ready'): self.lbl_scanner_ready.setText(f"READY: {int(analytics.get('scanner_ready', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_pending'): self.lbl_scanner_pending.setText(f"PENDING: {int(analytics.get('scanner_pending', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_failed'): self.lbl_scanner_failed.setText(f"FAILED: {int(analytics.get('scanner_failed', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_safe'): self.lbl_scanner_safe.setText(f"SAFE: {int(analytics.get('scanner_safe', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_caution'): self.lbl_scanner_caution.setText(f"CAUTION: {int(analytics.get('scanner_caution', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_risky'): self.lbl_scanner_risky.setText(f"RISKY: {int(analytics.get('scanner_risky', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_blacklist'): self.lbl_scanner_blacklist.setText(f"BLACKLIST: {int(analytics.get('scanner_blacklist', 0) or 0)}")
        if hasattr(self, 'lbl_scanner_exec_watch'): self.lbl_scanner_exec_watch.setText(f"EXEC WATCH: {exec_watch}")
