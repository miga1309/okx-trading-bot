# patch_v022_from_current_to_dual_systems.py
# Патч для ТЕКУЩЕГО main_v022.py
#
# Что делает:
# 1) Оставляет уже исправленный расчёт канала без текущей свечи.
# 2) Добавляет обе Turtle-системы в обе стороны:
#    - Turtle 20/10 для long и short
#    - Turtle 55/20 для long и short
# 3) Привязывает entry_period / exit_period к system_name, а не к направлению.
#
# Использование:
#   python patch_v022_from_current_to_dual_systems.py
#
# Создаёт:
#   main_v022.py.bak_dual2
# и обновляет:
#   main_v022.py
...
BACKUP_FILE = Path("main_v022.py.bak_dual2")
...
def main() -> None:
    ...
    def evaluate_entry(self, inst_id: str) -> None:
        ...
        prev_long_window = candles[-self.cfg.long_entry_period - 1:-1]
        prev_short_window = candles[-self.cfg.short_entry_period - 1:-1]
        if len(prev_long_window) < self.cfg.long_entry_period or len(prev_short_window) < self.cfg.short_entry_period:
            return

        long_level = max(float(c[2]) for c in prev_long_window)
        short_level = min(float(c[3]) for c in prev_short_window)
...
'''.strip("\n")
    ...
    text = replace_once(text, old_evaluate_entry, new_evaluate_entry, "evaluate_entry_current")
    text = replace_once(text, old_enter_position_state, new_enter_position_state, "enter_position_state")
