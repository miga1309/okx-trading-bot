# ============================================================
# OKX Turtle Bot
# Version: v048_fullhd_gui
# Date: 2026-03-13
#
# Changelog v048_fullhd_gui:
# - Upgraded the GUI canvas to a FullHD-oriented 1920x1080 cyber terminal layout
# - Added custom market pulse tiles with sparkline mini charts in the central radar block
# - Added a lower neon glow strip chart under the main tabs to echo the approved reference
# - Improved header glow, cards and activity feed styling while keeping the trading engine intact
#
# Previous changelog:
...
from PyQt6.QtCore import QAbstractTableModel, QModelIndex, QObject, QPoint, QPointF, Qt, QThread, QTimer, pyqtSignal, QEvent
    ...
    if is_dark:
        return """
            ...
            QGroupBox {
                ...
                margin-top: 16px;
                padding-top: 16px;
                background: qlineargradient(x1:0,y1:0,x2:1,y2:1,stop:0 #091221, stop:1 #070d18);
            }
            QGroupBox::title {
...
class MiniSparklineWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        glow_color = QColor(line_color.red(), line_color.green(), line_color.blue(), 55)
        min_v = min(self.values)
        max_v = max(self.values)
        if abs(max_v - min_v) < 1e-9:
            max_v += 1.0
            min_v -= 1.0
        step_x = rect.width() / max(1, len(self.values) - 1)
        def y_of(v):
            return rect.bottom() - ((v - min_v) / (max_v - min_v)) * rect.height()
        pts = [(rect.left() + i * step_x, y_of(v)) for i, v in enumerate(self.values)]
        path = QPainterPath()
...
class MarketPulseTile(QFrame):
    def __init__(self, title='SCAN', parent=None):
        ...
        self.lbl_symbol.setStyleSheet('font-size: 18px; font-weight: 900; color: #dbeafe;')
    ...
    def set_data(self, symbol: str, change_pct: float, values=None):
        ...
        color = '#00ffa3' if change_pct >= 0 else '#ff4d4f'
        self.lbl_change.setStyleSheet(f'font-size: 18px; font-weight: 900; color: {color};')
        self.spark.set_series(values or [change_pct], positive=change_pct >= 0)
...
class GlowBandWidget(QWidget):
    ...
    def _draw_line(self, painter, rect, values, line_color):
        ...
        fill.closeSubpath()
        grad = QLinearGradient(rect.left(), rect.top(), rect.left(), rect.bottom())
        grad.setColorAt(0.0, QColor(line_color.red(), line_color.green(), line_color.blue(), 80))
        grad.setColorAt(1.0, QColor(0, 0, 0, 0))
...
class LaunchConfigWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        form.addRow("Плечо:", self.leverage_spin)
...
class NeonRadarWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        painter.setBrush(Qt.BrushStyle.NoBrush)
        for frac, color, w in ((1.00, QColor(65, 185, 255, 120), 2), (0.78, QColor(0, 255, 191, 120), 1), (0.56, QColor(255, 67, 214, 120), 1), (0.34, QColor(105, 245, 255, 120), 1)):
            r = radius * frac
...
class NeonGlyphWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        star = QPolygon([
            QPoint(int(center.x()), int(center.y() - radius * 0.50)),
            QPoint(int(center.x() + radius * 0.14), int(center.y() - radius * 0.14)),
            QPoint(int(center.x() + radius * 0.52), int(center.y())),
            QPoint(int(center.x() + radius * 0.14), int(center.y() + radius * 0.14)),
            QPoint(int(center.x()), int(center.y() + radius * 0.52)),
            QPoint(int(center.x() - radius * 0.14), int(center.y() + radius * 0.14)),
            QPoint(int(center.x() - radius * 0.52), int(center.y())),
            QPoint(int(center.x() - radius * 0.14), int(center.y() - radius * 0.14)),
        ])
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"OKX Turtle Bot {APP_VERSION} — Cyber Quant Terminal FullHD")
        self.resize(1920, 1080)
        self.setMinimumSize(1700, 980)
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_terminal_title = QLabel("OKX TURTLE BOT v048  —  CYBER QUANT TERMINAL")
        ...
        self.lbl_header_status_chip = QLabel("STATUS: IDLE")
        ...
        launch_box = QGroupBox("Command Deck")
        launch_layout = QVBoxLayout(launch_box)
        launch_layout.setContentsMargins(10, 12, 10, 10)
        ...
        self.lbl_session_pnl = QLabel("Session PnL: 0.00")
        ...
        for w in (self.lbl_balance_summary, self.lbl_session_pnl, self.lbl_balance_trend, self.lbl_risk_panel):
        ...
        risk_layout.setContentsMargins(10, 14, 10, 10)
        risk_layout.setHorizontalSpacing(8)
        risk_layout.setVerticalSpacing(8)
        ...
        chart_box = QGroupBox("Equity Pulse")
        chart_layout = QVBoxLayout(chart_box)
        chart_layout.setContentsMargins(10, 14, 10, 10)
        ...
        cards=[self.lbl_account,self.lbl_timeframe,self.lbl_open_pnl,self.lbl_realized,self.lbl_winrate,self.lbl_closed_stats,self.lbl_avg_open,self.lbl_best,self.lbl_worst,self.lbl_long_short,self.lbl_trade_speed]
        positions=[(0,0),(0,1),(1,0),(1,1),(2,0),(2,1),(3,0),(3,1),(4,0),(4,1),(5,0,1,2)]
        for card,pos in zip(cards,positions):
            if len(pos)==2: analytics_layout.addWidget(card,pos[0],pos[1])
            else: analytics_layout.addWidget(card,pos[0],pos[1],pos[2],pos[3])
        right_col.addWidget(analytics_box, stretch=3)

        turtle_box = QGroupBox("Turtle Signal Center")
        turtle_layout = QVBoxLayout(turtle_box)
        turtle_layout.setContentsMargins(10, 14, 10, 10)
        turtle_layout.setSpacing(8)
        self.turtle_glyph = NeonGlyphWidget()
        turtle_layout.addWidget(self.turtle_glyph, alignment=Qt.AlignmentFlag.AlignHCenter)
        self.lbl_turtle_regime = QLabel("Turtle-индикатор: —")
        self.lbl_turtle_regime.setProperty("card", "true")
        self.lbl_turtle_regime.setMinimumHeight(54)
        self.lbl_turtle_regime.setWordWrap(True)
        self.lbl_turtle_state_a = QLabel("REGIME: —")
        self.lbl_turtle_state_a.setProperty("card", "true")
        self.lbl_turtle_state_b = QLabel("TREND STRENGTH: —")
        self.lbl_turtle_state_b.setProperty("card", "true")
        self.lbl_turtle_state_c = QLabel("ATR STATE: —")
        self.lbl_turtle_state_c.setProperty("card", "true")
        self.lbl_turtle_score = QLabel("SCORE: — / 10")
        self.lbl_turtle_score.setProperty("card", "true")
        for w in (self.lbl_turtle_regime,self.lbl_turtle_state_a,self.lbl_turtle_state_b,self.lbl_turtle_state_c,self.lbl_turtle_score):
            turtle_layout.addWidget(w)
        right_col.addWidget(turtle_box, stretch=2)

        activity_box = QGroupBox("Activity Feed")
        ...
        right_col.addWidget(activity_box, stretch=3)
        ...
        layout.addWidget(self.glow_band, stretch=1)
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        for idx, (tile, (inst, pnl_pct, trend)) in enumerate(zip(self.market_tiles, radar_rows)):
            base = float(pnl_pct or 0.0)
            values = [base * 0.25 + ((j - 8) * 0.03) + trend * (0.04 if j % 2 == 0 else -0.02) for j in range(18)]
            tile.set_data(inst, pnl_pct, values)
