...
def build_app_stylesheet(is_dark: bool) -> str:
    if is_dark:
        return """
            ...
            QWidget {
                selection-background-color: #ff42d0;
                selection-color: #ffffff;
            }
            QFrame#CyberHeader {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #08101f, stop:0.35 #0a1531, stop:0.68 #120a31, stop:1 #081a30);
            ...
            QLabel[chip="true"] {
                ...
                border: 1px solid rgba(72, 191, 255, 0.70);
                border-radius: 16px;
                color: #8efcff;
                font-size: 12px;
                font-weight: 800;
            ...
            QLabel[statusChip="run"] {
                color: #13ffaf;
                border-color: rgba(19, 255, 175, 0.70);
            }
            QLabel[statusChip="stop"] {
                color: #ff5d86;
                border-color: rgba(255, 93, 134, 0.65);
            }
            ...
            QGroupBox {
                color: #7fe8ff;
                border: 1px solid rgba(50, 112, 210, 0.55);
                border-radius: 18px;
                margin-top: 14px;
                padding-top: 16px;
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
                    stop:0 rgba(7, 13, 25, 0.97),
                    stop:0.55 rgba(8, 16, 32, 0.96),
                    stop:1 rgba(14, 8, 34, 0.95));
            }
            QGroupBox::title {
            ...
            QLabel[card="true"] {
                ...
                color: #edf4ff;
                border: 1px solid rgba(63, 120, 217, 0.45);
                border-radius: 15px;
                padding: 10px 12px;
            }
            QLabel[miniMetric="true"] {
                background: rgba(8, 18, 36, 0.9);
                border: 1px solid rgba(75, 140, 255, 0.42);
                border-radius: 16px;
                padding: 10px 12px;
                color: #e9f2ff;
                font-size: 13px;
            ...
            QComboBox, QLineEdit, QTextEdit, QDoubleSpinBox, QSpinBox, QTableView, QTableWidget {
                ...
                color: #f8fbff;
                border: 1px solid rgba(69, 122, 221, 0.45);
                border-radius: 12px;
                padding: 6px 8px;
            }
            QTextEdit#ActivityFeed, QTextEdit#SystemLogView {
                background: rgba(4, 10, 22, 0.98);
                border: 1px solid rgba(62, 124, 230, 0.40);
                color: #8efcff;
                border-radius: 16px;
            }
            QHeaderView::section {
                ...
                color: #7fe8ff;
                border: 1px solid rgba(63, 120, 217, 0.36);
                padding: 9px 8px;
                font-weight: 900;
            }
            QTabWidget::pane {
                border: 1px solid rgba(65, 124, 226, 0.35);
            ...
            QTabBar::tab {
                ...
                color: #8fa7cf;
                border: 1px solid rgba(65, 124, 226, 0.32);
                border-bottom: none;
                padding: 11px 18px;
                margin-right: 6px;
            ...
            QTabBar::tab:selected {
                background: rgba(14, 28, 56, 1.0);
                color: #6ef5ff;
                border-color: rgba(255, 67, 212, 0.8);
            }
            QPushButton {
                padding: 9px 14px;
                border-radius: 13px;
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(10, 25, 46, 1.0), stop:1 rgba(16, 34, 63, 1.0));
                color: #f8fbff;
                border: 1px solid rgba(72, 189, 255, 0.6);
                font-weight: 900;
            }
            QPushButton:hover {
                border-color: rgba(255, 65, 214, 0.85);
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(12, 30, 54, 1.0), stop:1 rgba(27, 18, 64, 1.0));
            }
            QPushButton#toggleBotButton[running="true"] {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(60, 10, 24, 1.0), stop:1 rgba(92, 11, 42, 1.0));
                border: 1px solid rgba(255, 87, 119, 0.85);
                color: #ffffff;
            }
            QPushButton#toggleBotButton[running="false"] {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(6, 55, 48, 1.0), stop:1 rgba(4, 87, 64, 1.0));
                border: 1px solid rgba(19, 255, 175, 0.75);
                color: #ffffff;
            }
        """
    return """
...
class NeonRadarWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        for i, alpha in enumerate((110, 80, 55, 35)):
            r = radius * (1.0 - i * 0.18)
        ...
        painter.setBrush(Qt.BrushStyle.NoBrush)
        for frac, color, w in (
            (1.00, QColor(65, 185, 255, 120), 2),
            (0.78, QColor(0, 255, 191, 120), 1),
            (0.56, QColor(255, 67, 214, 120), 1),
            (0.34, QColor(105, 245, 255, 120), 1),
        ):
            r = radius * frac
        ...
        for angle in range(0, 360, 30):
            ...
            painter.drawLine(center, QPoint(int(x2), int(y2)))

        painter.setPen(QPen(QColor(19, 255, 175, 170), 2))
        for angle, frac in ((26, 0.88), (141, 0.62), (224, 0.48)):
            rad = math.radians(angle)
...
class NeonGlyphWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        grad.setColorAt(0.0, QColor(0, 231, 255, 55))
        grad.setColorAt(0.45, QColor(23, 96, 255, 90))
        grad.setColorAt(0.8, QColor(255, 67, 214, 35))
        grad.setColorAt(1.0, QColor(255, 67, 214, 0))
        painter.setBrush(QBrush(grad))
        painter.setPen(Qt.PenStyle.NoPen)
        ...
        painter.drawEllipse(center, int(radius * 0.66), int(radius * 0.66))
        painter.setPen(QPen(QColor(255, 67, 214, 150), 2))
        painter.drawEllipse(center, int(radius * 0.47), int(radius * 0.47))
        painter.setPen(QPen(QColor(88, 157, 255, 140), 1))
        for angle in range(0, 360, 45):
            rad = math.radians(angle)
            x1 = center.x() + math.cos(rad) * radius * 0.18
            y1 = center.y() + math.sin(rad) * radius * 0.18
            x2 = center.x() + math.cos(rad) * radius * 0.78
            y2 = center.y() + math.sin(rad) * radius * 0.78
            painter.drawLine(QPoint(int(x1), int(y1)), QPoint(int(x2), int(y2)))

        star = QPolygon([
            QPoint(center.x(), int(center.y() - radius * 0.50)),
            QPoint(int(center.x() + radius * 0.14), int(center.y() - radius * 0.14)),
            QPoint(int(center.x() + radius * 0.52), center.y()),
            QPoint(int(center.x() + radius * 0.14), int(center.y() + radius * 0.14)),
            QPoint(center.x(), int(center.y() + radius * 0.52)),
            QPoint(int(center.x() - radius * 0.14), int(center.y() + radius * 0.14)),
            QPoint(int(center.x() - radius * 0.52), center.y()),
            QPoint(int(center.x() - radius * 0.14), int(center.y() - radius * 0.14)),
        ])
        ...
        painter.setBrush(QColor(0, 242, 255, 180))
        painter.setPen(Qt.PenStyle.NoPen)
...
class LaunchConfigWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_terminal_title = QLabel("OKX TURTLE BOT v046  —  CYBER QUANT TERMINAL")
        self.lbl_terminal_title.setObjectName("CyberHeaderTitle")
        top_line.addWidget(self.lbl_terminal_title)
        top_line.addStretch(1)

        self.lbl_header_mode = QLabel("DEMO ACCOUNT")
        self.lbl_header_mode.setProperty("chip", "true")
        self.lbl_header_status_chip = QLabel("RUNNING")
        self.lbl_header_status_chip.setProperty("chip", "true")
        self.lbl_header_status_chip.setProperty("statusChip", "stop")
        self.lbl_header_tf_chip = QLabel("TF: 5m")
        ...
        self.lbl_header_uptime_chip = QLabel("—")
        ...
        for chip in (self.lbl_header_mode, self.lbl_header_status_chip, self.lbl_header_tf_chip, self.lbl_header_pos_chip, self.lbl_header_uptime_chip):
            top_line.addWidget(chip)
        header_layout.addLayout(top_line)

        self.lbl_header_sub = QLabel("Synthwave dashboard • neon execution shell • turtle trend engine")
        self.lbl_header_sub.setObjectName("CyberHeaderSub")
        header_layout.addWidget(self.lbl_header_sub)
        layout.addWidget(header, stretch=0)

        command_box = QGroupBox("Command Deck")
        command_layout = QGridLayout(command_box)
        command_layout.setContentsMargins(12, 14, 12, 12)
        command_layout.setHorizontalSpacing(10)
        ...
        command_layout.addWidget(self.start_window, 0, 0, 2, 4)
        ...
        command_layout.addWidget(self.toggle_button, 0, 4, 1, 2)
        ...
        command_layout.addWidget(self.mode_switch_toggle, 0, 6, 1, 2)

        self.btn_export_analysis = QPushButton("Сдать анализы")
        self.btn_export_analysis.clicked.connect(self.export_analysis_bundle)
        command_layout.addWidget(self.btn_export_analysis, 1, 4)

        self.btn_reset_test = QPushButton("Reset теста")
        self.btn_reset_test.clicked.connect(self.reset_test_run)
        command_layout.addWidget(self.btn_reset_test, 1, 5)

        self.btn_clear_bans = QPushButton("Сбросить бан-лист")
        self.btn_clear_bans.clicked.connect(self.clear_ban_lists)
        command_layout.addWidget(self.btn_clear_bans, 1, 6, 1, 2)

        for col, stretch in enumerate((1, 1, 1, 1, 1, 1, 1, 1)):
            command_layout.setColumnStretch(col, stretch)
        layout.addWidget(command_box, stretch=0)

        dashboard = QGridLayout()
        dashboard.setHorizontalSpacing(10)
        dashboard.setVerticalSpacing(10)

        balance_box = QGroupBox("Balance")
        balance_layout = QVBoxLayout(balance_box)
        balance_layout.setContentsMargins(16, 16, 16, 14)
        balance_layout.setSpacing(8)

        self.lbl_status = QLabel("Статус: ожидание запуска")
        self.lbl_status.setProperty("chip", "true")
        balance_layout.addWidget(self.lbl_status, 0, Qt.AlignmentFlag.AlignLeft)

        self.lbl_balance_hero_title = QLabel("TOTAL BALANCE")
        self.lbl_balance_hero_title.setProperty("metricTitle", "true")
        balance_layout.addWidget(self.lbl_balance_hero_title)

        self.lbl_balance_hero = QLabel("0 USDT")
        self.lbl_balance_hero.setProperty("heroValue", "true")
        balance_layout.addWidget(self.lbl_balance_hero)

        self.lbl_session_pnl = QLabel("SESSION PnL\\n0.00 USDT (0.00%)")
        self.lbl_session_pnl.setProperty("heroSubValue", "true")
        self.lbl_session_pnl.setWordWrap(True)
        balance_layout.addWidget(self.lbl_session_pnl)

        balance_ring_row = QHBoxLayout()
        balance_ring_row.setSpacing(10)
        balance_text_col = QVBoxLayout()
        ...
        for w in (self.lbl_balance_summary, self.lbl_balance_trend):
            balance_text_col.addWidget(w)
        balance_ring_row.addLayout(balance_text_col, 3)

        self.lbl_risk_panel = QLabel("RISK USED\\n0.00%")
        self.lbl_risk_panel.setProperty("card", "true")
        self.lbl_risk_panel.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_risk_panel.setMinimumSize(130, 130)
        balance_ring_row.addWidget(self.lbl_risk_panel, 1)
        balance_layout.addLayout(balance_ring_row)
        dashboard.addWidget(balance_box, 0, 0, 1, 3)

        chart_box = QGroupBox("Equity Chart (7D)")
        chart_layout = QVBoxLayout(chart_box)
        chart_layout.setContentsMargins(12, 14, 12, 12)
        ...
        chart_top.addWidget(self.lbl_balance_chart_title)
        ...
        self.balance_chart_step_combo.setMinimumWidth(140)
        self.balance_chart_step_combo.currentIndexChanged.connect(self.on_balance_chart_step_changed)
        chart_top.addWidget(self.balance_chart_step_combo)

        self.lbl_balance_step = QLabel("LIVE")
        self.lbl_balance_step.setProperty("chip", "true")
        chart_top.addWidget(self.lbl_balance_step)

        self.lbl_balance_points = QLabel("0/30")
        self.lbl_balance_points.setProperty("chip", "true")
        chart_top.addWidget(self.lbl_balance_points)
        chart_layout.addLayout(chart_top)

        self.balance_chart = BalanceChartWidget()
        self.balance_chart.setMinimumHeight(280)
        chart_layout.addWidget(self.balance_chart, 1)
        dashboard.addWidget(chart_box, 0, 3, 1, 4)

        metrics_wrap = QWidget()
        metrics_grid = QGridLayout(metrics_wrap)
        metrics_grid.setContentsMargins(0, 0, 0, 0)
        metrics_grid.setHorizontalSpacing(10)
        metrics_grid.setVerticalSpacing(10)

        self.lbl_open_pnl = QLabel("Open PnL\\n0")
        self.lbl_open_pnl.setProperty("miniMetric", "true")
        self.lbl_realized = QLabel("Realized PnL\\n0")
        self.lbl_realized.setProperty("miniMetric", "true")
        self.lbl_winrate = QLabel("Winrate\\n0%")
        self.lbl_winrate.setProperty("miniMetric", "true")
        self.lbl_closed_stats = QLabel("Closed Trades\\n0")
        self.lbl_closed_stats.setProperty("miniMetric", "true")
        self.lbl_long_short = QLabel("Long / Short\\n0 / 0")
        self.lbl_long_short.setProperty("miniMetric", "true")
        self.lbl_avg_open = QLabel("ATR (avg)\\n0")
        self.lbl_avg_open.setProperty("miniMetric", "true")

        for idx, card in enumerate((self.lbl_open_pnl, self.lbl_realized, self.lbl_winrate, self.lbl_closed_stats, self.lbl_long_short, self.lbl_avg_open)):
        ...
        metrics_grid.addWidget(self.lbl_best, 3, 0, 1, 2)
        metrics_grid.addWidget(self.lbl_worst, 4, 0, 1, 2)
        metrics_grid.addWidget(self.lbl_trade_speed, 5, 0, 1, 2)
        metrics_grid.addWidget(self.lbl_account, 6, 0)
        metrics_grid.addWidget(self.lbl_timeframe, 6, 1)
        dashboard.addWidget(metrics_wrap, 0, 7, 1, 2)

        turtle_box = QGroupBox("Turtle Signal Center")
        turtle_layout = QVBoxLayout(turtle_box)
        turtle_layout.setContentsMargins(12, 14, 12, 12)
        turtle_layout.setSpacing(8)

        self.turtle_emblem = NeonGlyphWidget()
        turtle_layout.addWidget(self.turtle_emblem, 0, Qt.AlignmentFlag.AlignHCenter)

        self.lbl_turtle_regime = QLabel("REGIME\\nTREND")
        self.lbl_turtle_regime.setProperty("card", "true")
        self.lbl_turtle_regime.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_turtle_regime.setMinimumHeight(58)
        self.lbl_turtle_regime.setWordWrap(True)

        self.lbl_turtle_state_a = QLabel("SCORE: —")
        self.lbl_turtle_state_a.setProperty("card", "true")
        self.lbl_turtle_state_b = QLabel("20D BREAKOUT: —")
        self.lbl_turtle_state_b.setProperty("card", "true")
        self.lbl_turtle_state_c = QLabel("55D BREAKOUT / ATR: —")
        self.lbl_turtle_state_c.setProperty("card", "true")

        for w in (self.lbl_turtle_regime, self.lbl_turtle_state_a, self.lbl_turtle_state_b, self.lbl_turtle_state_c):
            turtle_layout.addWidget(w)
        dashboard.addWidget(turtle_box, 0, 9, 2, 3)

        risk_box = QGroupBox("Risk Radar")
        risk_layout = QHBoxLayout(risk_box)
        risk_layout.setContentsMargins(12, 14, 12, 12)
        risk_layout.setSpacing(10)

        risk_left = QVBoxLayout()
        risk_left.setSpacing(8)
        self.lbl_positions = QLabel("Execution incidents: 0")
        self.lbl_positions.setProperty("card", "true")
        self.lbl_runtime = QLabel("Liquidity traps blocked: 0")
        self.lbl_runtime.setProperty("card", "true")
        self.lbl_cycle_duration = QLabel("Cycle duration: —")
        self.lbl_cycle_duration.setProperty("card", "true")
        self.lbl_blocked_count = QLabel("Quarantine markets: 0")
        ...
        for w in (self.lbl_positions, self.lbl_runtime, self.lbl_cycle_duration, self.lbl_blocked_count, self.lbl_close_pending, self.lbl_exec_watch):
            risk_left.addWidget(w)
        risk_layout.addLayout(risk_left, 3)

        self.radar_visual = NeonRadarWidget()
        risk_layout.addWidget(self.radar_visual, 2)
        dashboard.addWidget(risk_box, 1, 0, 1, 3)

        radar_box = QGroupBox("Market Status")
        radar_layout = QGridLayout(radar_box)
        radar_layout.setContentsMargins(12, 14, 12, 12)
        radar_layout.setHorizontalSpacing(10)
        radar_layout.setVerticalSpacing(10)
        self.market_radar_labels = []
        for i in range(6):
            lbl = QLabel(f"RADAR SLOT {i+1}: —")
            lbl.setProperty("card", "true")
            lbl.setMinimumHeight(58)
            self.market_radar_labels.append(lbl)
            radar_layout.addWidget(lbl, i // 3, i % 3)
        dashboard.addWidget(radar_box, 1, 3, 1, 6)

        activity_box = QGroupBox("Activity Log")
        activity_layout = QVBoxLayout(activity_box)
        activity_layout.setContentsMargins(12, 14, 12, 12)
        activity_layout.setSpacing(8)

        self.activity_feed = QTextEdit()
        self.activity_feed.setObjectName("ActivityFeed")
        self.activity_feed.setReadOnly(True)
        self.activity_feed.setMinimumHeight(200)
        activity_layout.addWidget(self.activity_feed, 3)

        self.log_preview = QTextEdit()
        self.log_preview.setObjectName("SystemLogView")
        self.log_preview.setReadOnly(True)
        self.log_preview.setMinimumHeight(140)
        activity_layout.addWidget(self.log_preview, 2)
        dashboard.addWidget(activity_box, 1, 9, 1, 3)

        for col, stretch in enumerate((3, 3, 3, 4, 4, 4, 4, 2, 2, 3, 3, 3)):
            dashboard.setColumnStretch(col, stretch)
        layout.addLayout(dashboard, stretch=7)

        positions_box = QGroupBox("Open Positions / Trading Desk")
        positions_layout = QVBoxLayout(positions_box)
        positions_layout.setContentsMargins(10, 14, 10, 12)
        positions_layout.setSpacing(8)

        footer_metrics = QHBoxLayout()
        footer_metrics.setSpacing(10)
        self.lbl_positions_footer = QLabel("Total Positions: 0/16")
        self.lbl_positions_footer.setProperty("card", "true")
        self.lbl_margin_footer = QLabel("Used Margin: 0%")
        self.lbl_margin_footer.setProperty("card", "true")
        self.lbl_available_footer = QLabel("Available: 0 USDT")
        self.lbl_available_footer.setProperty("card", "true")
        footer_metrics.addWidget(self.lbl_positions_footer)
        footer_metrics.addWidget(self.lbl_margin_footer)
        footer_metrics.addWidget(self.lbl_available_footer)
        positions_layout.addLayout(footer_metrics)
        ...
        positions_layout.addWidget(self.tabs, 1)

        action_row = QHBoxLayout()
        action_row.setSpacing(10)
        action_row.addWidget(QLabel("Главные действия вынесены в верхний Command Deck"))
        action_row.addStretch(1)
        positions_layout.addLayout(action_row)

        layout.addWidget(positions_box, stretch=5)
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        if hasattr(self, 'lbl_balance_hero'): self.lbl_balance_hero.setText('0 USDT')
        if hasattr(self, 'lbl_session_pnl'): self.lbl_session_pnl.setText('Session PnL: 0.00')
        if hasattr(self, 'lbl_header_status_chip'): self.lbl_header_status_chip.setText('STATUS: IDLE')
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        self.lbl_header_mode.setText(f"{account_name.upper()} ACCOUNT")
        ...
        self.lbl_balance_hero.setText(f"{bal_total:,.2f} USDT")
        ...
        self.lbl_turtle_regime.setText(f"Turtle-индикатор: {regime_label} | score {regime_score}/4 | {regime_inst}")
        self.lbl_turtle_state_a.setText(f"REGIME: {regime_label}")
        self.lbl_turtle_state_b.setText(f"TREND STRENGTH: ch/ATR {regime_channel:.2f} | eff {regime_eff:.2f}")
        self.lbl_turtle_state_c.setText(f"ATR STATE: {regime_atr_pct:.2f}% | MODE {mode.upper()}")
        if regime_label == "Трендовый":
            ...
            turtle_style = ""
        for lbl in (self.lbl_turtle_regime, self.lbl_turtle_state_a, self.lbl_turtle_state_b, self.lbl_turtle_state_c):
        ...
        self.lbl_header_status_chip.setText(f"STATUS: {header_status}")
        uptime_text = self.lbl_runtime.text().replace('Время работы: ', '')
        self.lbl_header_uptime_chip.setText(f"UPTIME: {uptime_text}")

        # market radar
        radar_rows = []
        for row in sorted(open_positions, key=lambda x: abs(float(x.get('pnl_pct', 0.0))), reverse=True)[:6]:
            radar_rows.append((row.get('inst_id', '—'), float(row.get('pnl_pct', 0.0)), float(row.get('trend_strength_atr', 0.0))))
        watchlist = list((getattr(self.current_cfg, 'execution_risk_watchlist', []) if self.current_cfg else []) or [])
        for inst in watchlist:
            if len(radar_rows) >= 6:
                break
            if all(r[0] != inst for r in radar_rows):
                radar_rows.append((inst, 0.0, 0.0))
        while len(radar_rows) < 6:
            radar_rows.append((f'SCAN-{len(radar_rows)+1}', 0.0, 0.0))
        for lbl, (inst, pnl_pct, trend) in zip(self.market_radar_labels, radar_rows):
            arrow = '↑' if pnl_pct > 0.2 else ('↓' if pnl_pct < -0.2 else '→')
            lbl.setText(f"{inst}  {pnl_pct:+.2f}%  {arrow}  trend {trend:.2f} ATR")
            color = '#00ffa3' if pnl_pct > 0 else ('#ff4d4f' if pnl_pct < 0 else '#67e8f9')
            lbl.setStyleSheet(f"color: {color}; font-weight: 800;")
    ...
    def on_status(self, message: str) -> None:
        ...
        if "запущен" in lower:
            ...
            if hasattr(self, 'lbl_header_status_chip'): self.lbl_header_status_chip.setText('STATUS: RUNNING')
        elif "остановлен" in lower:
            self._bot_running = False
            self.lbl_status.setStyleSheet("color: #ff4d4f; font-weight: 800;")
            if hasattr(self, 'lbl_header_status_chip'): self.lbl_header_status_chip.setText('STATUS: STOPPED')
        else:
