        ...
        return """
            ...
            QFrame#terminalHeader {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #0b1020, stop:0.45 #10152a, stop:1 #0a0d18);
            ...
            QLabel[chip_role="ok"] { color: #00ffa3; }
            QLabel[chip_role="warn"] { color: #ffb347; }
            QLabel[chip_role="bad"] { color: #ff5c7a; }
            QGroupBox {
                font-weight: 800;
                color: #82dcff;
            ...
            QComboBox, QLineEdit, QTextEdit, QDoubleSpinBox, QSpinBox, QTableView, QTableWidget {
                ...
                border: 1px solid #223456;
                border-radius: 12px;
                selection-background-color: #103866;
                selection-color: #ffffff;
                gridline-color: #16263f;
            }
            ...
            QPushButton:hover { border-color: #45e8ff; color: #ffffff; }
            QPushButton:disabled { color: #6d7c98; border-color: #1a2438; }
            ...
            QPushButton#modeButton { background: #0c1d1c; border: 1px solid #00ffa3; }
            QPushButton#resetButton { background: #22132b; border: 1px solid #ff4fd8; }
            QPushButton#exportButton { background: #0e1630; border: 1px solid #45e8ff; }
            QPushButton#secondaryButton { background: #0d1525; border: 1px solid #3a5d92; }
        """
    return """
        QMainWindow, QWidget { background: #f5f7fb; color: #162033; }
    """
...
class TurtleEngine(QObject):
    ...
    def evaluate_entry(self, inst_id: str, cycle_id: Optional[int] = None) -> list[dict]:
        ...
        for system in systems:
            ...
            # Ближе к классической Turtle: принимаем только свежий пробой на последней закрытой свече.
    ...
    def _confirm_breakout(self, candles: list, atr: float, side: str, level: float) -> tuple[bool, str]:
        """
        v034:
        Подтверждение ближе к классической Turtle: свежий пробой канала на последней закрытой свече
        без старого зависания над/под уровнем.
        """
        ...
        last = candles[-1]
        prev = candles[-2]
        last_high = float(last[2]); last_low = float(last[3]); last_close = float(last[4])
        prev_high = float(prev[2]); prev_low = float(prev[3])

        breakout_buffer = max(atr * 0.02, abs(level) * 0.00005)

        if side == "long":
            if prev_high >= level:
                return False, "пробой не свежий: предыдущая свеча уже была на уровне/выше канала"
            if last_high < level:
                return False, "нет пробоя верхнего канала"
            if last_close < level - breakout_buffer:
                return False, "свеча вернулась под верхний канал к закрытию"
            return True, "classic_turtle_fresh_breakout_long"

        if prev_low <= level:
            return False, "пробой не свежий: предыдущая свеча уже была на уровне/ниже канала"
        if last_low > level:
            return False, "нет пробоя нижнего канала"
        if last_close > level + breakout_buffer:
            return False, "свеча вернулась над нижний канал к закрытию"
        return True, "classic_turtle_fresh_breakout_short"
...
class StartWindow(QWidget):
    ...
    def build_config(self) -> BotConfig:
        ...
        return BotConfig(
            ...
            max_open_positions_total=16,
            max_open_positions_per_side=0,
            blacklist=["USDC-USDT-SWAP", "XSR-USDT-SWAP"],
        )
...
class MainWindow(QMainWindow):
    def apply_system_theme(self) -> None:
        ...
        stylesheet = build_app_stylesheet(True)
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"OKX Turtle Bot {APP_VERSION} — Cyberpunk Quant Trading Terminal")
        ...
        self.telegram_ui_notifier: Optional[TelegramNotifier] = None
        self.telegram_screenshot_timer: Optional[QTimer] = None
        self.telegram_screenshot_interval_ms = 15 * 60 * 1000
        self._build_ui()
        self.apply_system_theme()
    ...
    def _make_chip(self, text: str, role: str = "") -> QLabel:
        lbl = QLabel(text)
        lbl.setProperty("chip", "true")
        if role:
            lbl.setProperty("chip_role", role)
        lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        return lbl

    def _make_card_value(self, title: str, value: str = "—", small: bool = False) -> tuple[QWidget, QLabel]:
        card = QFrame()
        ...
        title_lbl.setProperty("panelTitle", "true")
        value_lbl = QLabel(value)
        value_lbl.setProperty("metricSmall" if small else "metric", "true")
        lay.addWidget(title_lbl)
        lay.addWidget(value_lbl)
        lay.addStretch(1)
        return card, value_lbl

    def _make_radar_item(self, symbol: str) -> tuple[QFrame, QLabel, QLabel]:
        frame = QFrame()
        ...
        lay.setSpacing(2)
        sym = QLabel(symbol)
        sym.setProperty("panelTitle", "true")
        change = QLabel("—")
        change.setProperty("metricSmall", "true")
        detail = QLabel("waiting")
        detail.setStyleSheet("color:#88a7d8;")
        lay.addWidget(sym)
        lay.addWidget(change)
        lay.addWidget(detail)
        return frame, change, detail
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_terminal_title = QLabel("CYBERPUNK QUANT TRADING TERMINAL")
        self.lbl_terminal_title.setObjectName("terminalTitle")
        self.lbl_terminal_subtitle = QLabel(f"OKX TURTLE BOT {APP_VERSION}")
        self.lbl_terminal_subtitle.setObjectName("terminalSubtitle")
        chips = QHBoxLayout()
        chips.setSpacing(8)
        self.chip_account = self._make_chip("DEMO ACCOUNT")
        self.chip_status = self._make_chip("STANDBY", "warn")
        self.chip_tf = self._make_chip("TF: —")
        self.chip_pos = self._make_chip("POS: 0 / 16")
        self.chip_uptime = self._make_chip("UPTIME: 00:00:00")
        for w in (self.chip_account, self.chip_status, self.chip_tf, self.chip_pos, self.chip_uptime):
            chips.addWidget(w)
        chips.addStretch(1)
        header_layout.addWidget(self.lbl_terminal_title)
        header_layout.addWidget(self.lbl_terminal_subtitle)
        ...
        layout.addWidget(self.start_window, 0)

        top = QHBoxLayout()
        top.setSpacing(8)
        left_col = QVBoxLayout(); left_col.setSpacing(8)
        mid_col = QVBoxLayout(); mid_col.setSpacing(8)
        right_col = QVBoxLayout(); right_col.setSpacing(8)

        balance_box = QGroupBox("Balance Hub")
        balance_layout = QVBoxLayout(balance_box)
        balance_layout.setContentsMargins(12, 12, 12, 12)
        self.lbl_balance_hub_title = QLabel("BALANCE")
        self.lbl_balance_hub_title.setProperty("panelTitle", "true")
        self.lbl_balance_hero = QLabel("0.00 USDT")
        self.lbl_balance_hero.setProperty("hero", "true")
        self.lbl_balance_session = QLabel("Session PnL: 0")
        self.lbl_balance_session.setProperty("metricSmall", "true")
        self.lbl_balance_today = QLabel("TODAY 0.00%  |  7D 0.00%")
        self.lbl_balance_today.setProperty("card", "true")
        self.lbl_risk_gauge = QLabel("RISK USED 0.00% / 0.00%")
        self.lbl_risk_gauge.setProperty("card", "true")
        self.lbl_balance_summary = QLabel("Баланс: 0 | Использовано: 0 | Доступно: 0")
        self.lbl_balance_summary.setProperty("card", "true")
        for w in (self.lbl_balance_hub_title, self.lbl_balance_hero, self.lbl_balance_session, self.lbl_balance_today, self.lbl_risk_gauge, self.lbl_balance_summary):
            balance_layout.addWidget(w)
        left_col.addWidget(balance_box, 2)

        risk_box = QGroupBox("Risk Radar")
        risk_layout = QVBoxLayout(risk_box)
        self.lbl_status = QLabel("Статус: ожидание запуска"); self.lbl_status.setProperty("card", "true")
        self.lbl_cycle_duration = QLabel("Цикл движка: —"); self.lbl_cycle_duration.setProperty("card", "true")
        self.lbl_positions = QLabel("Открытых позиций: 0"); self.lbl_positions.setProperty("card", "true")
        self.lbl_runtime = QLabel("Время работы: —"); self.lbl_runtime.setProperty("card", "true")
        self.lbl_trade_speed = QLabel("Сделок сегодня: 0 | Средняя длительность: —"); self.lbl_trade_speed.setProperty("card", "true")
        self.lbl_exec_incidents = QLabel("Execution incidents: 0"); self.lbl_exec_incidents.setProperty("card", "true")
        self.lbl_liquidity_traps = QLabel("Liquidity traps blocked: 0"); self.lbl_liquidity_traps.setProperty("card", "true")
        self.lbl_close_pending = QLabel("Close pending: 0"); self.lbl_close_pending.setProperty("card", "true")
        self.lbl_quarantine = QLabel("Quarantine markets: 0"); self.lbl_quarantine.setProperty("card", "true")
        for w in (self.lbl_status, self.lbl_cycle_duration, self.lbl_positions, self.lbl_runtime, self.lbl_trade_speed, self.lbl_exec_incidents, self.lbl_liquidity_traps, self.lbl_close_pending, self.lbl_quarantine):
            risk_layout.addWidget(w)
        left_col.addWidget(risk_box, 3)

        chart_box = QGroupBox("Equity Chart")
        chart_layout = QVBoxLayout(chart_box)
        header_row = QHBoxLayout()
        ...
        self.lbl_balance_step = self._make_chip("Шаг: 1m")
        self.lbl_balance_points = self._make_chip("Показано значений: 0/30")
        header_row.addWidget(self.balance_chart_step_combo)
        header_row.addWidget(self.lbl_balance_step)
        header_row.addWidget(self.lbl_balance_points)
        header_row.addStretch(1)
        chart_layout.addLayout(header_row)
        self.balance_chart = BalanceChartWidget()
        self.balance_chart.setMinimumHeight(220)
        chart_layout.addWidget(self.balance_chart, 1)
        mid_col.addWidget(chart_box, 3)

        metrics_box = QGroupBox("Analytics")
        metrics_grid = QGridLayout(metrics_box)
        cards = []
        self.lbl_open_pnl_card, self.lbl_open_pnl = self._make_card_value("OPEN PNL", "0.0000", small=True)
        self.lbl_realized_card, self.lbl_realized = self._make_card_value("REALIZED PNL", "0.0000", small=True)
        self.lbl_winrate_card, self.lbl_winrate = self._make_card_value("WINRATE", "0.00%", small=True)
        self.lbl_closed_card, self.lbl_closed_stats = self._make_card_value("CLOSED TRADES", "0", small=True)
        self.lbl_longshort_card, self.lbl_long_short = self._make_card_value("LONG / SHORT", "0 / 0", small=True)
        self.lbl_avgopen_card, self.lbl_avg_open = self._make_card_value("AVG PNL %", "0.00%", small=True)
        self.lbl_best_card, self.lbl_best = self._make_card_value("BEST PNL %", "0.00%", small=True)
        self.lbl_worst_card, self.lbl_worst = self._make_card_value("WORST PNL %", "0.00%", small=True)
        card_widgets = [self.lbl_open_pnl_card, self.lbl_realized_card, self.lbl_winrate_card, self.lbl_closed_card, self.lbl_longshort_card, self.lbl_avgopen_card, self.lbl_best_card, self.lbl_worst_card]
        for i, card in enumerate(card_widgets):
            metrics_grid.addWidget(card, i // 2, i % 2)
        mid_col.addWidget(metrics_box, 2)

        turtle_box = QGroupBox("Turtle Signal Center")
        ...
        self.lbl_account = QLabel("Аккаунт: —"); self.lbl_account.setProperty("card", "true")
        self.lbl_timeframe = QLabel("Таймфрейм: —"); self.lbl_timeframe.setProperty("card", "true")
        for w in (self.lbl_turtle_regime, self.lbl_balance_trend, self.lbl_account, self.lbl_timeframe):
            turtle_layout.addWidget(w)
        right_col.addWidget(turtle_box, 2)

        radar_box = QGroupBox("Market Radar")
        radar_layout = QGridLayout(radar_box)
        self.radar_symbols = ["BTC", "ETH", "ATOM", "MINA", "LINK", "SOL"]
        self.market_radar_widgets = []
        for idx, sym in enumerate(self.radar_symbols):
            frame, change, detail = self._make_radar_item(sym)
            radar_layout.addWidget(frame, idx // 2, idx % 2)
            self.market_radar_widgets.append((sym, change, detail))
        right_col.addWidget(radar_box, 2)
        ...
        right_col.addWidget(activity_box, 3)

        top.addLayout(left_col, 3)
        top.addLayout(mid_col, 4)
        top.addLayout(right_col, 3)
        layout.addLayout(top, 5)

        button_row = QHBoxLayout(); button_row.setSpacing(8)
        self.toggle_button = QPushButton("Запустить бота"); self.toggle_button.setObjectName("toggleBotButton"); self.toggle_button.clicked.connect(self.toggle_engine)
        self.lbl_blocked_count = self._make_chip("Блокировок: 0")
        self.mode_switch_toggle = QPushButton("Режим: Авто"); self.mode_switch_toggle.setObjectName("modeButton"); self.mode_switch_toggle.setCheckable(True); self.mode_switch_toggle.clicked.connect(self.on_trade_mode_changed); self.mode_switch_toggle.setMinimumWidth(170)
        self.btn_clear_bans = QPushButton("Очистить блокировки"); self.btn_clear_bans.setObjectName("secondaryButton"); self.btn_clear_bans.clicked.connect(self.clear_ban_lists)
        self.btn_reset_test = QPushButton("RESET TEST"); self.btn_reset_test.setObjectName("resetButton"); self.btn_reset_test.clicked.connect(self.reset_test_run)
        self.btn_export_analysis = QPushButton("EXPORT ANALYSIS"); self.btn_export_analysis.setObjectName("exportButton"); self.btn_export_analysis.clicked.connect(self.export_analysis_bundle)
        for w in (self.toggle_button, self.lbl_blocked_count, self.mode_switch_toggle, self.btn_clear_bans, self.btn_reset_test, self.btn_export_analysis):
            button_row.addWidget(w)
        button_row.addStretch(1)
        layout.addLayout(button_row, 0)

        self.tabs = QTabWidget(); self.tabs.setDocumentMode(True)
        self.table = QTableView(); self.table.setModel(self.table_model); self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch); self.table.setAlternatingRowColors(True); self.table.doubleClicked.connect(self.show_open_position_context)
        self.closed_table = QTableView(); self.closed_table.setModel(self.closed_table_model); self.closed_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch); self.closed_table.setAlternatingRowColors(True); self.closed_table.doubleClicked.connect(self.show_closed_trade_context)
        self.blocked_table = QTableWidget(); self.blocked_table.setColumnCount(4); self.blocked_table.setHorizontalHeaderLabels(["Инструмент", "Тип", "Причина", "Осталось"]); self.blocked_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch); self.blocked_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers); self.blocked_table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows); self.blocked_table.setAlternatingRowColors(True)
        self.tabs.addTab(self.table, "POSITIONS")
        self.tabs.addTab(self.closed_table, "CLOSED TRADES")
        self.tabs.addTab(self.log_text, "LOG")
        self.tabs.addTab(self.blocked_table, "WATCHLIST")
    ...
    def _collect_blocked_rows(self) -> list[tuple[str, str, str, str]]:
        ...
        for inst_id in sorted(all_inst):
            ...
            if inst_id in illiquid_map:
                ...
                if until_ts > now_ts:
                    rows.append((inst_id, "Неликвидный рынок", base_reason or "illiquidity-filter", self._format_remaining(until_ts - now_ts)))
                    continue
            if inst_id in temp_blocked:
                until_ts = float(temp_blocked.get(inst_id, 0.0) or 0.0)
                if until_ts > now_ts:
                    rows.append((inst_id, "Временный бан", base_reason or "exchange/temp block", self._format_remaining(until_ts - now_ts)))
                    continue
            if inst_id in stopouts:
                data = stopouts.get(inst_id) or {}
                until_ts = float(data.get("until", 0.0) or 0.0)
                if until_ts > now_ts:
                    rows.append((inst_id, "Cooldown после стопа", str(data.get("reason") or "cooldown after stop"), self._format_remaining(until_ts - now_ts)))
                    continue
    ...
    def refresh_blocked_instruments_view(self) -> None:
        ...
        self.blocked_table.setRowCount(max(1, len(rows)))
        if not rows:
            rows = [("—", "—", "Активных блокировок нет", "—")]
        for row_idx, row_values in enumerate(rows):
            for col_idx, value in enumerate(row_values):
                self.blocked_table.setItem(row_idx, col_idx, QTableWidgetItem(str(value)))
    ...
    def _stop_telegram_screenshot_timer(self) -> None:
        if self.telegram_screenshot_timer is not None:
            self.telegram_screenshot_timer.stop()
            self.telegram_screenshot_timer.deleteLater()
            self.telegram_screenshot_timer = None
    ...
    def _configure_telegram_screenshots(self, cfg: Optional[BotConfig]) -> None:
        ...
        if not self._telegram_enabled_in_cfg(cfg):
            return
        try:
            self.telegram_ui_notifier = TelegramNotifier(enabled=True, bot_token=str(getattr(cfg, "telegram_bot_token", "") or "").strip(), chat_id=str(getattr(cfg, "telegram_chat_id", "") or "").strip())
            self.telegram_screenshot_timer = QTimer(self)
            self.telegram_screenshot_timer.timeout.connect(self.send_main_window_screenshot_to_telegram)
            self.telegram_screenshot_timer.start(self.telegram_screenshot_interval_ms)
    ...
    def send_main_window_screenshot_to_telegram(self) -> None:
        ...
        if notifier is None or not getattr(notifier, "enabled", False) or not self.isVisible():
            return
        try:
            ...
            pixmap = self.grab()
            if pixmap.isNull() or not pixmap.save(str(file_path), "PNG"):
                raise RuntimeError("не удалось сохранить PNG")
            notifier.send_photo(str(file_path), caption=f"OKX Turtle Bot {APP_VERSION} | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _clear_ui_runtime_data(self) -> None:
        ...
        self.lbl_open_pnl.setText("0")
        self.lbl_realized.setText("0")
        self.lbl_closed_stats.setText("0")
        self.lbl_winrate.setText("0%")
    ...
    def reset_test_run(self) -> None:
        answer = QMessageBox.question(self, "Reset теста", "Остановить бота, попытаться закрыть все SWAP-позиции и очистить локальные логи/состояние?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if answer != QMessageBox.StandardButton.Yes:
            return
        if bool(self.worker and self.worker.isRunning()):
            self.stop_engine()
        try:
            closed, close_errors = self._reset_remote_positions()
        except Exception as exc:
            closed, close_errors = 0, [str(exc)]
        reset_info = reset_local_runtime_files()
        if self.engine is not None:
            self.engine.position_state.clear(); self.engine.closed_trades.clear()
            with self.engine.balance_lock:
                self.engine.balance_history.clear(); self.engine.latest_balance_snapshot = {}
        ...
        self._clear_ui_runtime_data(); self.refresh_blocked_instruments_view()
        self.append_log(f"Reset теста выполнен: закрыто позиций {closed}, удалено entry-context {reset_info['entry_context_removed']}, trade-context {reset_info['trade_context_removed']}")
        if close_errors:
    ...
    def export_analysis_bundle(self) -> None:
        try:
            ...
            QMessageBox.information(self, "Сдать анализы", f"Диагностический архив готов.\n\nФайл: {archive_path}\n\nЕго можно загружать в чат для разбора работы стратегии.")
        except Exception as exc:
    ...
    def clear_ban_lists(self) -> None:
        if self.engine is None:
            self.refresh_blocked_instruments_view(); self.append_log("Бан-лист очищен (движок ещё не запущен)")
            return
        for attr in ("blocked_instruments", "temp_blocked_until", "illiquid_instruments", "recent_stopouts", "illiquid_rejections", "execution_risk_events"):
            ...
            if isinstance(data, dict):
                data.clear()
        self.refresh_blocked_instruments_view(); self.append_log("Все заблокированные инструменты удалены из бан-листа")

    def set_pending_config(self, cfg: BotConfig) -> None:
        self.current_cfg = cfg
        self._apply_trade_mode_to_controls(getattr(cfg, "trade_mode", "auto"))
        self.append_log(f"Параметры обновлены: {'Основной' if cfg.flag == '0' else 'Демо'}, {cfg.timeframe}, режим: {getattr(cfg, 'trade_mode', 'auto')}")
    ...
    def _apply_trade_mode_to_controls(self, mode: str) -> None:
        ...
        self.mode_switch_toggle.setText("MANUAL MODE" if normalized == "manual" else "AUTO MODE")
    ...
    def on_trade_mode_changed(self, *_args) -> None:
        mode = "manual" if self.mode_switch_toggle.isChecked() else "auto"
        ...
        if self.engine is not None:
            self.append_log(f"Режим торговли переключён: {mode}")

    def on_entry_candidate(self, payload: dict) -> None:
        self._pending_manual_signal = payload
        ...
        text = (
            f"Найден сигнал на новую позицию\n\n"
            f"Инструмент: {inst_id}\n"
            f"Сторона: {side}\n"
            ...
            f"Цена: {price:.6f}\n"
            f"ATR: {atr:.6f}\n\n"
            "Торгуем или пропускаем?"
        )
        box = QMessageBox(self)
        box.setWindowTitle("Ручной режим")
        box.setText(text)
    ...
    def _sync_toggle_button_state(self) -> None:
        if self._bot_running:
            ...
            self.chip_status.setText("RUNNING"); self.chip_status.setProperty("chip_role", "ok")
        else:
            self.toggle_button.setText("START BOT")
            self.toggle_button.setProperty("running", False)
            self.chip_status.setText("STANDBY"); self.chip_status.setProperty("chip_role", "warn")
        for w in (self.toggle_button, self.chip_status):
            w.style().unpolish(w); w.style().polish(w); w.update()

    def toggle_engine(self) -> None:
        if self.worker and self.worker.isRunning():
            self.stop_engine(); return
        try:
            cfg = self.start_window.build_config()
        except Exception as exc:
            QMessageBox.critical(self, "Ошибка параметров", str(exc)); return
        self.current_cfg = cfg
        self.launch_engine(cfg)

    def launch_engine(self, cfg: BotConfig) -> None:
        if self.worker and self.worker.isRunning():
            QMessageBox.information(self, "Уже запущен", "Сначала останови текущего бота"); return
        self.current_cfg = cfg; self._configure_telegram_screenshots(cfg)
        try:
            self.engine = TurtleEngine(cfg)
        except Exception as exc:
            QMessageBox.critical(self, "Ошибка запуска", str(exc)); return
        self.engine.snapshot.connect(self.on_snapshot); self.engine.log_line.connect(self.append_log); self.engine.status.connect(self.on_status); self.engine.error.connect(self.on_error); self.engine.entry_candidate.connect(self.on_entry_candidate)
        self.worker = WorkerThread(self.engine); self.worker.start(); self._bot_running = True
        self._snapshot_refresh_interval_sec = max(1, int(getattr(cfg, "snapshot_interval_sec", 2) or 2)); self._snapshot_countdown_sec = 0; self._bot_started_ts = time.time()
        self._apply_trade_mode_to_controls(getattr(cfg, "trade_mode", "auto")); self._sync_toggle_button_state(); self.refresh_blocked_instruments_view()
        self.append_log(f"Бот запущен пользователем (шаг: {cfg.timeframe})")
        self.append_log(f"Signal audit: {SIGNAL_AUDIT_FILE}")

    def stop_engine(self) -> None:
        if self.engine:
            self.engine._set_manual_entry_decision(False); self.engine.stop(); self.append_log("Остановка запрошена")
        self._pending_manual_signal = None; self._manual_dialog_open = False
        if self.worker:
            self.worker.quit(); self.worker.wait(2000); self.worker = None
        self._bot_running = False; self._bot_started_ts = None; self._snapshot_countdown_sec = self._snapshot_refresh_interval_sec
        self._sync_toggle_button_state(); self.refresh_blocked_instruments_view(); self.lbl_runtime.setText("Время работы: —")
    ...
    def request_snapshot(self) -> None:
        if self.engine and self.worker and self.worker.isRunning():
            try:
                self.engine.emit_snapshot(); return
            except Exception as exc:
    ...
    def _update_runtime_label(self) -> None:
        if self._bot_started_ts and self.engine and self.worker and self.worker.isRunning():
            elapsed = max(0, int(time.time() - self._bot_started_ts)); hours, rem = divmod(elapsed, 3600); minutes, seconds = divmod(rem, 60)
            txt = f"Время работы: {hours:02d}:{minutes:02d}:{seconds:02d}"
            self.lbl_runtime.setText(txt)
            self.chip_uptime.setText(f"UPTIME: {hours:02d}:{minutes:02d}:{seconds:02d}")
        else:
            self.lbl_runtime.setText("Время работы: —"); self.chip_uptime.setText("UPTIME: 00:00:00")

    def _on_gui_timer_tick(self) -> None:
        if self.engine and self.worker and self.worker.isRunning():
            self._snapshot_countdown_sec -= 1
            if self._snapshot_countdown_sec <= 0:
                self.request_snapshot(); self.refresh_blocked_instruments_view(); self._snapshot_countdown_sec = self._snapshot_refresh_interval_sec
        else:
            self._snapshot_countdown_sec = self._snapshot_refresh_interval_sec
    ...
    def _update_market_radar(self, payload: dict) -> None:
        open_rows = list(payload.get("open_positions", []) or [])
        picks = {}
        for row in open_rows:
            picks[str(row.get("inst_id", "")).split('-')[0]] = row
        analytics = payload.get("analytics", {}) or {}
        regime_inst = str(analytics.get("turtle_regime_instrument", "BTC-USDT-SWAP")).split('-')[0]
        for sym, value_lbl, detail_lbl in self.market_radar_widgets:
            row = picks.get(sym)
            if row:
                pnl_pct = float(row.get("pnl_pct", 0.0) or 0.0)
                value_lbl.setText(f"{pnl_pct:+.2f}%")
                color = "#00ffa3" if pnl_pct >= 0 else "#ff5c7a"
                value_lbl.setStyleSheet(f"color:{color}; font-size:20px; font-weight:800;")
                detail_lbl.setText(f"{str(row.get('side','')).upper()}  |  risk {float(row.get('execution_risk_score',0.0) or 0.0):.2f}")
            else:
                if sym == regime_inst:
                    value_lbl.setText(f"ATR {float(analytics.get('turtle_regime_atr_pct',0.0) or 0.0):.2f}%")
                    value_lbl.setStyleSheet("color:#76f7ff; font-size:18px; font-weight:800;")
                    detail_lbl.setText(f"regime {analytics.get('turtle_regime_label','—')}")
                else:
                    value_lbl.setText("—")
                    value_lbl.setStyleSheet("color:#d7e7ff; font-size:18px; font-weight:700;")
                    detail_lbl.setText("watching")

    def on_snapshot(self, payload: dict) -> None:
        self.latest_snapshot = payload
        settings = payload.get('settings', {})
        self.lbl_account.setText(f"Аккаунт: {settings.get('account', '—')}")
        self.lbl_timeframe.setText(f"Таймфрейм: {settings.get('timeframe', '—')}")
        self.chip_account.setText(f"{settings.get('account', '—').upper()} ACCOUNT")
        self.chip_tf.setText(f"TF: {settings.get('timeframe', '—')}")
        self.chip_pos.setText(f"POS: {len(payload.get('open_positions', []))} / {getattr(self.current_cfg, 'max_open_positions_total', 16) if self.current_cfg else 16}")
        mode = settings.get('trade_mode', getattr(self.current_cfg, 'trade_mode', 'auto'))
        self._apply_trade_mode_to_controls(mode)
        balance_total = float(payload.get('balance_total', 0.0) or 0.0)
        balance_used = float(payload.get('balance_used', 0.0) or 0.0)
        balance_available = float(payload.get('balance_available', 0.0) or 0.0)
        self.lbl_balance_summary.setText(f"Баланс: {balance_total:.0f} | Использовано: {balance_used:.0f} | Доступно: {balance_available:.0f}")
        self.lbl_balance_hero.setText(f"{balance_total:,.2f} USDT")
        self.lbl_positions.setText(f"Открытых позиций: {len(payload.get('open_positions', []))}")
        ...
        analytics = payload.get('analytics', {}) or {}
        self.lbl_open_pnl.setText(f"{analytics.get('open_pnl', 0.0):.4f}")
        self.lbl_avg_open.setText(f"{analytics.get('avg_open_pnl_pct', 0.0):.2f}%")
        self.lbl_best.setText(f"{analytics.get('best_open_pnl_pct', 0.0):.2f}%")
        self.lbl_worst.setText(f"{analytics.get('worst_open_pnl_pct', 0.0):.2f}%")
        self.lbl_long_short.setText(f"{analytics.get('long_count', 0)} / {analytics.get('short_count', 0)}")
        self.lbl_realized.setText(f"{analytics.get('realized_pnl', 0.0):.4f}")
        self.lbl_closed_stats.setText(f"{analytics.get('closed_count', 0)}")
        self.lbl_winrate.setText(f"{analytics.get('winrate', 0.0):.2f}%")
        ...
        self.lbl_risk_gauge.setText(f"RISK USED {analytics.get('used_risk_pct', 0.0):.2f}% / {analytics.get('max_risk_budget_pct', 0.0):.2f}%")
        self.lbl_balance_session.setText(f"Session PnL  {analytics.get('realized_pnl', 0.0) + analytics.get('open_pnl', 0.0):+.2f} USDT")
        self.lbl_balance_today.setText(f"TODAY {analytics.get('day_change_pct', 0.0):+.2f}%   |   7D {analytics.get('week_change_pct', 0.0):+.2f}%")
        ...
        self.lbl_turtle_regime.setText(f"Turtle Signal Center: {regime_label} | score {regime_score}/4 | {regime_inst} | ch/ATR {regime_channel:.2f} | eff {regime_eff:.2f} | ATR {regime_atr_pct:.2f}%")
        ...
        # risk radar
        engine_map = engine_info if isinstance(engine_info, dict) else {}
        self.lbl_exec_incidents.setText(f"Execution incidents: {int(engine_map.get('execution_risk_markets', 0) or 0)}")
        self.lbl_liquidity_traps.setText(f"Liquidity traps blocked: {int(analytics.get('execution_risk_count', 0) or 0)}")
        pending = sum(1 for row in payload.get('open_positions', []) if row.get('close_pending'))
        self.lbl_close_pending.setText(f"Close pending: {pending}")
        self.lbl_quarantine.setText(f"Quarantine markets: {len(getattr(self.engine, 'temp_blocked_until', {}) or {}) if self.engine else 0}")
        self._update_market_radar(payload)
        self.apply_filters()
    ...
    def _apply_status_style(self, label: QLabel, value: float, percent: bool = False) -> None:
        try:
            ...
            numeric = 0.0
        color = '#00ffa3' if numeric > 0 else ('#ff5c7a' if numeric < 0 else '#d7e7ff')
        label.setStyleSheet(f"color: {color}; font-weight: 800;")

    def apply_filters(self) -> None:
        if not self.latest_snapshot:
            self.table_model.update_rows([]); self.closed_table_model.update_rows([]); return
        open_rows = [row for row in self.latest_snapshot.get('open_positions', []) if not is_hidden_instrument(row.get('inst_id'))]
        open_rows.sort(key=lambda x: float(x.get('pnl_pct', 0.0)), reverse=True)
        self.table_model.update_rows(open_rows)
        closed_rows = [row for row in self.latest_snapshot.get('closed_trades', []) if not is_hidden_instrument(row.get('inst_id'))]
    ...
    def append_log(self, message: str) -> None:
        ...
        while doc.blockCount() > 400:
            cursor = self.log_text.textCursor(); cursor.movePosition(cursor.MoveOperation.Start); cursor.select(cursor.SelectionType.BlockUnderCursor); cursor.removeSelectedText(); cursor.deleteChar()

    def on_status(self, message: str) -> None:
        self.lbl_status.setText(f"Статус: {message}")
        lower = message.lower()
        if 'запущен' in lower:
            self._bot_running = True; self.lbl_status.setStyleSheet('color:#00ffa3; font-weight:800;')
        elif 'остановлен' in lower:
            self._bot_running = False; self.lbl_status.setStyleSheet('color:#ff5c7a; font-weight:800;')
        else:
            self.lbl_status.setStyleSheet('color:#d7e7ff; font-weight:700;')
        self._sync_toggle_button_state(); self.append_log(message)
