...
class MarketDataCache:
    ...
    def put_ticker(self, inst_id: str, ticker: dict) -> None:
        with self.lock:
            self.ticker_cache[inst_id] = dict(ticker)
            self.ticker_ts[inst_id] = time.time()
...
HIDDEN_INSTRUMENTS = {"BREV-USDT-SWAP", "LINK-USDT-SWAP", "QTUM-USDT-SWAP", "TSLA-USDT-SWAP"}
...
def build_analysis_export_bundle(snapshot: Optional[dict] = None, cfg: Optional["BotConfig"] = None) -> Path:
    ...
    for row in engine_event_rows:
        ...
        if event_name not in include_events:
            continue
        compact_engine_events.append(row)

    compact_signal_rows: List[dict] = []
    signal_stages = {"candidate", "ranked_candidate", "rejected", "entry_sent", "entry_failed", "entry_skipped_manual", "rotation_started", "rotation_completed", "rotation_failed"}
    ...
    trade_headers = [
        "time", "inst_id", "side", "qty", "entry_px", "exit_px", "pnl", "pnl_pct", "duration_sec",
        "duration_human", "units", "system_name", "reason", "entry_atr", "initial_stop_price",
        "final_stop_price", "peak_price", "trough_price", "peak_unrealized_pnl", "channel_exit_level", "trade_context_file",
    ]
    _write_csv(export_dir / "trades_export.csv", trade_headers, trades_export_rows)

    journal_headers = ["ts", "event", "inst_id", "side", "price", "stop_price", "prev_stop_price", "qty", "units", "unrealized_pnl", "peak_unrealized_pnl", "reason", "note"]
    journal_csv_rows = []
    for row in journal_rows:
        journal_csv_rows.append({
            "ts": row.get("ts", ""),
            "event": row.get("event", ""),
            "inst_id": row.get("inst_id", ""),
            "side": row.get("side", ""),
            ...
            "prev_stop_price": _safe_float(row.get("prev_stop_price", 0.0)),
            "qty": _safe_float(row.get("qty", 0.0)),
    ...
    _write_csv(export_dir / "position_journal.csv", journal_headers, journal_csv_rows)
...
class BotConfig:
    ...
    blacklist: List[str] = field(default_factory=lambda: ["USDC-USDT-SWAP", "XSR-USDT-SWAP", "BREV-USDT-SWAP", "LINK-USDT-SWAP", "QTUM-USDT-SWAP", "TSLA-USDT-SWAP"])
...
class TurtleEngine(QObject):
    ...
    def evaluate_entry(self, inst_id: str, cycle_id: Optional[int] = None) -> list[dict]:
        ...
        if not signals:
            if cycle_id is not None:
                self._audit_signal(cycle_id, inst_id, "rejected", reason="нет свежего пробоя Donchian на последней закрытой свече", price=price, atr=atr)
            return []
    ...
    def enter_position(self, inst_id: str, side: str, price: float, atr: float, system_name: str) -> bool:
        ...
        self.position_journal_logger.log("OPEN", inst_id=inst_id, side=side, price=price, stop_price=stop_price, qty=qty, units=1, atr=atr, system_name=system_name, note="Первичный вход")
        self._save_state()
    ...
    def try_pyramid(self, state: PositionState, last_close: float, candles: List[List[float]]) -> None:
        ...
        self.position_journal_logger.log("ADD_UNIT", inst_id=state.inst_id, side=state.side, price=last_close, stop_price=state.stop_price, qty=state.qty, units=state.units, unrealized_pnl=state.unrealized_pnl, peak_unrealized_pnl=getattr(state, "peak_unrealized_pnl", 0.0), note=f"Добавлен unit #{state.units}")
        self.stats_logger.log(
            "pyramid_added",
            inst_id=state.inst_id,
            side=state.side,
    ...
    def close_position(self, state: PositionState, price: float, reason: str, candles: Optional[List[List[float]]] = None) -> None:
        ...
        self.position_journal_logger.log("CLOSE", inst_id=state.inst_id, side=state.side, price=price, stop_price=state.stop_price, qty=state.qty, units=state.units, unrealized_pnl=state.unrealized_pnl, peak_unrealized_pnl=getattr(state, "peak_unrealized_pnl", 0.0), reason=reason, note="Выход из позиции")
        duration_sec = 0
        try:
...
class StartWindow(QWidget):
    ...
    def build_config(self) -> BotConfig:
        ...
        return BotConfig(
            ...
            blacklist=["USDC-USDT-SWAP", "XSR-USDT-SWAP", "BREV-USDT-SWAP", "LINK-USDT-SWAP", "QTUM-USDT-SWAP", "TSLA-USDT-SWAP"],
        )
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setMinimumSize(1480, 1120)
        self.resize(1480, 1120)
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_balance_chart_title = QLabel("Баланс аккаунта")
        ...
        layout.addWidget(self.tabs, stretch=18)
    ...
    def reset_test_run(self) -> None:
        ...
        if self.engine is not None:
            ...
            for attr in ("blocked_instruments", "temp_blocked_until", "illiquid_instruments", "recent_stopouts", "illiquid_rejections", "close_retry_after"):
    ...
    def export_analysis_bundle(self) -> None:
        try:
            ...
            QMessageBox.information(
                ...
                f"Файл: {archive_path}\n\n"
                "Его можно загружать в чат для разбора ночного теста.",
            )
        except Exception as exc:
    ...
    def append_log(self, message: str) -> None:
        ...
        if "BREV-" in upper_message or "LINK-USDT" in upper_message or "QTUM-USDT" in upper_message or "TSLA-USDT" in upper_message:
            return
