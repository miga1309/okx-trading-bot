...
#
# Changelog v041:
# - Expanded "Сдать анализы" export into a fuller diagnostic bundle for deep strategy analysis
# - Added config snapshot, full engine/signal logs, entry-context index, rejection counters and richer trade metrics
# - Added stable trade_id through entry, journal, lifecycle context and closed-trade export
# - Added richer signal-audit fields: Donchian levels, candle extremes and reject_code classification
# - Removed the old UI wording about analyzing only night tests
#
# Previous changelog:
# Changelog v040:
...
class TurtleEngine(QObject):
    ...
    def try_pyramid(self, state: PositionState, last_close: float, candles: List[List[float]]) -> None:
        ...
        self.position_journal_logger.log("ADD_UNIT", trade_id=getattr(state, "trade_id", ""), inst_id=state.inst_id, side=state.side, price=last_close, stop_price=state.stop_price, qty=state.qty, units=state.units, unrealized_pnl=state.unrealized_pnl, peak_unrealized_pnl=getattr(state, "peak_unrealized_pnl", 0.0), note=f"Добавлен unit #{state.units}")
        self.stats_logger.log(
            "pyramid_added",
            inst_id=state.inst_id,
            side=state.side,
