...
class TurtleEngine(QObject):
    ...
    def scan_markets(self) -> None:
        ...
        for rank, candidate in enumerate(candidates, start=1):
            ...
            self.enter_position(candidate["inst_id"], candidate["side"], candidate["price"], candidate["atr"], candidate["system_name"])
            opened += 1
    ...
    def evaluate_entry(self, inst_id: str, cycle_id: Optional[int] = None) -> list[dict]:
        ...
        if not liquid_ok:
            if cycle_id is not None and bool(getattr(self.cfg, "signal_audit_log_all_rejections", True)):
                self._audit_signal(cycle_id, inst_id, "rejected", reason=f"liquidity: {liquid_reason}", price=price, atr=atr)
        ...
        liquidity_score = max(0.0, vol_24h / max(float(self.cfg.liquidity_min_24h_quote_volume or 1.0), 1.0))
        if spread_pct > 0:
            liquidity_score += max(0.0, float(self.cfg.liquidity_max_spread_pct) / spread_pct)
...
class MainWindow(QMainWindow):
    ...
    def _reset_remote_positions(self) -> tuple[int, list[str]]:
        ...
        for pos in positions:
            ...
            if qty <= 0:
                continue
            resp = gateway.close_position(inst_id, "reset_test")
            if resp.get("code") == "0":
                closed += 1
            else:
