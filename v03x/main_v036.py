    ...
    def _pick_rotation_victim(self, incoming_candidate: dict) -> tuple[Optional[PositionState], str]:
        ...
        for state in self.position_state.values():
            ...
            if int(getattr(state, "units", 1) or 1) > threshold:
                continue
            pnl_pct = 0.0
            if float(getattr(state, "margin", 0.0) or 0.0) > 0:
                pnl_pct = (float(getattr(state, "unrealized_pnl", 0.0) or 0.0) / float(state.margin)) * 100.0
            else:
                avg_px = float(getattr(state, "avg_px", 0.0) or 0.0)
                last_px = float(getattr(state, "last_px", avg_px) or avg_px)
                if avg_px > 0:
                    pnl_pct = ((last_px - avg_px) / avg_px) * 100.0 if state.side == "long" else ((avg_px - last_px) / avg_px) * 100.0
            if require_negative and pnl_pct >= 0:
                continue
