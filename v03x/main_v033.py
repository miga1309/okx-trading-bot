...
def reset_local_runtime_files() -> dict:
    ...
    for file_path in (ENGINE_STATS_FILE, APP_LOG):
        try:
    ...
    def scan_markets(self) -> None:
        ...
        for inst_id in self.gateway.swap_ids:
            ...
            if inst_id in self.position_state:
                continue
            try:
                candidates.extend(self.evaluate_entry(inst_id))
            except Exception as exc:
        ...
        for candidate in candidates:
            ...
            if total_slots > 0 and len(self.position_state) >= total_slots:
                self.log_line.emit(f"{candidate['inst_id']}: вход пропущен — достигнут общий лимит открытых позиций: {len(self.position_state)}/{total_slots}")
                continue
        ...
        if candidates:
            self.stats_logger.log("entry_candidates_ranked", total=len(candidates), opened=opened, timeframe=self.cfg.timeframe)


    def evaluate_entry(self, inst_id: str) -> list[dict]:
        profile = self._tf_entry_profile()
