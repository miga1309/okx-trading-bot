...
class TradeLifecycleChartWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        start_stop = self._safe_float(self.payload.get("start_stop_price"), 0.0)
        final_stop = self._safe_float(self.payload.get("final_stop_price"), 0.0)
        channel_level = self._safe_float(self.payload.get("channel_exit_level"), 0.0)
        prices.extend([p for p in (entry_price, exit_price, start_stop, final_stop, channel_level) if p > 0])
        ...
        draw_hline(entry_price, "ENTRY", QColor(30, 144, 255))
        draw_hline(exit_price, "EXIT", QColor(255, 193, 7))
        draw_hline(start_stop, "Старт. стоп", QColor(220, 53, 69), Qt.PenStyle.DotLine)
        draw_hline(final_stop, "Фин. стоп", QColor(160, 50, 50), Qt.PenStyle.DashLine)
        draw_hline(channel_level, "Канал выхода", QColor(111, 66, 193), Qt.PenStyle.DashDotLine)
        ...
        for marker in markers:
            try:
                ...
                if idx < 0 or idx >= len(x_coords):
                    continue
                x = x_coords[idx]
                price = self._safe_float(marker.get("price"), 0.0)
                y = self._price_to_y(price if price > 0 else candles[idx][4], min_price, max_price, plot_top, plot_height)
                kind = str(marker.get("kind") or "").lower()
                label = str(marker.get("label") or "")
                color = QColor(30, 144, 255)
                if kind.startswith("add"):
                    color = QColor(32, 201, 151)
                elif kind == "exit":
                ...
                painter.setPen(label_pen)
                painter.drawText(x + 6, max(plot_top + 12, y - 8), label)
            except Exception:
                continue
