...
# - Fixed GUI crash in BalanceChartWidget paintEvent (undefined pnl_values reference)
# - Fixed runtime state saving for balance history (balance_history_copy NameError)
# - Seeded balance cache before starting the balance poller to reduce duplicate startup balance requests
#
# Previous changelog:
# Changelog v032_2:
...
class TurtleEngine(QObject):
    ...
    def start_balance_poller(self) -> None:
        if self.balance_poller_running:
            return
        if not self.latest_balance_snapshot:
            try:
                account = self.gateway.get_account_balance()
                self._append_balance_point_from_account(account)
            except Exception as exc:
                logging.warning("Initial balance preload failed: %s", exc)
