    ...
    def show_open_position_context(self, index) -> None:
        try:
            ...
            for open_row in list(latest_snapshot.get("open_positions") or []):
                if str(open_row.get("inst_id") or "").strip() == inst_id and (not trade_id or str(open_row.get("trade_id") or "").strip() == trade_id):
            ...
            registry_state = None
            try:
                if engine is not None and hasattr(engine, "position_state"):
            ...
            if registry_state is not None:
                for key in (
                    ...
                    try:
                        live_state[key] = getattr(registry_state, key)
                    except Exception:
                        pass
