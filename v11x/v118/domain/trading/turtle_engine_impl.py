    ...
    def sync_positions_from_exchange(self) -> None:
        ...
        for inst_id in list(self.position_state):
            if inst_id not in seen:
                ...
                finalize_threshold = max(3, int(getattr(self.cfg, "missing_exchange_finalize_count", 3) or 3))
                in_entry_recovery = self._entry_recovery_active(prev_state)
                stop_fault = stop_state in {"ERROR", "MISSING"} or health_state in {"STOP_UNVERIFIED", "ERROR"}
                should_finalize = absence_synced or close_pending or protective_pending or missing_count >= finalize_threshold
                if stop_fault and not in_entry_recovery:
