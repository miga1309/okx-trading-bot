...
class StopEngine:
    ...
    def _sync_absent_position_once(self, state: PositionState, current_price: float = 0.0, reason: str = "position_absent_on_exchange") -> bool:
        ...
        if self._stop_management_disabled(state):
            return False
        attempts = max(2, int(getattr(self.cfg, "position_absence_confirm_attempts", 3) or 3))
        delay_sec = max(0.2, float(getattr(self.cfg, "position_absence_confirm_delay_sec", 0.35) or 0.35))
        observed_absent = 0
        for attempt in range(1, attempts + 1):
            ...
            for row in positions:
                ...
                if qty > 0.0:
                    still_present = True
                    break
            if not still_present and self._find_existing_exchange_stop_row(state, target_stop=float(getattr(state, "stop_price", 0.0) or 0.0), force_rest=True) is not None:
                still_present = True
            if still_present:
    ...
    def _amend_exchange_stop(self, state: PositionState, target_stop: float, current_price: float = 0.0, reason: str = "") -> bool:
        ...
        for attempt in range(1, attempts + 1):
            ...
            if fail_code == "51003" or "client order id or order id is required" in lower_msg:
                ...
                self._finish_stop_action(state, False)
                if relinked and bool(getattr(state, "exchange_stop_full_position", False)):
                ...
                if relinked:
                    ...
                    return self._force_replace_exchange_stop(state, target_stop, current_price=current_price, reason=reason or "amend_relinked_partial")
