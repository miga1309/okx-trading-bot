...
class MarketDataCache:
    ...
    def _prune_stale_entries(self, ordered: List[str], bar: str) -> tuple[int, int, int]:
        keep = set(ordered)
        keep_candle_keys = {(inst_id, bar) for inst_id in ordered}
        ticker_pruned = 0
        candle_pruned = 0
        history_pruned = 0
        with self.lock:
            stale_tickers = [inst_id for inst_id in list(self.ticker_cache.keys()) if inst_id not in keep]
            for inst_id in stale_tickers:
                self.ticker_cache.pop(inst_id, None)
                self.ticker_ts.pop(inst_id, None)
                ticker_pruned += 1
            stale_histories = [inst_id for inst_id in list(self.ticker_history.keys()) if inst_id not in keep]
            for inst_id in stale_histories:
                self.ticker_history.pop(inst_id, None)
                history_pruned += 1
            stale_candles = [key for key in list(self.candles_cache.keys()) if key not in keep_candle_keys]
            for key in stale_candles:
                self.candles_cache.pop(key, None)
                self.candles_ts.pop(key, None)
                candle_pruned += 1
        return ticker_pruned, candle_pruned, history_pruned
        ...
        while self.running:
            ...
            for inst_id in ordered:
                ...
                try:
                    ...
                    transient_markers = ("winerror 10035", "timed out", "timeout", "readerror", "temporarily", "socket", "completed immediately")
                    if any(marker in lowered for marker in transient_markers):
                        self.transient_error_count += 1
                        self.last_error = message
                        logging.warning("Market-data worker temporary fetch failure for %s: %s", inst_id, message)
                    else:
                        self.error_count += 1
                        self.last_error = message
                        logging.error("Market-data worker fetch failed for %s: %s", inst_id, exc)
                time.sleep(max(0.02, float(getattr(self.cfg, "market_data_worker_sleep_sec", 0.12) or 0.12)))

            ticker_pruned, candle_pruned, history_pruned = self._prune_stale_entries(ordered, bar)
            ...
            if self.last_cycle_at - self.last_log_at >= log_every:
                ...
                missing_tickers = max(0, len(ordered) - len(self.ticker_cache))
                self._log(
                    f"Market-data worker: instruments={len(ordered)}, fetched={fetched_this_cycle}, "
                    f"ticker_cache={len(self.ticker_cache)}, candle_cache={len(self.candles_cache)}, "
                    f"missing_tickers={missing_tickers}, pruned_tickers={ticker_pruned}, pruned_candles={candle_pruned}, "
                    f"errors={self.error_count}, transient_errors={self.transient_error_count}"
                )
            if not ordered:
                time.sleep(0.25)
    ...
    def snapshot_stats(self) -> dict:
        return {
            ...
            "transient_error_count": int(getattr(self, "transient_error_count", 0)),
