...
class UpdateManager:
    ...
    def cleanup_loaded_artifacts(self, loaded: LoadedManifest) -> list[str]:
        cleaned: list[str] = []
        try:
            manifest_root = loaded.manifest_path.parent.resolve()
            updates_full_root = (self.updates_root / "full").resolve()
            if manifest_root == updates_full_root:
                payload_dir = (manifest_root / str(loaded.manifest.get("payload_dir") or "payload")).resolve()
                if payload_dir.exists() and payload_dir.is_dir():
                    for child in sorted(payload_dir.iterdir()):
                        try:
                            ...
                            cleaned.append(str(child.relative_to(self.project_root)))
                        except Exception:
                            self.log.exception("remote_update_cleanup_failed", extra={"path": str(child)})
                if loaded.manifest_path.exists():
                    try:
                        loaded.manifest_path.unlink(missing_ok=True)
                        cleaned.append(str(loaded.manifest_path.relative_to(self.project_root)))
                    except Exception:
                        self.log.exception("remote_update_manifest_cleanup_failed", extra={"path": str(loaded.manifest_path)})
        except Exception:
            self.log.exception("remote_update_cleanup_unhandled_failed")
        return cleaned
