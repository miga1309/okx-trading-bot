...
class MarketScannerRuntime:
    ...
    def run_chunk(self, chunk_size: Optional[int] = None) -> int:
        ...
        inter_symbol_pause_sec = max(0.0, float(getattr(self.cfg, "scanner_symbol_pause_sec", 0.05) or 0.0))
        processed = 0
        while self._queue and processed < quota:
            inst_id = self._queue.popleft()
            self.refresh_symbol(inst_id)
            processed += 1
            if self._queue and processed < quota and inter_symbol_pause_sec > 0.0:
                time.sleep(inter_symbol_pause_sec)
        return processed
    ...
    def refresh_for_entry(self, inst_id: str, force: bool = False) -> ScannerStatus:
        inst_id = str(inst_id).upper()
        status = self.get_status(inst_id)
        recent_enough = status.updated_at > 0.0 and (time.time() - float(status.updated_at or 0.0)) <= min(20.0, max(5.0, float(getattr(self.cfg, "scanner_status_ttl_sec", 90) or 90) / 3.0))
        if force and recent_enough and not status.failed and not status.pending:
            return status
    ...
    def allows_entry(self, inst_id: str) -> tuple[bool, str, ScannerStatus]:
        ...
        raw = dict(getattr(status, 'raw', {}) or {})
        if bool(raw.get('book_stale_hard')):
            return False, 'scanner_book_stale', status
        if bool(raw.get('candle_stale_hard')):
            return False, 'scanner_candle_stale', status
        if bool(raw.get('thin_book')):
            return False, 'scanner_thin_book', status
        if bool(raw.get('liquidity_hole')):
            return False, 'scanner_liquidity_hole', status
        if status.admitted:
    ...
    def refresh_symbol(self, inst_id: str) -> ScannerStatus:
        ...
        if inst_id.split('-', 1)[0].startswith('TEST'):
            ...
            status.filter_type = 'BLOCK'
            status.primary_reason = 'test_instrument'
            status.summary_text = 'test_instrument'
            status.admitted = False
            status.raw = {'test_instrument': True}
            return status
        status.pending = True
        status.failed = False
        status.error = ""
        try:
            ...
            ws_manager = getattr(self.gateway, 'ws_manager', None)
            long_ws_ok = bool(ws_manager and ws_manager.get_candles_snapshot(inst_id, tf_long, min(8, max(2, long_limit // 4))) is not None)
            fast_ws_ok = bool(ws_manager and ws_manager.get_candles_snapshot(inst_id, tf_fast, min(8, max(2, fast_limit // 4))) is not None)
            analysis = self._analyze_symbol(inst_id, long_candles, fast_candles, trades, book_rows)
            grace_sec = max(0.0, float(getattr(self.cfg, 'scanner_ws_grace_period_sec', 180.0) or 180.0))
            startup_in_grace = (time.time() - self._last_reset_ts) <= grace_sec
            has_rest_candles = len(long_candles) >= max(20, min(long_limit, 40)) and len(fast_candles) >= max(20, min(fast_limit, 40))
            has_ws_book = any(str(r.get('source') or '') == 'ws' for r in book_rows)
            has_any_book = any((r.get('spread_bps') is not None) or float(r.get('depth_sum') or 0.0) > 0.0 for r in book_rows)
            analysis['candle_stale'] = bool(getattr(self.cfg, 'ws_candles_enabled', True)) and not (long_ws_ok and fast_ws_ok)
            analysis['book_stale'] = bool(getattr(self.cfg, 'ws_books_enabled', True)) and not has_ws_book
            analysis['candle_stale_hard'] = bool(analysis['candle_stale']) and (not has_rest_candles) and (not startup_in_grace)
            analysis['book_stale_hard'] = bool(analysis['book_stale']) and (not has_any_book) and (not startup_in_grace)
    ...
    def update_validation(self, inst_id: str, filter_type: str, verdict: str, comment: str = "", tags=None) -> None:
        ...
        if not record:
            ...
            self._upsert_review_record(status)
            record = self._review_records.get(key)
        if not record and filter_type == 'GOOD':
            now_ts = time.time()
            record = {
                ...
                'reason': 'Проверка хорошей сделки', 'user_verdict': VERDICT_UNCHECKED, 'review_tags': [], 'comment': '', 'verdict_time': 0.0,
                'recheck_needed': False, 'active': True, 'metrics': {}, 'side': '', 'timeframe': str(getattr(self.cfg, 'timeframe', '5m') or '5m'), 'popup_candles': [],
            }
    ...
    def _analyze_symbol(self, inst_id: str, long_candles: List[List[float]], fast_candles: List[List[float]], trades: List[dict], book_rows: List[dict]) -> Dict[str, Any]:
        ...
        if dead_flag:
            ...
            reason_codes = "allow"
            final_score = 0.0
        spread_values = [float(r.get('spread_bps')) for r in book_rows if r.get('spread_bps') is not None]
        depth_values = [float(r.get('depth_sum') or 0.0) for r in book_rows]
        hole_values = [float(r.get('liquidity_hole_ratio') or 1.0) for r in book_rows]
        book_metrics = {
            'spread_bps': self._safe_fmean(spread_values, 0.0),
            'depth_sum': self._safe_fmean(depth_values, 0.0),
            'liquidity_hole_ratio': min(hole_values or [1.0]),
        }
        book_data_ready = bool(spread_values) or any(v > 0.0 for v in depth_values)
        thin_book = book_data_ready and (book_metrics['depth_sum'] <= float(getattr(self.cfg, 'ws_book_min_side_notional_usdt', 2500.0) or 2500.0) * 2.0)
        liquidity_hole = book_data_ready and (book_metrics['liquidity_hole_ratio'] <= float(getattr(self.cfg, 'ws_book_liquidity_hole_ratio', 0.22) or 0.22))
        metrics = dict(metrics or {})
        metrics.update({f'book_{k}': v for k, v in book_metrics.items()})
        return {
            "inst_id": inst_id,
    ...
    def _compute_dead_metrics(self, long_candles, fast_candles, trades, book_rows):
        ...
        spread_bps = self._safe_fmean(spreads, 0.0) if spreads else 0.0
        top_depth = self._safe_fmean(depths, 0.0) if depths else 0.0
    ...
    def _compute_ripping_metrics(self, fast_candles, trades, book_rows):
        ...
        closes = [float(c[4]) for c in candles if len(c) >= 5]
        median_range = self._median(ranges)
        mean_range = self._safe_fmean(ranges, 0.0) if ranges else 0.0
        atr_ratio = (statistics.fmean(ranges[-20:]) / closes[-1]) if ranges and closes and closes[-1] else 0.0
        ...
        sizes_mean = self._safe_fmean(sizes, 0.0)
        trade_size_cv = (statistics.pstdev(sizes) / sizes_mean) if len(sizes) > 1 and sizes_mean > 0 else 0.0
        burst_trade_ratio = self._burst_trade_ratio(trades)
        flow_ripping = trade_size_cv >= float(getattr(self.cfg, 'scanner_ripping_trade_size_cv_min', 3.5) or 3.5) or burst_trade_ratio >= float(getattr(self.cfg, 'scanner_ripping_burst_trade_ratio_min', 0.75) or 0.75)
        spreads = [float(r.get('spread_bps') or 0.0) for r in book_rows if r.get('spread_bps') is not None]
        depths = [float(r.get('depth_sum') or 0.0) for r in book_rows if r.get('depth_sum') is not None]
        spread_bps = self._safe_fmean(spreads, 0.0) if spreads else 0.0
        ...
        depth_mean = self._safe_fmean(depths, 0.0) if depths else 0.0
        ...
        book_ripping = (spread_ready and spread_cv >= float(getattr(self.cfg, 'scanner_ripping_spread_cv_min', 0.10) or 0.10)) or (depth_ready and depth_cv >= float(getattr(self.cfg, 'scanner_ripping_depth_cv_min', 0.20) or 0.20)) or top_depth <= float(getattr(self.cfg, 'scanner_ripping_depth_usdt_max', 1200.0) or 1200.0)
    ...
    def _collect_liquidity(self, inst_id: str) -> List[Dict[str, Any]]:
        ...
        for snap_no in range(1, snapshots + 1):
            try:
                metrics = dict(getattr(self.gateway, 'get_orderbook_metrics')(inst_id, depth_size=depth_size) or {})
                rows.append({
                    'symbol': inst_id,
                    'snapshot_no': snap_no,
                    'ts': metrics.get('ts'),
                    ...
                    'depth_sum': float(metrics.get('depth_sum') or 0.0),
                    'top5_bid_size_sum': float(metrics.get('bid_notional') or 0.0),
                    'top5_ask_size_sum': float(metrics.get('ask_notional') or 0.0),
                    'liquidity_hole_ratio': float(metrics.get('liquidity_hole_ratio') or 0.0),
                })
            except Exception:
                rows.append({'symbol': inst_id, 'snapshot_no': snap_no, 'ts': None, 'source': 'error', 'spread_bps': None, 'depth_sum': 0.0, 'liquidity_hole_ratio': 0.0})
