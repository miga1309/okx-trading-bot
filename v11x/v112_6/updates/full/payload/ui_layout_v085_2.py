...
def build_main_window_ui(window, deps):
    ...
    window.lbl_header_tg_chip = QLabel("TG: OFF")
    window.lbl_header_tg_chip.setProperty("chip", "true")
    for chip in (
    ...
    window.btn_tg_start = QPushButton("TG START")
    window.btn_tg_start.setObjectName("reloadUIButton")
    window.btn_tg_start.setCursor(Qt.CursorShape.PointingHandCursor)
    window.btn_tg_start.setMinimumHeight(30)
    window.btn_tg_start.clicked.connect(window.start_telegram_worker)
    header_layout.addWidget(window.btn_tg_start, 0, Qt.AlignmentFlag.AlignVCenter)

    window.btn_tg_stop = QPushButton("TG STOP")
    window.btn_tg_stop.setObjectName("reloadUIButton")
    window.btn_tg_stop.setCursor(Qt.CursorShape.PointingHandCursor)
    window.btn_tg_stop.setMinimumHeight(30)
    window.btn_tg_stop.clicked.connect(window.stop_telegram_worker)
    header_layout.addWidget(window.btn_tg_stop, 0, Qt.AlignmentFlag.AlignVCenter)
    ...
    window.lbl_balance_hero.setStyleSheet("font-size: 18px; font-weight: 900; padding: 14px 14px;")
    window.lbl_balance_used = QLabel("USED\n0 USDT")
    window.lbl_balance_used.setProperty("card", "true")
    window.lbl_balance_used.setStyleSheet("font-size: 18px; font-weight: 900; padding: 14px 14px;")
    window.lbl_balance_available = QLabel("AVAILABLE\n0 USDT")
    window.lbl_balance_available.setProperty("card", "true")
    window.lbl_balance_available.setStyleSheet("font-size: 18px; font-weight: 900; padding: 14px 14px;")
    ...
    window.lbl_balance_summary.hide()
    ...
    summary_grid.addWidget(window.lbl_session_pnl, 0, 0)
    summary_grid.addWidget(window.lbl_risk_panel, 0, 1)
    summary_grid.addWidget(window.lbl_balance_trend, 1, 0, 1, 2)
    ...
    window.trading_desk_widget = QWidget()
    trading_desk_layout = QVBoxLayout(window.trading_desk_widget)
    trading_desk_layout.setContentsMargins(0, 0, 0, 0)
    trading_desk_layout.setSpacing(8)
    trading_desk_layout.addWidget(window.table)

    trading_desk_actions = QHBoxLayout()
    trading_desk_actions.setContentsMargins(0, 0, 0, 0)
    trading_desk_actions.addStretch(1)
    window.btn_manual_add_unit = QPushButton("Повысить юнит")
    window.btn_manual_add_unit.setVisible(False)
    window.btn_manual_add_unit.setEnabled(False)
    window.btn_manual_add_unit.setStyleSheet(
        "QPushButton { background:#0b5ed7; color:white; border:none; border-radius:12px; padding:8px 14px; font-weight:800; }"
        "QPushButton:hover { background:#0d6efd; }"
        "QPushButton:disabled { background:#334155; color:#94a3b8; }"
    )
    window.btn_manual_add_unit.clicked.connect(window.show_manual_add_unit_dialog)
    trading_desk_actions.addWidget(window.btn_manual_add_unit)
    trading_desk_layout.addLayout(trading_desk_actions)
    try:
        window.table.selectionModel().selectionChanged.connect(window.on_open_position_selection_changed)
    except Exception:
        pass
    window.tabs.addTab(window.trading_desk_widget, "Trading Desk")
