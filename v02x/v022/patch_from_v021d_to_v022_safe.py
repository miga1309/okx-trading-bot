...
def main() -> None:
    try:
        ...
        old_block = '''    def _block_illiquid_instrument(self, inst_id: str, reason: str) -> None:
        until_ts = time.time() + self.cfg.illiquid_block_hours * 3600
        self.illiquid_instruments[inst_id] = until_ts
        self.temp_blocked_until[inst_id] = until_ts
        logging.info("%s: инструмент временно заблокирован как неликвидный на %sч (%s)", inst_id, self.cfg.illiquid_block_hours, reason)

'''
        new_block = '''    def _block_illiquid_instrument(self, inst_id: str, reason: str) -> None:
...
'''
        text = replace_once(text, old_block, new_block, "_block_illiquid_instrument")
...
""",
        ...
        old_refresh = '''    def refresh_blocked_instruments_view(self) -> None:
        rows = self._collect_blocked_rows()
        self.lbl_blocked_count.setText(f"Блокировок: {len(rows)}")
        if not rows:
            self.blocked_text.setPlainText("Активных блокировок нет")
            return

        lines = []
        for inst_id, block_type, reason, ttl in rows:
            lines.append(f"{inst_id} | {block_type} | {reason} | {ttl}")
...
'''
        text = replace_once(text, old_refresh, new_refresh, "refresh_blocked_instruments_view")

        # 10
        log("Шаг 10/12: clear_ban_lists")
        old_clear = '''    def clear_ban_lists(self) -> None:
...
'''
        text = replace_once(text, old_clear, new_clear, "clear_ban_lists")
...
""",
        ...
        if dst.exists():
            log(f"Делаю backup старого {dst.name}")
            shutil.copy2(dst, backup)

        log(f"Записываю {dst.name}")
