...
# Version: v027
# Date: 2026-03-12
# Based on: main_v025_3.py
#
# Changelog:
# - Added auto/manual trading mode with always-available GUI switch
# - In manual mode, new entry signals now open a confirmation popup: trade or skip
# - Manual mode affects only creation of new positions; position management stays automatic
# - Preserved accelerated snapshot refresh from v026_2 and updated version metadata
# ============================================================
...
class StartWindow(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.trade_mode_combo.addItem("Автоматический", "auto")
        self.trade_mode_combo.addItem("Ручной", "manual")
        form.addRow("Режим торговли:", self.trade_mode_combo)
    ...
    def build_config(self) -> BotConfig:
        ...
        return BotConfig(
            ...
            trade_mode=self.trade_mode_combo.currentData(),
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.mode_switch_combo = QComboBox()
        self.mode_switch_combo.addItem("Авто", "auto")
        self.mode_switch_combo.addItem("Ручной", "manual")
        self.mode_switch_combo.currentIndexChanged.connect(self.on_trade_mode_changed)
        self.mode_switch_combo.setMinimumWidth(130)
        button_row.addWidget(self.mode_switch_combo)
    ...
    def _apply_trade_mode_to_controls(self, mode: str) -> None:
        ...
        if hasattr(self, "mode_switch_combo"):
            idx = 1 if normalized == "manual" else 0
            blocked = self.mode_switch_combo.blockSignals(True)
            self.mode_switch_combo.setCurrentIndex(idx)
            self.mode_switch_combo.blockSignals(blocked)
        if hasattr(self, "start_window") and hasattr(self.start_window, "trade_mode_combo"):
            idx = 1 if normalized == "manual" else 0
            blocked = self.start_window.trade_mode_combo.blockSignals(True)
            self.start_window.trade_mode_combo.setCurrentIndex(idx)
            self.start_window.trade_mode_combo.blockSignals(blocked)

    def on_trade_mode_changed(self) -> None:
        mode = self.mode_switch_combo.currentData() if hasattr(self, "mode_switch_combo") else "auto"
    ...
    def on_entry_candidate(self, payload: dict) -> None:
        ...
        message = (
            f"Найден новый сигнал для входа.\n\n"
            f"Инструмент: {inst_id}\n"
            f"Направление: {side}\n"
            f"Система: {system_name}\n"
        ...
        answer = QMessageBox.question(self, "Ручной режим: найден сигнал", message, QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.Yes)
    ...
    def on_snapshot(self, payload: dict) -> None:
        ...
        self._apply_trade_mode_to_controls(payload.get('settings', {}).get('trade_mode', 'auto'))
